...
class RPCServer {
  ...
  Future<void> start() async {
    ...
    try {
      ...
      print('✅ RPC server listening on http://${_server!.address.host}:${_server!.port}');
    } catch (e) {
  ...
  Middleware _loggingMiddleware() {
    return (Handler handler) {
      return (Request request) async {
        ...
        print('[RPC ${request.method}] ${request.url.path} - ${response.statusCode} (${duration.inMilliseconds}ms)');
