...
/// IO-specific WebTransport dialer.
///
/// WebTransport relies on the browser's `WebTransport` API and has no IO
/// backend, so [dial] always throws [TransportUnavailableException].
class WebTransportDialerIO implements WebTransportDialer {
  ...
  Future<libp2p.Conn> dial(libp2p.MultiAddr addr) async {
    ...
    throw TransportUnavailableException.forPlatform('WebTransport', 'IO');
  }
}
