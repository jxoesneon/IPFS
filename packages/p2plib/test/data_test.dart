...
void main() async {
  ...
  test('MessageHeader equality and serialization', () {
    final headerA = PacketHeader(
      issuedAt: DateTime.timestamp().millisecondsSinceEpoch,
      id: genRandomInt(),
    );
    final headerB = headerA.copyWith(messageType: PacketType.confirmation);
    final headerC = headerA.copyWith(id: genRandomInt());
  ...
  test('Route.getActualAddresses, removeStaleAddresses', () {
    final now = DateTime.timestamp().millisecondsSinceEpoch;
    ...
    final route = Route(
      ...
      addresses: {
        ...
        actualAddress: AddressProperties(isLocal: true, lastSeen: now),
        staleAddress: AddressProperties(isLocal: true, lastSeen: staleAt - 1),
