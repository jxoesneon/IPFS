...
class MockStunServer {
  ...
  void _handleRequest(Datagram datagram) {
    ...
    final addressBytes =
        mappedAddress.address.split('.').map((e) => int.parse(e)).toList();
    ...
    for (var i = 0; i < 4; i++) {
      xorAddressBytes[i + 4] =
          addressBytes[i] ^ ((StunMessage.magicCookie >> (8 * (3 - i))) & 0xFF);
    }
    ...
    if (_otherAddress != null && _otherPort != null) {
      ...
      final otherIpBytes =
          _otherAddress!.address.split('.').map((e) => int.parse(e)).toList();
  ...
  void _handleChangeRequest(
      StunMessage request, Datagram datagram, Uint8List changeRequestData) {
    if (changeRequestData.length < 4) return;

    final buffer = ByteData.view(
        changeRequestData.buffer, changeRequestData.offsetInBytes);
    ...
    final mappedAddress =
        responseConfig.mappedAddress ?? _simulatedAddress ?? datagram.address;
    final mappedPort =
        responseConfig.mappedPort ?? _simulatedPort ?? datagram.port;
    ...
    final originIpBytes = responseConfig.responseOrigin!.address
        .split('.')
        .map((e) => int.parse(e))
        .toList();
