  ...
  group('MemoryPeerstore', () {
    ...
    test('should retrieve peer with addresses after addOrUpdatePeer', () async {
      ...
      if (retrievedPeerInfo != null) {
        expect(retrievedPeerInfo.addrs, isNotEmpty,
            reason: 'PeerInfo.addrs should not be empty.');
        expect(retrievedPeerInfo.addrs.length, equals(multiaddrs.length),
            reason: 'PeerInfo.addrs length should match added addresses.');
        for (var addr in multiaddrs) {
          expect(retrievedPeerInfo.addrs.contains(addr), isTrue,
    ...
    test('addOrUpdatePeer with protocols and metadata', () async {
      ...
      peerstore.addOrUpdatePeer(peerId,
          protocols: protocols, metadata: metadata);
      final retrievedPeerInfo = await peerstore.getPeer(peerId);
    ...
    test('addOrUpdatePeer updates existing peer data', () async {
      // Initial add
      peerstore.addOrUpdatePeer(peerId,
          addrs: [multiaddrs[0]], protocols: ['/initial/1.0']);
      ...
      if (retrievedPeerInfo != null) {
        ...
        expect(retrievedPeerInfo.addrs.contains(multiaddrs[1]), isTrue,
            reason: "Should contain the subsequently added address");

        expect(retrievedPeerInfo.protocols, equals(updatedProtocols.toSet()),
            reason: "Protocols should be overwritten by setProtocols");
        expect(retrievedPeerInfo.metadata, equals(updatedMetadata),
            reason: "Metadata should be updated");
      }
    });

    test('direct addrBook.addrs() call should not hang after addAddrs',
        () async {
      ...
      expect(existingAddrs.length, equals(2),
          reason: 'Should find both addresses that were added');
      expect(existingAddrs, containsAll(targetAddrs),
          reason: 'Should contain the exact addresses that were added');
    }, timeout: Timeout(Duration(seconds: 10)));

    test(
        'peerstore with own key initialization should not hang on addrs lookup',
        () async {
      ...
      for (final addrs in results) {
        expect(addrs.length, equals(2),
            reason: 'Each lookup should find both addresses');
        expect(addrs, containsAll(targetAddrs),
