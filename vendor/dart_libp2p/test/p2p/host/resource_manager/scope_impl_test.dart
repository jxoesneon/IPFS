...
class MockResourceScope extends ResourceScopeImpl {
  ...
  MockResourceScope(Limit limit, String name,
      {ResourceScopeImpl? owner, List<ResourceScopeImpl>? edges})
...
void main() {
  group('ResourceScopeImpl Initialization and Basic Lifecycle', () {
    ...
    test(
        'done() sets scope to done, clears local resources, and becomes unused',
        () {
      final limit = createTestLimit();
      final scope = ResourceScopeImpl(limit, 'testScope');
    ...
    test('done() is idempotent', () {
      ...
      expect(
          scope.stat.numStreamsOutbound, statAfterFirstDone.numStreamsOutbound);
    ...
    test('initial edges correctly increment parent ref counts', () {
      ...
      final childScope = ResourceScopeImpl(childLimit, 'childScope',
          edges: [mockParent1, mockParent2]);
  ...
  group('ResourceScopeImpl Reference Counting', () {
    test(
        'incRef() and decRef() modify refCnt correctly (indirectly via isUnused)',
        () {
      final limit = createTestLimit();
      final scope = ResourceScopeImpl(limit, 'testScope');

      expect(scope.isUnused(), true,
          reason: 'Scope should initially be unused');

      scope.incRef();
      expect(scope.isUnused(), false,
          reason: 'Scope should not be unused after incRef');

      scope.decRef();
      expect(scope.isUnused(), true,
          reason: 'Scope should be unused after matching decRef');
    });
    ...
    test('decRef() does not allow refCnt to go below zero (stays unused)', () {
      ...
      expect(scope.isUnused(), true,
          reason: 'Scope should remain unused even after extra decRef');
    ...
    test(
        'isUnused() returns false if resources are in use, even if refCnt is zero',
        () {
      final limit = createTestLimit();
      final scope = ResourceScopeImpl(limit, 'testScope');
    ...
    test('isUnused() returns true if done, regardless of refCnt or resources',
      ...
      expect(() => scope.addStream(Direction.outbound),
          returnsNormally); // Has resources
      expect(scope.isUnused(), false);

      scope.done();
      expect(scope.isUnused(), true,
          reason:
              'Scope should be unused after done(), even if refCnt might have been >0 or resources existed before done');
    });

    test(
        'incRef() after done() does not change isUnused state or allow new resources',
        () {
      final limit = createTestLimit();
      final scope = ResourceScopeImpl(limit, 'testScope');
      ...
      scope.done();
      expect(scope.isUnused(), true,
          reason: 'Scope is done, so it should be unused');

      scope.incRef(); // Attempt to increment ref count
      expect(scope.isUnused(), true,
          reason: 'incRef() after done() should not make scope "used"');

      // Also verify that it doesn't allow new resource allocations
      expect(() => scope.addStream(Direction.outbound),
          throwsA(isA<network_errors.ResourceScopeClosedException>()),
          reason:
              'Should not be able to add resources to a done scope, even after incRef');
    });
    ...
    test('decRef() after done() does not change isUnused state', () {
      ...
      expect(scope.isUnused(), true,
          reason: 'decRef() after done() should not change isUnused state');
    });

    test(
        'isUnused() is false when resources are present and refCnt is zero, true when resources are released',
        () {
      ...
      expect(scope.isUnused(), true,
          reason: 'Initially unused (refCnt=0, no resources)');

      // Add a resource. refCnt is still 0.
      expect(() => scope.addStream(Direction.outbound), returnsNormally);
      expect(scope.stat.numStreamsOutbound, 1);
      expect(scope.isUnused(), false,
          reason: 'Not unused (refCnt=0, but has resources)');

      // Remove the resource. refCnt is still 0.
      scope.removeStream(Direction.outbound);
      expect(scope.stat.numStreamsOutbound, 0);
      expect(scope.isUnused(), true,
          reason: 'Unused again (refCnt=0, no resources)');
    });
  });
  ...
  group('ResourceScopeImpl DAG Hierarchy (Parent-Child)', () {
    ...
    setUp(() {
      ...
      mockParent
          .resetCounters(); // Reset after child's constructor calls incRef
    });
    ...
    test('child.addStream() propagates to parent via _addStreamForChild', () {
      ...
      expect(mockParent.stat.numStreamsOutbound, 1,
          reason: 'Parent should also have 1 stream');
    });
    ...
    test('child.addStream() fails if parent limit is exceeded', () {
      ...
      expect(childScope.stat.numStreamsOutbound, 0,
          reason: 'Child stream should have been rolled back');
      expect(mockParent.stat.numStreamsOutbound, 1,
          reason: 'Parent stream count should remain 1');
    });
    ...
    test('child.removeStream() propagates to parent via _removeStreamForChild',
      ...
      expect(childScope.stat.numStreamsOutbound, 0);
      expect(mockParent.stat.numStreamsOutbound, 0,
          reason: 'Parent stream count should be 0 after child removes stream');
    });

    test('child.done() calls _releaseResourcesForChild and decRef on parent',
        () {
      expect(() => childScope.addStream(Direction.outbound),
          returnsNormally); // Child has 1 outbound stream
      expect(mockParent.stat.numStreamsOutbound, 1);
      ...
      expect(mockParent.stat.numStreamsOutbound, 0,
          reason:
              'Parent should have its stream count (from child) released. Expected 0, got ${mockParent.stat.numStreamsOutbound}');
    ...
    test(
        'parent._releaseResourcesForChild correctly updates its resources and propagates to its own ancestors',
        () {
      // Setup: Grandparent -> Parent (mock) -> Child
      final grandParentLimit = createTestLimit(streamsOutbound: 5);
      final mockGrandParent =
          MockResourceScope(grandParentLimit, 'mockGrandParent');

      final parentLimitForThisTest = createTestLimit(streamsOutbound: 3);
      final actualParent = ResourceScopeImpl(
          parentLimitForThisTest, 'actualParent',
          edges: [mockGrandParent]);

      final childLimitForThisTest = createTestLimit(streamsOutbound: 1);
      final actualChild = ResourceScopeImpl(
          childLimitForThisTest, 'actualChild',
          edges: [actualParent]);
      ...
      expect(actualParent.stat.numStreamsOutbound, 0,
          reason: "Actual parent's stream count should be 0");
      expect(mockGrandParent.stat.numStreamsOutbound, 0,
          reason:
              "Grandparent's stream count should be 0 due to propagation from actualParent");
    });
  });

  group('ResourceScopeImpl Span Hierarchy (Owner-Span)', () {
    late MockResourceScope mockOwner;
    late ResourceScopeImpl
        spanScope; // This will be ResourceScopeSpan, but ResourceScopeImpl implements it

    final ownerLimit = createTestLimit(streamsOutbound: 1, streams: 1);

    setUp(() async {
      // Needs to be async for beginSpan
      mockOwner = MockResourceScope(ownerLimit, 'mockOwner');
      // Create a span from the owner
      spanScope = await mockOwner.beginSpan()
          as ResourceScopeImpl; // Cast to access underlying impl details if needed
      mockOwner.resetCounters(); // Reset after beginSpan calls incRef
    });

    test('beginSpan() increments owner refCnt and creates valid span',
        () async {
      // Made async
      ...
      expect(freshOwner.isUnused(), true,
          reason: "Fresh owner should initially be unused.");
      ...
      expect(freshOwner.isUnused(), false,
          reason: "Owner should not be unused after a span is created");
    });
    ...
    test('span.addStream() propagates to owner', () async {
      ...
      expect(() => spanScope.addStream(Direction.outbound),
          returnsNormally); // spanScope is ResourceScopeImpl

      expect(spanScope.stat.numStreamsOutbound, 1);
      expect(mockOwner.stat.numStreamsOutbound, 1,
          reason: 'Owner should also have 1 stream from span');
    });
    ...
    test('span.addStream() fails if owner limit is exceeded', () async {
      ...
      expect(spanScope.stat.numStreamsOutbound, 0,
          reason: 'Span stream should have been rolled back');
      expect(mockOwner.stat.numStreamsOutbound, 1,
          reason: 'Owner stream count should remain 1');
    });
    ...
    test('span.removeStream() propagates to owner', () async {
      ...
      expect(spanScope.stat.numStreamsOutbound, 0);
      expect(mockOwner.stat.numStreamsOutbound, 0,
          reason: 'Owner stream count should be 0 after span removes stream');
    });

    test('span.done() calls _releaseResourcesForChild and decRef on owner',
        () async {
      expect(() => spanScope.addStream(Direction.outbound),
          returnsNormally); // Span has 1 outbound stream
      expect(mockOwner.stat.numStreamsOutbound, 1);
  ...
  group('ResourceScopeImpl Resource Limits and Errors', () {
    test(
        'addStream() throws ResourceLimitExceededException when local stream limit is reached',
        () {
      ...
      expect(() => scope.addStream(Direction.outbound),
          throwsA(isA<network_errors.ResourceLimitExceededException>()));
      expect(scope.stat.numStreamsOutbound, 1,
          reason: 'Stream count should remain 1 after failed attempt');
    });

    test(
        'addStream() throws ResourceLimitExceededException when local total stream limit is reached',
        () {
      final limit = createTestLimit(
          streamsInbound: 1, streamsOutbound: 1, streams: 1); // Total 1 stream
      final scope = ResourceScopeImpl(limit, 'testScope');
      ...
      expect(scope.stat.numStreamsOutbound, 1);
      expect(scope.stat.numStreamsInbound, 0,
          reason: 'Inbound stream count should remain 0 after failed attempt');
    });
    ...
    test('addStream() rolls back local reservation if an ancestor fails', () {
      ...
      final parentLimit = createTestLimit(
          streamsOutbound: 0, streams: 0); // Parent cannot take any streams
      final mockParent = MockResourceScope(parentLimit, 'mockParent');

      final childLimit = createTestLimit(streamsOutbound: 1, streams: 1);
      final childScope =
          ResourceScopeImpl(childLimit, 'childScope', edges: [mockParent]);
      ...
      expect(() => childScope.addStream(Direction.outbound),
          throwsA(isA<network_errors.ResourceLimitExceededException>()),
          reason: 'Error should come from parent');

      expect(childScope.stat.numStreamsOutbound, 0,
          reason: 'Childs local reservation should be rolled back');
      expect(mockParent.stat.numStreamsOutbound, 0,
          reason: 'Parent should not have any streams');
    });
    ...
    test(
        'reserveMemory() throws ResourceLimitExceededException if limit is surpassed',
        () async {
      final limit = createTestLimit(memory: 100);
      final scope = ResourceScopeImpl(limit, 'testScope');
      ...
      expect(() async => await scope.reserveMemory(30, 0),
          throwsA(isA<network_errors.ResourceLimitExceededException>()));
      expect(scope.stat.memory, 80,
          reason: 'Memory should remain 80 after failed attempt');
    });
    ...
    test('reserveMemory() rolls back local reservation if an ancestor fails',
        () async {
      ...
      if (childScope.stat.memory != 0) {
        ...
        fail(
            'Childs local memory reservation was not rolled back. Expected 0, but got ${childScope.stat.memory}.');
      }

      // If we reach here, childScope.stat.memory was 0. Let's check mockParent as well.
      expect(mockParent.stat.memory, 0,
          reason: 'Parent should not have any memory reserved');
    });
  });
}
