...
class TestLimiter implements Limiter {
  ...
  TestLimiter({
    ...
    BaseLimit? serviceLimits,
            ...
            BaseLimit(
                ...
                streamsOutbound: 1,
                memory: 1024),
        this.transientLimits = transientLimits ??
            BaseLimit(
                conns: 1,
                connsInbound: 1,
                connsOutbound: 1,
  ...
  Limit getStreamLimits(PeerId peer) =>
      streamLimits; // Simplified: same stream limit for all peers
  @override
  ...
  Limit getAllowlistedSystemLimits() =>
      BaseLimit.unlimited(); // Not testing allowlisting yet
  @override
...
void main() {
  group('ResourceManagerImpl', () {
    ...
    test('openConnection and release basic functionality', () async {
      ...
      final connScope = await resourceManager.openConnection(
          Direction.outbound, false, testMultiAddr);
    ...
    test('openStream, setProtocol, and release scopes', () async {
      ...
      final streamScope =
          await resourceManager.openStream(peerA, Direction.outbound);
    ...
    group('with restrictive limits', () {
      ...
      setUp(() async {
        final testLimiter = TestLimiter(
            connLimits: BaseLimit(
                conns: 1,
                connsInbound: 1,
                connsOutbound: 1,
                memory: 1024), // For ConnectionScopeImpl's own limit
            streamLimits: BaseLimit(
                streams: 1,
                streamsInbound: 1,
                streamsOutbound: 1,
                memory: 1024), // For StreamScopeImpl's own limit
            peerLimits: BaseLimit(
                conns: 1,
                connsInbound: 1,
                connsOutbound: 1,
                streams: 1,
            ...
            transientLimits: BaseLimit(
                ...
                streamsOutbound: 3,
                memory: 3072),
            protocolLimits: BaseLimit(
                streams: 1,
                streamsInbound: 1,
                streamsOutbound: 1,
                memory: 1024) // For ProtocolScopeImpl's own limit
            );
      ...
      test('exceeding connection limit throws ResourceLimitExceededException',
          () async {
        // Open one connection, should succeed
        final connScope1 = await limitedResourceManager.openConnection(
            Direction.outbound, false, testMultiAddr);
        expect(connScope1, isA<ConnManagementScope>());

        // Try to open a second connection, should fail
        expect(
          () async => await limitedResourceManager.openConnection(
              Direction.outbound, false, testMultiAddr),
      ...
      test(
          'exceeding stream limit for a peer throws ResourceLimitExceededException',
          () async {
        // Need a connection to the peer first
        final connScope = await limitedResourceManager.openConnection(
            Direction.outbound, false, testMultiAddr);
        await connScope.setPeer(peerA);

        // Open one stream to peerA, should succeed
        final streamScope1 =
            await limitedResourceManager.openStream(peerA, Direction.outbound);
        ...
        expect(
          () async => await limitedResourceManager.openStream(
              peerA, Direction.outbound),
          throwsA(isA<network_errors.ResourceLimitExceededException>()),
        );
      ...
      test('streams to different peers respect individual peer limits',
          () async {
        // Connection to Peer A
        final connScopeA = await limitedResourceManager.openConnection(
            Direction.outbound, false, testMultiAddr);
        ...
        final multiConnLimiter = TestLimiter(
            connLimits: BaseLimit(
                conns: 2, connsInbound: 2, connsOutbound: 2, memory: 2048),
            streamLimits: BaseLimit(
                streams: 1,
                streamsInbound: 1,
                streamsOutbound: 1,
        ...
        final newConnScopeA = await limitedResourceManager.openConnection(
            Direction.outbound, false, testMultiAddr);
        await newConnScopeA.setPeer(peerA);
        final newStreamScopeA = await limitedResourceManager.openStream(
            peerA, Direction.outbound); // 1st for peerA
        ...
        final connScopeB = await limitedResourceManager.openConnection(
            Direction.outbound, false, multiAddrB);
        await connScopeB.setPeer(peerB);
        // Stream to Peer B (should succeed, 1st stream for peerB)
        final streamScopeB =
            await limitedResourceManager.openStream(peerB, Direction.outbound);
      ...
      test(
          'protocol scope lifecycle with multiple streams to same peer and protocol',
          () async {
        ...
        expect(() async {
          streamScope2 = await limitedResourceManager.openStream(
              peerA, Direction.outbound);
          await streamScope2.setProtocol(
              testProto); // This might re-acquire/re-incRef the protocol scope
          ...
          streamScope2.done();
            ...
            reason:
                'Opening and closing a second stream for the same peer/protocol should not cause refCnt errors.');
