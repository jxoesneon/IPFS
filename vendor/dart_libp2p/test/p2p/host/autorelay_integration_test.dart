  ...
  group('AutoRelay Integration', () {
    ...
    setUp(() async {
      ...
      relayNode = await createLibp2pNode(
        ...
        forceReachability:
            Reachability.public, // Force public to start relay service
      );
      ...
      await Future.delayed(
          Duration(milliseconds: 500)); // Give it time to start
      ...
      final autoRelaySubASub =
          peerAHost.eventBus.subscribe(EvtAutoRelayAddrsUpdated);
      autoRelaySubA = autoRelaySubASub.stream.listen((event) {
        if (event is EvtAutoRelayAddrsUpdated) {
          ...
          for (var addr in event.advertisableAddrs) {
            print('   - $addr');
            print(
                '     Is circuit: ${addr.toString().contains('/p2p-circuit')}');
          }
        }
      });

      final autoRelaySubBSub =
          peerBHost.eventBus.subscribe(EvtAutoRelayAddrsUpdated);
    ...
    test('Peers advertise circuit relay addresses and can ping through relay',
      ...
      final hasCircuitV2 = relayProtocols
          .any((p) => p.contains('circuit') || p.contains('libp2p/circuit'));
      ...
      expect(peerACircuitAddrs, isNotEmpty,
          reason:
              'Peer A should advertise at least one circuit relay address through relay ${relayPeerId.toBase58()}');
      ...
      final relayKnownPeerAAddrs =
          await relayHost.peerStore.addrBook.addrs(peerAPeerId);
      print(
          'Relay server knows ${relayKnownPeerAAddrs.length} addresses for Peer A:');
      for (var addr in relayKnownPeerAAddrs) {
        print('   - $addr');
      }

      // Check relay's peerstore for peer B's addresses
      final relayKnownPeerBAddrs =
          await relayHost.peerStore.addrBook.addrs(peerBPeerId);
      print(
          'Relay server knows ${relayKnownPeerBAddrs.length} addresses for Peer B:');
      ...
      print(
          'Relay knows ${relayKnownPeerACircuitAddrs.length} circuit addresses for Peer A');
      print(
          'Relay knows ${relayKnownPeerBCircuitAddrs.length} circuit addresses for Peer B');

      // Verify relay server received circuit addresses via Identify Push
      expect(relayKnownPeerACircuitAddrs, isNotEmpty,
          reason:
              'Relay server should have received Peer A\'s circuit addresses via Identify Push');
      ...
      expect(
          storedCircuitAddrs.length, equals(peerBDialableCircuitAddrs.length),
          reason:
              'Peerstore should contain exactly ${peerBDialableCircuitAddrs.length} circuit addresses');
      ...
      try {
        ...
        print(
            'Ping result: error=${pingResult.hasError}, rtt=${pingResult.rtt?.inMilliseconds}ms');
    ...
    test('Circuit relay connections are reused by Swarm for multiple dials',
        () async {
      ...
      expect(peerBCircuitAddrs, isNotEmpty,
          reason:
              'Peer B should advertise circuit relay addresses through relay ${relayPeerId.toBase58()}');
      ...
      final cleanAddr = peerBCircuitAddr.toString().endsWith('/')
          ? peerBCircuitAddr
              .toString()
              .substring(0, peerBCircuitAddr.toString().length - 1)
      ...
      expect(conns1, isNotEmpty,
          reason: 'Should have connection after first dial');
      ...
      final cleanAddrA = peerACircuitAddr.toString().endsWith('/')
          ? peerACircuitAddr
              .toString()
              .substring(0, peerACircuitAddr.toString().length - 1)
          : peerACircuitAddr.toString();
      final dialableCircuitAddrToA =
          '$cleanAddrA/p2p/${peerAPeerId.toBase58()}';
      ...
      expect(connsFromAAfter.length, equals(1),
          reason:
              'Peer A should STILL have only 1 connection to B after B dials A back (bidirectional reuse)');

      expect(connsFromBAfter.length, equals(1),
          reason:
              'Peer B should have exactly 1 connection to A (reused existing connection, not created new one)');
