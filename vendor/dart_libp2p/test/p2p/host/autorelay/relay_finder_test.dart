...
void main() {
  group('RelayFinder', () {
    ...
    setUp(() {
      ...
      final relayPeerId = PeerId.fromString(
          '12D3KooWRcr1tPJ5D46uESVgA1sJrNmcGrW2XHGcHztHadBvcv3G');
    ...
    group('Circuit Address Construction', () {
      test('should construct valid circuit addresses from reservations',
          () async {
        // Arrange
        ...
        expect(circuitAddrs, isNotEmpty,
            reason:
                'Should return at least private addresses or circuit addresses');
        ...
        expect(circuitAddrsWithRelayId, isNotEmpty,
            reason:
                'Should have at least one circuit address with relay peer ID');
        ...
        final firstCircuitAddr =
            circuitAddrsWithRelayId[0].toString().trimRight();
        ...
        expect(
            firstCircuitAddr.endsWith('/p2p-circuit') ||
                firstCircuitAddr.endsWith('/p2p-circuit/'),
            isTrue,
            reason:
                'Address should end with /p2p-circuit (with or without trailing slash)');
      });
      ...
      test('should construct valid circuit address format', () {
        ...
        expect(addrString,
            matches(RegExp(r'/ip4/.*/tcp/\d+/p2p/.*?/p2p-circuit')));
      });
      ...
      test('should handle multiple relay addresses', () async {
        ...
        final relayPeerId1 = PeerId.fromString(
            '12D3KooWRcr1tPJ5D46uESVgA1sJrNmcGrW2XHGcHztHadBvcv3G');
        final relayPeerId2 = PeerId.fromString(
    ...
    group('Relay Candidate Selection', () {
      test('should filter out relay addresses from candidates', () {
        ...
        final relayCircuitAddr =
            MultiAddr('/ip4/10.10.1.1/tcp/4001/p2p/QmRelay/p2p-circuit');
