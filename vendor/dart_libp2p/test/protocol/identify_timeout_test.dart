...
void main() {
  group('Identify Timeout Reproduction', () {
    ...
    setUpAll(() async {
      ...
      serverHost.setStreamHandler(id,
          (core_network_stream.P2PStream stream, PeerId peerId) async {
        print(
            'Server received identify request from $peerId, NOT responding (simulating timeout)...');
      ...
      print(
          'Setup Complete. Client: $clientPeerId, Server: $serverPeerId listening on $serverListenAddr');
    });
    ...
    test('identify timeout throws typed IdentifyTimeoutException', () async {
      ...
      try {
        ...
        print(
            'Client calling connect() - this will trigger identify protocol...');
        await clientHost.connect(serverAddrInfo);

        print(
            'Connection succeeded (unexpected - identify should have timed out)');
        ...
        print(
            '- Exception can be caught specifically using on IdentifyTimeoutException');
      ...
      print('=== Test Complete ===\n');
        ...
        timeout:
            Timeout(Duration(seconds: 40))); // Allow time for timeout to occur
  });
}
