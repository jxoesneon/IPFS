  ...
  group('Relay Server', () {
    ...
    group('Bidirectional Data Forwarding', () {
      ...
      test('should handle stream errors gracefully', () async {
        ...
        expect(
            () => relay.relayDataForTesting(
                srcStream, dstStream, srcPeer, dstPeer),
            returnsNormally);
    ...
    group('Connection Reuse Integration', () {
      test(
          'relay connection count stays at 1 when same peer pair communicates multiple times',
          () async {
        // Arrange
        final srcKeyPair = await generateEd25519KeyPair();
        final dstKeyPair = await generateEd25519KeyPair();
        ...
        final count1 =
            relay.getConnectionCount(srcPeer.toString(), dstPeer.toString());
        expect(count1, equals(1),
            reason: 'First relay should register 1 connection');
        ...
        expect(count2, equals(2),
            reason:
                'Second relay increments connection count to 2 (this is the bug we fixed in the client)');

        // Wait for cleanup
        await Future.delayed(Duration(milliseconds: 500));
      });

      test('connection count decrements when relay connections close',
          () async {
        // Arrange
        final srcKeyPair = await generateEd25519KeyPair();
        final dstKeyPair = await generateEd25519KeyPair();
        ...
        expect(relay.getConnectionCount(srcPeer.toString(), dstPeer.toString()),
            greaterThan(0));
      ...
      test(
          'multiple concurrent connections between different peer pairs are tracked separately',
          () async {
        // Arrange
        final srcPeer1 =
        ...
        expect(
            relay.getConnectionCount(srcPeer1.toString(), dstPeer1.toString()),
            equals(1));
        expect(
            relay.getConnectionCount(srcPeer2.toString(), dstPeer2.toString()),
            equals(1));

        // Different peer pairs should not interfere
        expect(
            relay.getConnectionCount(srcPeer1.toString(), dstPeer2.toString()),
            equals(0));
        expect(
            relay.getConnectionCount(srcPeer2.toString(), dstPeer1.toString()),
            equals(0));
