  ...
  group('HTTP-like Protocol Tests', () {
    ...
    test('basic HTTP GET request/response', () async {
      ...
      try {
        ...
        await clientHost.peerStore.addrBook.addAddrs(
            serverHost.id, [actualListenAddr], AddressTTL.permanentAddrTTL);
        clientHost.peerStore.keyBook.addPubKey(serverHost.id,
        ...
        final response2 =
            await httpClient.getRequest(serverHost.id, '/users/123');
    ...
    test('HTTP POST with JSON data', () async {
      ...
      try {
        ...
        final response1 =
            await httpClient.postJson(serverHost.id, '/users', userData);
        ...
        final response2 =
            await httpClient.postJson(serverHost.id, '/process', processData);
    ...
    test('HTTP protocol with connection reuse', () async {
      ...
      try {
        ...
        final response1 =
            await httpClient.getRequest(serverHost.id, '/api/status');
        expect(response1.status, equals(HttpStatus.ok));
        ...
        // Request 2: GET data
        final response2 =
            await httpClient.getRequest(serverHost.id, '/api/data/123');
        expect(response2.status, equals(HttpStatus.ok));
        ...
        final response3 =
            await httpClient.postJson(serverHost.id, '/api/submit', submitData);
    ...
    test('HTTP error handling and status codes', () async {
      ...
      try {
        ...
        httpServer.get('/error/400', (request) async {
          return HttpResponse.error(
              HttpStatus.badRequest, 'Bad request example');
        });
        ...
        httpServer.post('/validate', (request) async {
          ...
          if (!data.containsKey('name')) {
            return HttpResponse.error(
                HttpStatus.badRequest, 'Missing required field: name');
          }

          if (!data.containsKey('email')) {
            return HttpResponse.error(
                HttpStatus.badRequest, 'Missing required field: email');
          }
        ...
        // Missing name field
        final response6 = await httpClient.postJson(
            serverHost.id, '/validate', {'email': 'test@example.com'});
        ...
        final response7 = await httpClient
            .postJson(serverHost.id, '/validate', {'name': 'John'});
        ...
        final response8 =
            await httpClient.postJson(serverHost.id, '/validate', validData);
