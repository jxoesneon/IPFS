...
void main() {
  group('MockConnection', () {
    ...
    test('handles rapid writes and reads', () async {
      final testData = List.generate(100,
          (i) => Uint8List.fromList([1, 2, 3]) // Fixed values for each array
          );

      // Write all data rapidly
      await Future.wait(testData.map((data) => conn1.write(data)));

      // Read all data rapidly
      final results =
          await Future.wait(List.generate(100, (i) => conn2.read()));
    ...
    test('proves read chunking issue', () async {
      ...
      expect(messageBody.sublist(32), List.filled(16, 0x4D),
    ...
    test('correctly handles large messages with length prefix', () async {
      ...
      final messageBody = Uint8List(164)
        ..fillRange(0, 164, 42); // Fill with 42s
    ...
    test('correctly handles message boundaries with length prefixes', () async {
      final (conn1, conn2) =
          MockConnection.createPair(id1: 'sender', id2: 'receiver');
      ...
      try {
        ...
        expect(length1, equals(32),
            reason: 'First message length prefix should be 32');

        // Read first message body
        final body1 = await conn2.read(length1);
        expect(body1.length, equals(32),
            reason: 'First message body should be 32 bytes');
        expect(body1.every((b) => b == 0x42), isTrue,
            reason: 'First message body should be all 0x42');

        // Read second length prefix (2 bytes)
        final length2Bytes = await conn2.read(2);
        final length2 = (length2Bytes[0] << 8) | length2Bytes[1];
        expect(length2, equals(80),
            reason: 'Second message length prefix should be 80');

        // Read second message body
        final body2 = await conn2.read(length2);
        expect(body2.length, equals(80),
            reason: 'Second message body should be 80 bytes');
        expect(body2.every((b) => b == 0x43), isTrue,
            reason: 'Second message body should be all 0x43');
      } finally {
        await conn1.close();
        await conn2.close();
      }
    ...
    test('handles noise protocol message sequence', () async {
      ...
      expect(body3.length, equals(48),
          reason: 'Third message body should be 48 bytes');
      expect(body3.every((b) => b == 0x44), isTrue,
          reason: 'Third message body should be all 0x44');
