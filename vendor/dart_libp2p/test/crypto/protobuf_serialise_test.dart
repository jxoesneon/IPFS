...
void main() {
  group('Protobuf Key Deserialization Tests', () {
    test('ECDSA key deserialization', () {
      final privateKeyHex =
      ...
      final privateKey = PrivateKey.fromBuffer(privateKeyBytes);
      final publicKey = PublicKey.fromBuffer(publicKeyBytes);

      expect(privateKey.type, KeyType.ECDSA);
      expect(publicKey.type, KeyType.ECDSA);
      expect(hex.encode(privateKey.data).toUpperCase(),
          privateKeyHex.substring(8).toUpperCase());
      expect(hex.encode(publicKey.data).toUpperCase(),
          publicKeyHex.substring(8).toUpperCase());
    });

    test('ED25519 key deserialization', () {
      final privateKeyHex =
      ...
      expect(privateKey.type, KeyType.Ed25519);
      expect(publicKey.type, KeyType.Ed25519);
      expect(hex.encode(privateKey.data),
          privateKeyHex.substring(8).toLowerCase());
      expect(
          hex.encode(publicKey.data), publicKeyHex.substring(8).toLowerCase());
    });

    test('Secp256k1 key deserialization', () {
      final privateKeyHex =
      ...
      expect(privateKey.type, KeyType.Secp256k1);
      expect(publicKey.type, KeyType.Secp256k1);
      expect(hex.encode(privateKey.data),
          privateKeyHex.substring(8).toLowerCase());
      expect(
          hex.encode(publicKey.data), publicKeyHex.substring(8).toLowerCase());
    ...
    test('RSA key deserialization', () {
      ...
      expect(privateKey.type, KeyType.RSA);
      expect(publicKey.type, KeyType.RSA);
      expect(hex.encode(privateKey.data).toLowerCase(),
          privateKeyHex.substring(10).toLowerCase());
      expect(hex.encode(publicKey.data).toLowerCase(),
          publicKeyHex.substring(10).toLowerCase());
    });
  });
}
