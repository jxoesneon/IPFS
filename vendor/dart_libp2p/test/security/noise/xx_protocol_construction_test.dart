...
class XXProtocolConstruction {
  ...
  Uint8List constructFinalMessage() {
    ...
    final encryptedPayload =
        Uint8List.fromList(List.generate(payload.length, (i) => i + 200));
...
void main() {
  group('XXProtocolConstruction', () {
    ...
    test('second message has correct structure', () {
      ...
      expect(encryptedStatic.length, equals(32),
          reason: 'Encrypted static key should be 32 bytes');
    ...
    test('final message has correct structure', () {
      ...
      expect(encryptedStatic.length, equals(32),
          reason: 'Encrypted static key should be 32 bytes');
      expect(staticMac.length, equals(16),
          reason: 'Static MAC should be 16 bytes');
      expect(encryptedPayload.length, equals(96),
          reason: 'Encrypted payload should be 96 bytes');
      expect(payloadMac.length, equals(16),
          reason: 'Payload MAC should be 16 bytes');
    });
  });
}
