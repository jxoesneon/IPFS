  ...
  group('MessageFraming', () {
    test('adds length prefix correctly', () {
      final testCases = [
        ...
        Uint8List.fromList(
            List.generate(256, (i) => i % 256)), // Message with length > 255
        Uint8List(0), // Empty message
        Uint8List.fromList(
            List.generate(65535, (i) => i % 256)), // Maximum size message
      ];
      ...
      for (final message in testCases) {
        ...
        // Verify length
        expect(framedMessage.length,
            equals(message.length + MessageFraming.PREFIX_LENGTH),
            reason:
                'Framed message should be original length plus prefix length');
        ...
        expect(framedMessage.sublist(MessageFraming.PREFIX_LENGTH),
            equals(message),
    ...
    test('validates message length correctly', () {
      ...
      expect(
          ...
          throwsA(isA<StateError>().having(
              (e) => e.message, 'message', 'Failed to read complete message')),
    ...
    test('handles error cases', () {
      ...
      expect(
          ...
          throwsA(isA<StateError>()
              .having((e) => e.message, 'message', 'Invalid length prefix')),
      ...
      expect(() => MessageFraming.validateMessageLength(Uint8List(0), 1),
          throwsA(isA<StateError>()),
          reason: 'Should throw when message is shorter than expected length');
    });
    ...
    test('handles maximum message size', () {
      ...
      final tooLargePrefix =
          framedTooLarge.sublist(0, MessageFraming.PREFIX_LENGTH);
