...
void main() {
  group('SecuredConnection', () {
    test('verifies length prefix and message splitting', () async {
      ...
      final secured1 =
          SecuredConnection(conn1, key, key, securityProtocolId: '');
      final secured2 =
          SecuredConnection(conn2, key, key, securityProtocolId: '');
    ...
    test('verifies nonce handling between connections', () async {
      ...
      final secured1 = SecuredConnection(conn1, sendKey, recvKey,
          securityProtocolId: ''); // sendKey for encryption
      final secured2 = SecuredConnection(conn2, recvKey, sendKey,
          securityProtocolId: ''); // recvKey for decryption
    ...
    test('verifies message framing end-to-end', () async {
      ...
      expect(received, equals(testData),
          reason:
              'Message should be correctly framed, transmitted and decrypted');
    ...
    test('guards against double-framing', () async {
      ...
      final testData =
          Uint8List.fromList([0, 32, ...List.generate(30, (i) => i)]);
      ...
      final rawBytes = conn1.writes.first;
      final actualLength = (rawBytes[0] << 24) |
          (rawBytes[1] << 16) |
          (rawBytes[2] << 8) |
          rawBytes[3];
      ...
      expect(received, equals(testData),
          reason:
              'Message should be correctly decrypted without double-framing');
