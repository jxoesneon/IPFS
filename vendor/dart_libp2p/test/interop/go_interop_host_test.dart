...
void main() {
  ...
  /// Creates a BasicHost with TCP transport via Config.newNode().
  ...
  group('BasicHost Go-libp2p Interop', () {
    ...
    test('Dart BasicHost connects to Go and completes identify', () async {
      ...
      // Go supports echo and identify at minimum
      expect(protocols, contains('/echo/1.0.0'));

      print('Identify exchange verified');
    }, timeout: Timeout(Duration(seconds: 30)));

    test('Dart BasicHost echoes via newStream to Go server', () async {
      await goProcess.startServer();
      final goAddr = goProcess.listenAddr;
      final goPeerId = goProcess.peerId;
      ...
      print('Echo verified via BasicHost.newStream');

      await stream.close();
    }, timeout: Timeout(Duration(seconds: 30)));

    test('Go echo-client connects to Dart BasicHost echo handler', () async {
      final keyPair = await crypto_ed25519.generateEd25519KeyPair();
      ...
      // Run Go echo-client targeting Dart host
      final targetAddr = '$dartAddr/p2p/${dartPeerId.toBase58()}';
      print('Go echo-client target: $targetAddr');

      final result =
          await goProcess.runEchoClient(targetAddr, 'hello from go-libp2p');
    ...
    test('Go pings Dart BasicHost', () async {
      ...
      // PingService is started by BasicHost.start() by default.
      // No explicit handler registration needed.

      final targetAddr = '$dartAddr/p2p/${dartPeerId.toBase58()}';
      print('Go ping target: $targetAddr');

      final result = await goProcess.runPing(targetAddr);
