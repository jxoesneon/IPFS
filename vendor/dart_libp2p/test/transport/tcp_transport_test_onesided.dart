  ...
  group('Yamux One-Sided Initialization Test', () {
    ...
    setUp(() async {
      ...
      simpleTcpServer.listen((socket) {
        print(
            'Simple TCP Server accepted a connection from ${socket.remoteAddress.address}:${socket.remotePort}');
    ...
    tearDown(() async {
      ...
      await clientYamuxSession
          ?.close(); // This should also close clientTcpConnection
    ...
    test('Client-side YamuxSession initializes against a non-Yamux TCP server',
        () async {
      ...
      try {
        ...
        expect(clientTcpConnection, isNotNull,
            reason: 'Client TCP connection should be established.');
        ...
        expect(clientTcpConnection!.isClosed, isTrue,
            reason:
                'Underlying TCP connection should be closed by YamuxSession.close()');
      } catch (e, s) {
        ...
        timeout: Timeout(
            Duration(seconds: 15))); // Shorter timeout for this specific test
  });
}
