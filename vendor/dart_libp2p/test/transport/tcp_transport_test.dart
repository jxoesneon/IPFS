...
void main() {
  group('TCPTransport Raw Connection Tests', () {
    ...
    late MultiAddr
        actualListenAddr; // To store the address with the OS-assigned port
    ...
    setUp(() async {
      ...
      actualListenAddr =
          listener.addr; // Get the actual address with the assigned port
    ...
    test(
        'Test the ability to send 20 bytes, which requires no looping on the read side (ideally)',
        () async {
      final dataToSend = Uint8List.fromList(List.generate(20, (i) => i % 256));

      expect(clientConn, isNotNull,
          reason: 'Client connection should be established.');
      expect(serverConn, isNotNull,
          reason: 'Server connection should be established.');
      ...
      while (totalBytesRead < dataToSend.length) {
        ...
        if (chunk.isEmpty) {
          ...
          if (totalBytesRead < dataToSend.length) {
            fail(
                'Stream closed prematurely. Expected ${dataToSend.length} bytes, got $totalBytesRead');
          }
          break;
        }
        // Ensure we don't write past the buffer
        final bytesToCopy =
            min(chunk.length, dataToSend.length - totalBytesRead);
        receivedBuffer.setRange(totalBytesRead, totalBytesRead + bytesToCopy,
            chunk.sublist(0, bytesToCopy));
        totalBytesRead += bytesToCopy;

        if (totalBytesRead > dataToSend.length) {
          // Should not happen if server sends exact amount
          fail(
              'Read more bytes than expected. Expected ${dataToSend.length}, got $totalBytesRead');
        }
      }

      expect(totalBytesRead, equals(dataToSend.length),
          reason: 'Should read the exact number of bytes sent.');
      expect(
          receivedBuffer.sublist(0, totalBytesRead), orderedEquals(dataToSend),
          reason: 'Received data should match sent data.');
    });

    test(
        'Test the ability to send 500 bytes of data which requires looping on the read side',
        () async {
      ...
      if (totalBytesRead > dataToSend.length) {
        // Check if more data was received than sent
        fail(
            'Read more bytes than expected. Expected ${dataToSend.length}, got $totalBytesRead. Actual data: ${receivedBytesBuilder.toBytes()}');
      }

      expect(totalBytesRead, equals(dataToSend.length),
          reason: 'Should read the exact number of bytes sent.');
      expect(receivedBytesBuilder.toBytes(), orderedEquals(dataToSend),
          reason: 'Received data should match sent data.');
    });
    ...
    group('Yamux over TCP Tests', () {
      ...
      setUp(() async {
        ...
        yamuxClientTransport = TCPTransport(
            resourceManager: yamuxResourceManager, connManager: connManager);
        yamuxServerTransport = TCPTransport(
            resourceManager: yamuxResourceManager, connManager: connManager);
        ...
        final clientDialFuture =
            yamuxClientTransport.dial(yamuxActualListenAddr);
        ...
        expect(clientTcpConnForYamux, isNotNull,
            reason: 'Client TCP for Yamux must be established.');
        expect(serverTcpConnForYamux, isNotNull,
            reason: 'Server TCP for Yamux must be established.');

        clientYamuxSession =
            YamuxSession(clientTcpConnForYamux!, multiplexerConfig, true);
        serverYamuxSession =
            YamuxSession(serverTcpConnForYamux!, multiplexerConfig, false);
      ...
      test('Test YamuxStream for SMALL data (20 bytes)', () async {
        expect(clientYamuxSession, isNotNull,
            reason: 'Client Yamux session must be established.');
        expect(serverYamuxSession, isNotNull,
            reason: 'Server Yamux session must be established.');
        ...
        try {
          ...
          expect(totalBytesRead, dataToSend.length,
              reason: 'Should read exact small data length over Yamux.');
          expect(receivedDataBuffer.toBytes(), orderedEquals(dataToSend),
              reason: 'Received small data over Yamux should match sent data.');
        } finally {
      ...
      test('Test YamuxStream for LARGE data (1000 bytes)', () async {
        ...
        try {
          ...
          expect(totalBytesRead, dataToSend.length,
              reason: 'Should read exact large data length over Yamux.');
          expect(receivedDataBuffer.toBytes(), orderedEquals(dataToSend),
              reason: 'Received large data over Yamux should match sent data.');
        } finally {
