...
void main() {
  ...
  group('UDXTransport with Noise and OBP Integration Test', () {
    ...
    test(
        'should establish UDX connection, upgrade to Noise/Yamux, and perform OBP handshake',
        () async {
      ...
      final serverHandlerFuture =
          _handleBasicServerOBPStream(serverStream, 'BasicTransportTest');
  ...
  group('UDXTransport with Noise/OBP, Real ConnMgr and RsrcMgr', () {
    ...
    test(
      ...
      // Start server-side OBP handler
      final serverHandlerFuture =
          _handleBasicServerOBPStream(serverStream, 'RealManagersTest');

      // Perform OBP handshake
      print(
          'Client performing OBP handshake on stream ${clientStream.id()} (Real Managers Test)');
      final handshakeSuccess = await OBPProtocolHandler.performHandshake(
        clientStream,
        isClient: true,
      ...
      print(
          'Client sending OBP ping frame on stream ${clientStream.id()} (Real Managers Test)');
      ...
      expect(pongResponse!.type, equals(OBPMessageType.pong));
      expect(
          utf8.decode(pongResponse.payload), equals('real-managers-test-data'));
  ...
  group('Swarm with UDX, Noise, and OBP', () {
    ...
    setUpAll(() async {
      ...
      serverHost.setStreamHandler(OBPConstants.protocolId,
      ...
      print(
          'Swarm UDX/Noise/OBP Setup Complete. Client: ${clientPeerId.toString()}, Server: ${serverPeerId.toString()} listening on $serverListenAddr');
    });
    ...
    test(
        'should establish connection via Swarm, negotiate Noise/Yamux, and perform OBP protocol',
        () async {
      print(
          'Client Host (${clientPeerId.toString()}) attempting to open new stream to Server Host (${serverPeerId.toString()}) for protocol ${OBPConstants.protocolId}');
      ...
      try {
        ...
        clientStream = await clientHost.newStream(
            serverPeerId, [OBPConstants.protocolId], core_context.Context());
      } catch (e, s) {
...
Future<void> _handleServerOBPStream(
    core_network_stream.P2PStream stream) async {
  print(
      'Server received stream for OBP: ${stream.id()} from ${stream.conn.remotePeer}');
  try {
    ...
    while (!stream.isClosed) {
      try {
        ...
        print(
            'Server received OBP frame: ${frame.type} on stream ${stream.id()}');
        ...
        switch (frame.type) {
          ...
          case OBPMessageType.error:
            print(
                'Server received error frame on stream ${stream.id()}: ${utf8.decode(frame.payload)}');
            break;

          default:
            print(
                'Server received unsupported frame type: ${frame.type} on stream ${stream.id()}');
            await OBPProtocolHandler.sendError(
              stream,
...
Future<void> _testOBPFeatures(
    core_network_stream.P2PStream initialStream) async {
  ...
  void checkStreamHealth(String phase) {
    ...
    if (currentStream.isClosed) {
      throw StateError(
          'Stream closed during $phase - connection died unexpectedly');
    }

    if (currentStream.conn.isClosed) {
      throw StateError(
          'Underlying connection closed during $phase - transport layer failure');
    }
  }

  // Helper function to ensure stream is healthy before proceeding
  Future<core_network_stream.P2PStream> ensureHealthyStream(
      core_network_stream.P2PStream stream, String testName) async {
    try {
  ...
  currentStream =
      await ensureHealthyStream(currentStream, 'Test 1 - Pre-check');
  ...
  currentStream =
      await ensureHealthyStream(currentStream, 'Test 2a - Pre-check');

  try {
    final mediumPayload =
        Uint8List.fromList(List.generate(1024 * 32, (i) => i % 256)); // 32KB
    ...
    expect(mediumPongResponse, isNotNull,
        reason: 'Should receive medium pong response');
  ...
  currentStream =
      await ensureHealthyStream(currentStream, 'Test 2b - Pre-check');

  try {
    final largePayload =
        Uint8List.fromList(List.generate(1024 * 100, (i) => i % 256)); // 100KB
    ...
    expect(largePongResponse, isNotNull,
  ...
  currentStream =
      await ensureHealthyStream(currentStream, 'Test 3 - Pre-check');
  ...
  currentStream =
      await ensureHealthyStream(currentStream, 'Test 4 - Pre-check');
  ...
  try {
    ...
    for (int i = 0; i < rapidResponses.length; i++) {
      expect(rapidResponses[i], isNotNull,
          reason: 'Should receive rapid response $i');
...
Future<void> _handleBasicServerOBPStream(
    YamuxStream stream, String context) async {
  ...
  try {
    ...
    if (!handshakeSuccess) {
      print(
          '[$context] Basic server OBP handshake failed on stream ${stream.id()}');
      await OBPProtocolHandler.sendError(
        stream,
        'Handshake failed',
        OBPErrorCodes.handshakeFailed,
    ...
    print(
        '[$context] Basic server OBP handshake successful on stream ${stream.id()}');
    ...
    while (!stream.isClosed) {
      try {
        ...
        if (frame == null) {
          print(
              '[$context] Basic server received EOF on stream ${stream.id()}');
          break;
        }

        print(
            '[$context] Basic server received OBP frame: ${frame.type} on stream ${stream.id()}');
        ...
        switch (frame.type) {
          case OBPMessageType.ping:
            ...
            print(
                '[$context] Basic server sent pong response on stream ${stream.id()}');
            break;

          case OBPMessageType.error:
            print(
                '[$context] Basic server received error frame on stream ${stream.id()}: ${utf8.decode(frame.payload)}');
            break;

          default:
            print(
                '[$context] Basic server received unsupported frame type: ${frame.type} on stream ${stream.id()}');
            await OBPProtocolHandler.sendError(
              stream,
              'Unsupported message type: ${frame.type}',
              OBPErrorCodes.invalidMessage,
        ...
        if (e is TimeoutException) {
          print(
              '[$context] Basic server timeout reading frame on stream ${stream.id()}');
          break;
        }
        print(
            '[$context] Basic server error handling frame on stream ${stream.id()}: $e');
        await OBPProtocolHandler.sendError(
          stream,
          'Frame processing error: $e',
          OBPErrorCodes.internalError,
    ...
    print(
        '[$context] Basic server OBP handler error on stream ${stream.id()}: $e');
