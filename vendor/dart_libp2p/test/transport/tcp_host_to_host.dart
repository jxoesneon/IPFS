...
import 'package:ipfs_libp2p/p2p/transport/tcp_transport.dart'; // Changed from UDX
...
void main() {
  ...
  */
  ...
  group('Host-to-Host TCP, Noise, Yamux, Ping (via BasicHost)', () {
    ...
    setUpAll(() async {
      // udxInstance = UDX(); // Removed UDX instance initialization
      ...
      clientNetwork = Swarm(
        ...
        transports: [clientTcpTransport], // Changed to TcpTransport
      ...
      // serverHost.start() will now also call serverNetwork.listen() internally if listenAddrs are configured.
