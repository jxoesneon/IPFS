...
void main() {
  ...
  group('YamuxStream Connection Re-use via UDX Transport', () {
    ...
    test('swarm streams reuse yamux connections correctly with UDX transport',
        () async {
      print(
      ...
      try {
        ...
        await swarmB.peerstore.addrBook.addAddrs(
            swarmA.localPeer, [actualListenAddr], AddressTTL.permanentAddrTTL);
        swarmB.peerstore.keyBook.addPubKey(swarmA.localPeer,
    ...
    test('connection health during mixed stream states with UDX', () async {
      ...
      print(
          '[${DateTime.now().difference(testStart).inMilliseconds}ms] Creating swarmA...');
      final swarmA = await createUDXTestSwarm(
        name: 'MixedSwarmA',
        udxInstance: udxInstance,
        resourceManager: resourceManager,
      ...
      print(
          '[${DateTime.now().difference(testStart).inMilliseconds}ms] SwarmA created');

      print(
          '[${DateTime.now().difference(testStart).inMilliseconds}ms] Creating swarmB...');
      final swarmB = await createUDXTestSwarm(
        name: 'MixedSwarmB',
        udxInstance: udxInstance,
        resourceManager: resourceManager,
      ...
      print(
          '[${DateTime.now().difference(testStart).inMilliseconds}ms] SwarmB created');

      try {
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Setting up listener...');
        final listenAddr = MultiAddr('/ip4/127.0.0.1/udp/0/udx');
        await swarmA.listen([listenAddr]);
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Listener ready');
        ...
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Creating 5 streams with mixed states...');
        final streams = <P2PStream>[];

        for (int i = 0; i < 5; i++) {
          print(
              '[${DateTime.now().difference(testStart).inMilliseconds}ms] Creating stream $i...');
          final stream = await swarmB.newStream(Context(), swarmA.localPeer);
          print(
              '[${DateTime.now().difference(testStart).inMilliseconds}ms] Stream $i created, writing data...');
          streams.add(stream);
          await stream.write(utf8.encode('mixed-state-stream-$i'));
          print(
              '[${DateTime.now().difference(testStart).inMilliseconds}ms] Stream $i write complete');
        }
        ...
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] All streams using connection: $connectionId');

        // Close some streams, keep others open
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Closing streams 0 and 2...');
        await streams[0].close(); // Closed
        await streams[2].close(); // Closed
        // streams[1], streams[3], streams[4] remain open

        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Closed streams 0 and 2, keeping 1, 3, 4 open');

        // Verify connection is still healthy
        expect(connections.first.isClosed, isFalse);
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Connection still healthy');

        // Create new streams while others are still open
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Creating newStream1...');
        final newStream1 = await swarmB.newStream(Context(), swarmA.localPeer);
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Creating newStream2...');
        final newStream2 = await swarmB.newStream(Context(), swarmA.localPeer);

        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] Writing to new streams...');
        await newStream1.write(utf8.encode('new-stream-1'));
        await newStream2.write(utf8.encode('new-stream-2'));
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] New stream writes complete');
        ...
        await swarmA.close();
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] FINALLY BLOCK: swarmA closed');
        await swarmB.close();
        print(
            '[${DateTime.now().difference(testStart).inMilliseconds}ms] FINALLY BLOCK: swarmB closed');
      }
    }, timeout: Timeout(Duration(seconds: 60)));
    ...
    test('bidirectional data exchange with connection reuse', () async {
      ...
      try {
        ...
        final serverNotifiee = TestNotifiee(connectedCallback: (network, conn) {
        ...
        final serverConn = await serverConnCompleter.future.timeout(
            ...
            onTimeout: () => throw TimeoutException(
                'Server did not receive connection in time'));
    ...
    test('large data transfer with connection reuse', () async {
      ...
      try {
        ...
        final clientConn = await swarmB.dialPeer(Context(), swarmA.localPeer);
        final serverConn =
            await serverConnCompleter.future.timeout(Duration(seconds: 10));
        ...
        for (final size in testSizes) {
          ...
          // Read echo response
          final receivedData =
              await clientStream.read().timeout(Duration(seconds: 10));
  ...
  try {
    while (!cancellation.isCompleted) {
      try {
        ...
        final stream =
            await ((serverConn as dynamic).conn as core_mux_types.MuxedConn)
                .acceptStream()
...
Future<void> _performIndependentEchoTest({
  ...
  try {
    ...
    final testData =
        'independent-echo-test-$testId-${DateTime.now().millisecondsSinceEpoch}';
    final dataBytes = utf8.encode(testData);
