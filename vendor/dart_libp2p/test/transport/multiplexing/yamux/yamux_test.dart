...
void main() {
  group('Yamux Multiplexing', () {
    ...
    setUp(() async {
      ...
      expect(session1.isClosed, isFalse,
    ...
    tearDown(() async {
      ...
      try {
        ...
        if (!conn1.isClosed) {
          await conn1
              .close()
              .catchError((e) => print('Error closing conn1: $e'));
        }
        if (!conn2.isClosed) {
          await conn2
              .close()
              .catchError((e) => print('Error closing conn2: $e'));
        }
      } catch (e) {
    ...
    test('creates new streams', () async {
      ...
      session2.setStreamHandler((stream) async {
        print(
            'Server received stream with ID: ${(stream as YamuxStream).id()}');
      ...
      final stream2 =
          await withTimeout(streamReceived.future, 'server stream acceptance');
    ...
    test('handles concurrent streams', () async {
      ...
      try {
        ...
        for (var i = 0; i < streams1.length; i++) {
          await streams1[i]
              .close()
              .catchError((e) => print('Error closing client stream $i: $e'));
          await streams2[i]
              .close()
              .catchError((e) => print('Error closing server stream $i: $e'));
        }
    ...
    test(
        'should allow opening multiple sequential streams after closing previous ones',
        () async {
      print('\n=== Sequential Stream Reuse Test ===');
      print(
          'Goal: Verify that a Yamux session can open new streams after closing old ones');
      ...
      try {
        ...
        for (var i = 0; i < numSequentialStreams; i++) {
          ...
          print(
              'Server accepted stream: ${(serverStream as YamuxStream).id()}');
          ...
          if (i < numSequentialStreams - 1) {
            ...
            expect(session1.isClosed, isFalse,
                reason:
                    'Client session should still be open after closing stream $i');
            expect(session2.isClosed, isFalse,
                reason:
    ...
    test('should not lose stream events with concurrent acceptStream calls',
        () async {
      print('\n=== Broadcast Stream Race Condition Test ===');
      print(
          'Goal: Verify that acceptStream() does not miss stream events due to broadcast stream timing');
      ...
      try {
        ...
        for (var i = 0; i < numIterations; i++) {
          ...
          final results = await Future.wait([
            ...
            onTimeout: () {
              ...
              print(
                  '   lost the event when the stream was added to the controller');
              throw TimeoutException(
                  'Race condition: acceptStream missed stream event');
            },
          );
          ...
          final received =
              await serverStream.read().timeout(Duration(seconds: 2));
          expect(received, equals(testData));
        ...
        print(
            'is losing stream events when acceptStream() is called concurrently with');
        print(
            'openStream(). The fix is to use a single-subscription StreamController');
    ...
    test('handles flow control', () async {
      ...
      expect(stream1.isClosed, isFalse,
          reason: 'Client stream should be open initially');
      expect(stream2.isClosed, isFalse,
          reason: 'Server stream should be open initially');
    ...
    test('handles session closure', () async {
      ...
      print('Creating streams...');
      final P2PStream<Uint8List> stream1 = await withTimeout(
              session1.openStream(core_context.Context()), 'stream creation')
          as P2PStream<Uint8List>;
      final stream2 =
          await withTimeout(streamReceived.future, 'stream acceptance');
      ...
      try {
        Future<void> pollForSession2Closure() async {
          ...
          if (!session2.isClosed) {
            ...
            print(
                'WARNING: Session2 did not close within the polling period (5s).');
          }
        }
      ...
      print(
          'Polling for session2 closure finished. session2.isClosed: ${session2.isClosed}');
    ...
    test('enforces maximum streams limit', () async {
      ...
      try {
        ...
        for (var i = 0; i < testMaxStreams; i += batchSize) {
          final count = (i + batchSize > testMaxStreams)
              ? (testMaxStreams - i)
              : batchSize;
          print(
          ...
          final batch = (await withTimeout(
            Future.wait(
              List.generate(count,
                  (_) => testSession1.openStream(core_context.Context())),
              eagerError: true,
            ),
    ...
    test('handles stream errors', () async {
      ...
      // Verify initial stream states
      expect(stream1.isClosed, isFalse,
          reason: 'Stream 1 should be open initially');
      expect(stream2.isClosed, isFalse,
          reason: 'Stream 2 should be open initially');
      ...
      try {
        ...
        expect(stream1.isClosed, isTrue,
            reason: 'Stream 1 should be closed after connection close');
        expect(stream2.isClosed, isTrue,
            reason: 'Stream 2 should be closed after connection close');
    ...
    test('handles stream reset', () async {
      ...
      expect(stream1.isClosed, isTrue,
          reason: 'Stream 1 should be closed after reset');
      ...
      await expectLater(
        () => withTimeout(stream2.write(Uint8List.fromList([1, 2, 3])),
      ...
      expect(stream2.isClosed, isTrue,
          reason: 'Stream 2 should be closed after reset');
    ...
    test('handles half-close (closeWrite)', () async {
      ...
      await expectLater(
        () => withTimeout(stream1.write(Uint8List.fromList([1])),
      ...
      expect(receivedData, equals(testData),
          reason: 'Stream 1 should receive data after its closeWrite');
      print('Stream 1 read data successfully');

      // 5. stream2 reading should now get an EOF because stream1 sent FIN
      await expectLater(
        () => withTimeout(
            stream2.read(), 'stream2 read after stream1 closeWrite'),
        throwsA(isA<StateError>()),
        reason:
            'Reading from stream2 should result in EOF/StateError after stream1 sent FIN',
      );
    ...
    test('handles local read closure (closeRead)', () async {
      ...
      expect(readResult.isEmpty, isTrue,
          reason: 'Read after closeRead should return EOF');
      ...
      await withTimeout(
          stream1.write(testDataFrom1), 'stream1 write after closeRead');
      ...
      expect(receivedData, equals(testDataFrom1),
          reason: 'Stream 2 should receive data from stream 1');
    ...
    test('handles large payload with rapid chunk delivery (OBP scenario)',
        () async {
      ...
      try {
        ...
        void checkSessionHealth(String phase) {
          ...
          if (session1.isClosed || session2.isClosed) {
            throw StateError(
                'Session closed during $phase - this indicates the Yamux GO_AWAY issue');
          }
        }
          ...
          try {
            for (final chunk in chunks) {
              ...
              if (chunksWritten % 10 == 0) {
                checkSessionHealth(
                    'Write chunk $chunksWritten/${chunks.length}');
              }
            }
        ...
        while (receivedData.length < largeData.length) {
          try {
            final chunk = await withTimeout(
                stream2.read(), 'chunk read ${readOperations + 1}');
            if (chunk.isEmpty) {
              print('Received empty chunk, stream might be closed');
              break;
            }
            ...
            if (readOperations % 10 == 0) {
              ...
              final progress = (receivedData.length * 100 ~/ largeData.length);
              print(
                  'Progress: ${receivedData.length}/${largeData.length} bytes (${progress}%)');
            }
          } catch (e) {
        ...
        expect(session1.isClosed, isFalse,
            reason: 'Session1 should remain open after large payload transfer');
        expect(session2.isClosed, isFalse,
            reason: 'Session2 should remain open after large payload transfer');
        expect(stream1.isClosed, isFalse,
            reason: 'Stream1 should remain open after large payload transfer');
        expect(stream2.isClosed, isFalse,
