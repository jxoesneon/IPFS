...
void main() {
  group('Swarm with UDX, Noise, and HTTP', () {
    ...
    setUpAll(() async {
      ...
      print(
          'Swarm UDX/Noise/HTTP Setup Complete. Client: ${clientPeerId.toString()}, Server: ${serverPeerId.toString()} listening on $serverListenAddr');
    });
    ...
    test(
        'should establish connection via Swarm, negotiate Noise/Yamux, and perform basic HTTP requests',
        () async {
      print(
          'Client Host (${clientPeerId.toString()}) connecting to Server Host (${serverPeerId.toString()}) for HTTP protocol');
      ...
      try {
        ...
        final response2 =
            await httpClient.getRequest(serverPeerId, '/api/status');
        ...
        final response3 =
            await httpClient.getRequest(serverPeerId, '/nonexistent');
    ...
    test('should handle POST requests with JSON data', () async {
      ...
      try {
        ...
        httpServer.post('/api/users', (request) async {
          ...
          if (userData == null) {
            return HttpResponse.error(
                HttpStatus.badRequest, 'Invalid JSON data');
          }

          if (!userData.containsKey('name') || !userData.containsKey('email')) {
            return HttpResponse.error(
                HttpStatus.badRequest, 'Missing required fields');
          }
        ...
        final response1 =
            await httpClient.postJson(serverPeerId, '/api/users', userData);
        ...
        final processData = {
          ...
          'metadata': {'source': 'http_test', 'version': '2.0'}
        };

        final response2 = await httpClient.postJson(
            serverPeerId, '/api/process', processData);
        ...
        final response4 = await httpClient.postJson(
            serverPeerId, '/api/users', incompleteData);
    ...
    test('should handle custom headers and advanced HTTP features', () async {
      ...
      try {
        ...
        await clientHost.connect(serverAddrInfo);
        print(
            'Client Host connected to Server Host for advanced features testing.');
        ...
        httpServer.get('/api/protected', (request) async {
          ...
          if (authHeader == null || !authHeader.startsWith('Bearer ')) {
            return HttpResponse.error(HttpStatus.unauthorized,
          ...
          return HttpResponse.json({
            ...
            'custom_header':
                request.headers['x-custom-header'] ?? 'not-provided',
          });
        });
        ...
        httpServer.get('/api/users/:userId/posts/:postId', (request) async {
          ...
          return HttpResponse.json({
            ...
            'content':
                'This is a sample post content for testing path parameters.',
          });
        });
        ...
        final response2 =
            await httpClient.getRequest(serverPeerId, '/api/protected');
        ...
        final response4 = await httpClient.getRequest(
            serverPeerId, '/api/users/user123/posts/post456');
        ...
        final response5 =
            await httpClient.getRequest(serverPeerId, '/api/headers-test');
    ...
    test('should handle PUT and DELETE HTTP methods', () async {
      ...
      try {
        ...
        httpServer.put('/api/users/:id', (request) async {
          ...
          if (updateData == null) {
            return HttpResponse.error(
                HttpStatus.badRequest, 'Invalid JSON data for update');
          }
        ...
        httpServer.put('/api/status/:code', (request) async {
          ...
          switch (statusCode) {
            ...
            case 202:
              return HttpResponse.json({'message': 'Accepted', 'code': 202},
            ...
            default:
              return HttpResponse.error(
                  HttpStatus.badRequest, 'Unsupported status code');
          }
        });
    ...
    test('should handle concurrent HTTP requests efficiently', () async {
      ...
      try {
        ...
        for (int i = 1; i <= 5; i++) {
          final future =
              httpClient.getRequest(serverPeerId, '/api/concurrent/req$i');
        ...
        for (int i = 6; i <= 8; i++) {
          mixedRequests.add(
              httpClient.getRequest(serverPeerId, '/api/concurrent/mixed$i'));
        }
        ...
        for (int i = 1; i <= 2; i++) {
          ...
          mixedRequests.add(
              httpClient.postJson(serverPeerId, '/api/load-test', postData));
        }
        ...
        for (int i = 1; i <= 10; i++) {
          final response = await httpClient.getRequest(
              serverPeerId, '/api/concurrent/seq$i');
      ...
      try {
        ...
        httpServer.get('/api/error/:code', (request) async {
          ...
          switch (errorCode) {
            case '400':
              return HttpResponse.error(
                  HttpStatus.badRequest, 'Bad request error');
            case '401':
              return HttpResponse.error(
                  HttpStatus.unauthorized, 'Unauthorized access');
            case '403':
              return HttpResponse.error(
                  HttpStatus.forbidden, 'Forbidden resource');
            case '404':
              return HttpResponse.error(
            ...
            default:
              return HttpResponse.error(
                  HttpStatus.badRequest, 'Unknown error code');
          }
        });
        ...
        httpServer.post('/api/validate-content', (request) async {
          ...
          if (data == null) {
            return HttpResponse.error(
                HttpStatus.badRequest, 'Invalid JSON in request body');
          }

          return HttpResponse.json(
              {'message': 'Content validated successfully', 'received': data});
        });
        ...
        httpServer.post('/api/size-check', (request) async {
          ...
          if (bodySize > 1000) {
            return HttpResponse.error(
                HttpStatus.badRequest, 'Request body too large');
          }
        ...
        final response1 =
            await httpClient.getRequest(serverPeerId, '/api/error/exception');
        ...
        for (int i = 0; i < errorCodes.length; i++) {
          ...
          final response = await httpClient.getRequest(
              serverPeerId, '/api/error/${errorCodes[i]}');
        ...
        final response4a = await httpClient.postJson(
            serverPeerId, '/api/size-check', smallData);
        ...
        final response4b = await httpClient.postJson(
            serverPeerId, '/api/size-check', largeData);
