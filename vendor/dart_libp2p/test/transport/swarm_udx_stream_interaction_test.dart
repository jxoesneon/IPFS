...
class StreamEOFUtils {
  ...
  static Future<Uint8List> readExactBytes(
    ...
    while (buffer.length < expectedBytes) {
      ...
      if (chunk == null) {
        throw StateError(
            '[$context] Unexpected EOF: got ${buffer.length}/$expectedBytes bytes');
      }
      ...
      if (buffer.length > expectedBytes) {
        ...
        print(
            '[$context] Warning: received ${buffer.length} bytes, expected $expectedBytes');
...
void main() {
  group('SwarmStream and UDXP2PStreamAdapter Interaction', () {
      ...
      final muxerDef = StreamMuxer(
        ...
        muxerFactory: (conn, isClient) {
          if (conn is! TransportConn)
            throw ArgumentError('Expected TransportConn');
    ...
    Future<
        (
          core_network_stream.P2PStream,
          core_network_stream.P2PStream,
          Conn,
          Conn
        )> createStreamsWithConnections() async {
      ...
      final serverConn =
          await serverConnCompleter.future.timeout(Duration(seconds: 5));

      serverSwarm.stopNotify(serverNotifiee);

      // Access the underlying MuxedConn via the .conn property of SwarmConn
      final serverStreamFuture =
          ((serverConn as dynamic).conn as core_mux_types.MuxedConn)
              .acceptStream();
      final clientStream =
      ...
      final serverStream =
          await serverStreamFuture.timeout(Duration(seconds: 5));

      return (
        clientStream as core_network_stream.P2PStream,
        serverStream as core_network_stream.P2PStream,
        clientConn,
        serverConn
      );
    }
    ...
    test('should establish a stream and perform basic read/write', () async {
      final (clientStream, serverStream, clientConn, serverConn) =
          await createStreamsWithConnections();
    ...
    test('should handle large data transfer (client to server)', () async {
      ...
      try {
        ...
        final serverAcceptStreamFuture =
          ...
          print(
              'Server accepted P2PStream: ${serverP2PStream.id()} from ${serverP2PStream.conn.remotePeer}');
        ...
        final core_network_stream.P2PStream clientP2PStream =
            await ((clientConn as dynamic).conn as core_mux_types.MuxedConn)
                    .openStream(core_context.Context())
                as core_network_stream.P2PStream;
        print(
            'Client opened P2PStream: ${clientP2PStream.id()} to ${clientP2PStream.conn.remotePeer}');
        ...
        final pingData = Uint8List.fromList(
            List.generate(1024, (_) => random.nextInt(256))); // 1KB
        ...
        final serverReadFuture =
            serverP2PStream.read().timeout(Duration(seconds: 10));
        ...
        final echoedToClient =
            await clientP2PStream.read().timeout(Duration(seconds: 10));
        print(
    ...
    test('should handle stream reset correctly', () async {
      ...
      try {
        ...
        await expectLater(
            () => clientStream.write(Uint8List(1)), throwsA(isA<StateError>()));
  ...
  group('Stream EOF and Best Practices Demonstration', () {
    ...
    test('EOF Detection Patterns - Graceful closeWrite()', () async {
      ...
      try {
        ...
        final serverReadFuture = () async {
          ...
          final data = await StreamEOFUtils.readUntilEOF(serverStream,
              context: 'Server');
        ...
        final serverReadFuture = () async {
          ...
          try {
            final data = await StreamEOFUtils.readUntilEOF(serverStream,
                context: 'Server');
            print(
                '[Server] Read completed with ${data.length} bytes (unexpected)');
          } catch (e) {
    ...
    test('Chunked Data Reading with EOF', () async {
      ...
      try {
        ...
        final chunks = [
          Uint8List.fromList('Chunk 1: '.codeUnits),
          Uint8List.fromList(
              'This is a longer chunk with more data. '.codeUnits),
        ...
        final serverReadFuture = () async {
          ...
          final allData = await StreamEOFUtils.readUntilEOF(serverStream,
              context: 'Server');
        ...
        for (int i = 0; i < chunks.length; i++) {
          ...
          await Future.delayed(
              Duration(milliseconds: 100)); // Simulate network delay
        }
    ...
    test('Protocol-Style Reading (Length-Prefixed)', () async {
      ...
      try {
        ...
        final message = Uint8List.fromList(
            'This is a protocol message with known length!'.codeUnits);
        final lengthPrefix = ByteData(4)
          ..setUint32(0, message.length, Endian.big);

        // Server reads using protocol pattern
        final serverReadFuture = () async {
          print('[Server] Reading length prefix (4 bytes)...');
          final lengthBytes = await StreamEOFUtils.readExactBytes(
              serverStream, 4,
              context: 'Server-Length');
          final messageLength =
          ...
          final messageData = await StreamEOFUtils.readExactBytes(
              serverStream, messageLength,
              context: 'Server-Data');
        ...
        final ack =
            await StreamEOFUtils.safeRead(clientStream, context: 'Client-ACK');
    ...
    test('Bidirectional Communication with Proper EOF', () async {
      ...
      try {
        final clientMessage =
            Uint8List.fromList('Hello from client!'.codeUnits);
        final serverMessage =
            Uint8List.fromList('Hello from server!'.codeUnits);

        // Both sides communicate simultaneously
        final serverLogic = () async {
          print('[Server] Reading client message...');
          final receivedFromClient =
              await StreamEOFUtils.safeRead(serverStream, context: 'Server');
        ...
        final clientLogic = () async {
          ...
          final receivedFromServer =
              await StreamEOFUtils.safeRead(clientStream, context: 'Client');
        ...
        await Future.wait([serverLogic, clientLogic])
            .timeout(Duration(seconds: 15));
        print(
            '=== Bidirectional communication test completed successfully ===\n');
      } finally {
        await clientStream.close();
        await serverStream.close();
        await clientConn.close();
    ...
    test('Timeout and Error Recovery Patterns', () async {
      ...
      try {
        ...
        print(
            '[Server] Attempting read with short timeout (should timeout)...');
        final timeoutResult = await StreamEOFUtils.safeRead(serverStream,
            timeout: Duration(milliseconds: 500), context: 'Server-Timeout');
        ...
        final serverReadFuture = () async {
          ...
          final data = await StreamEOFUtils.safeRead(serverStream,
              context: 'Server-Real');
          expect(data, equals(testData));
        ...
        await serverReadFuture.timeout(Duration(seconds: 10));
        print(
            '=== Timeout and error recovery test completed successfully ===\n');
      } finally {
        await clientStream.close();
        await serverStream.close();
        await clientConn.close();
