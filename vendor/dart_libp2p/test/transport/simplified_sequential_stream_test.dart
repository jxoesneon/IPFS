...
void main() {
  ...
  Logger.root.level =
      Level.INFO; // Start with INFO, can be changed to ALL for deep dive
  ...
  group('Simplified Sequential Stream Test (TCP -> Noise -> Yamux)', () {
    ...
    setUp(() async {
      ...
      final connManager =
          NullConnMgr(); // Not strictly needed for upgrader but TCPTransport takes it

      clientTcpTransport = TCPTransport(
          resourceManager: resourceManager, connManager: connManager);
      serverTcpTransport = TCPTransport(
          resourceManager: resourceManager, connManager: connManager);

      clientKeyPair = await crypto_ed25519.generateEd25519KeyPair();
      clientPeerId =
          await concrete_peer_id.PeerId.fromPublicKey(clientKeyPair.publicKey);
      serverKeyPair = await crypto_ed25519.generateEd25519KeyPair();
      serverPeerId =
          await concrete_peer_id.PeerId.fromPublicKey(serverKeyPair.publicKey);
        ...
        ..muxers = [
          config_stream_muxer.StreamMuxer(
              ...
              muxerFactory: (Conn secureConn, bool isClient) {
                final yamuxInternalConfig =
                    p2p_mux.MultiplexerConfig(); // Default config
                return YamuxSession(
                    secureConn as TransportConn, yamuxInternalConfig, isClient);
              })
        ];
      ...
      final clientDialFuture = clientTcpTransport.dial(serverListenAddr,
      ...
      testLog.info(
          'Raw TCP connection established. Client: \${rawClientConn?.id}, Server: \${rawServerConn?.id}');

      expect(rawClientConn, isNotNull,
          reason: 'Raw client connection should be established.');
      expect(rawServerConn, isNotNull,
          reason: 'Raw server connection should be established.');
      ...
      final upgradedResults =
          await Future.wait([clientUpgradeFuture, serverUpgradeFuture]);
      upgradedClientConn = upgradedResults[0];
      upgradedServerConn = upgradedResults[1];
      testLog.info(
          'Connections upgraded. Client: \${upgradedClientConn?.state.security}/\${upgradedClientConn?.state.streamMultiplexer}, Server: \${upgradedServerConn?.state.security}/\${upgradedServerConn?.state.streamMultiplexer}');

      expect(upgradedClientConn, isNotNull,
          reason: 'Upgraded client connection should not be null.');
      expect(upgradedServerConn, isNotNull,
          reason: 'Upgraded server connection should not be null.');
      await Future.delayed(
          Duration(milliseconds: 200)); // Allow Yamux sessions to fully init
      testLog.info('=== Test Setup Complete ===');
    });

    tearDown(() async {
      testLog.info('=== Test Teardown Starting ===');
      await upgradedClientConn?.close().catchError(
          (e) => testLog.warning('Error closing upgradedClientConn: \$e'));
      await upgradedServerConn?.close().catchError(
          (e) => testLog.warning('Error closing upgradedServerConn: \$e'));
    ...
    test('Open, use, and close first stream, then open and use second stream',
        () async {
      ...
      final clientStream1Future =
          upgradedClientConn!.newStream(Context()); // Placeholder streamID
      ...
      final P2PStream clientStream1 = await clientStream1Future
          .timeout(Duration(seconds: 5)); // Explicit type P2PStream
      testLog.info(
          'Client stream 1 opened. ID (Yamux): \${(clientStream1 as YamuxStream).id()}'); // Cast to YamuxStream for id()
      final core_mux.MuxedStream serverStream1 = await serverStream1Future
          .timeout(Duration(seconds: 5)); // Explicit type
      testLog.info(
          'Server stream 1 accepted. ID (Yamux): \${(serverStream1 as YamuxStream).id()}'); // Cast to YamuxStream for id()
      ...
      final List<int> remainingFromServer1_list =
          await serverStream1.read(1); // Try to read 1 byte for EOF check
      final Uint8List remainingFromServer1 =
          Uint8List.fromList(remainingFromServer1_list);
      expect(remainingFromServer1.isEmpty, isTrue,
          reason:
              'Server should read EOF after client closes write on stream 1');
      ...
      final P2PStream clientStream2 = await clientStream2Future
          .timeout(Duration(seconds: 5)); // Explicit type P2PStream
      testLog.info(
          'Client stream 2 opened. ID (Yamux): \${(clientStream2 as YamuxStream).id()}'); // Cast to YamuxStream for id()
      final core_mux.MuxedStream serverStream2 = await serverStream2Future
          .timeout(Duration(seconds: 5)); // Explicit type
      testLog.info(
          'Server stream 2 accepted. ID (Yamux): \${(serverStream2 as YamuxStream).id()}'); // Cast to YamuxStream for id()
      ...
      try {
        ...
        await clientStream2.close().catchError(
            (e) => testLog.warning('Error closing clientStream2: \$e'));
        await serverStream2.close().catchError(
            (e) => testLog.warning('Error closing serverStream2: \$e'));
