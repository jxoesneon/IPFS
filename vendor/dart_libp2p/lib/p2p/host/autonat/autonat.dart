...
class AmbientAutoNAT implements AutoNAT, Notifiee {
  ...
  AmbientAutoNAT._(this._host, this._config, this._service) {
    _client = AutoNATV1ClientImpl(_host, _config.addressFunc,
        _config.metricsTracer, _config.requestTimeout);
  ...
  static Future<AmbientAutoNAT> create(
      Host h, List<AutoNATOption> options) async {
  ...
  void _scheduleNextProbe(bool forceProbe) {
    ...
    if (forceProbe && _currentStatus == Reachability.unknown) {
      ...
      nextProbeAfter =
          Duration(microseconds: _config.refreshInterval.inMicroseconds * 2);
  ...
  void _recordObservation(Reachability observation) {
    ...
    _config.metricsTracer
        ?.reachabilityStatus(_currentStatus); // Always update metric
    _config.metricsTracer
        ?.reachabilityStatusConfidence(_confidence); // Always update metric
  }
...
class StaticAutoNAT implements AutoNAT {
  ...
  static Future<StaticAutoNAT> create(
      Host h, Reachability reachability, List<AutoNATOption> options) async {
