...
class ProtocolScopeImpl extends ResourceScopeImpl implements ProtocolScope {
  ...
  ProtocolScopeImpl(Limit limit, this.protocol,
      {List<ResourceScopeImpl>? edges})
      : super(limit, 'protocol:$protocol',
            edges: edges); // Scope name is prefixed
  ...
  ResourceScopeImpl getPeerSubScope(
    ...
    return _peerSubScopes.putIfAbsent(peerId, () {
      final peerProtocolLimit =
          limiter.getProtocolPeerLimits(this.protocol, peerId);
      final scopeName =
          'protocol:${protocol.toString()}-peer:${peerId.toString()}';
      ...
      final newPeerSubScope = ResourceScopeImpl(
        ...
        edges: [
          this,
          systemScope
        ], // Parent is this protocol scope and system scope
      );
