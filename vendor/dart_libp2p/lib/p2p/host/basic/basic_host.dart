...
class BasicHost implements Host {
  ...
  BasicHost._({
    ...
    _network.setStreamHandler("/libp2p/host",
        (dynamic stream, PeerId remotePeer) async {
  ...
  void _emitAddressChangeEvent(
      List<MultiAddr> prev, List<MultiAddr> current) async {
    ...
    for (final addr in prevMap.values) {
      removedAddrs
          .add(UpdatedAddress(address: addr, action: AddrAction.removed));
    }
        ...
        for (final interface in interfaces) {
          ...
          for (final address in interface.addresses) {
            ...
            try {
              ...
              final protocolName =
                  address.type == InternetAddressType.IPv4 ? 'ip4' : 'ip6';
    ...
    for (final listenAddr in currentListenAddrs) {
      ...
      if (isUnspecified) {
        ...
        if (ipComponentIndex != -1 &&
          ...
          final suffixComponents =
              listenComponents.sublist(ipComponentIndex + 1);
  ...
  List<MultiAddr> get publicAddrs {
    ...
    if (result.isEmpty) {
      final fallbackAddrs =
          allAddrs.where((addr) => !isRelayAddress(addr)).toList();
      _log.fine(
  ...
  Future<void> connect(AddrInfo pi, {Context? context}) async {
    ...
    await peerStore.addrBook
        .addAddrs(pi.id, filteredAddrs, Duration(minutes: 5));
  ...
  void setStreamHandler(ProtocolID pid, StreamHandler handler) {
    ...
    if (_evtLocalProtocolsUpdated != null) {
      _evtLocalProtocolsUpdated!
          .emit(EvtLocalProtocolsUpdated(added: [pid], removed: []));
    }
  }
  ...
  void removeStreamHandler(ProtocolID pid) {
    ...
    if (_evtLocalProtocolsUpdated != null) {
      _evtLocalProtocolsUpdated!
          .emit(EvtLocalProtocolsUpdated(added: [], removed: [pid]));
    }
  }
