...
class YamuxStream implements P2PStream<Uint8List>, core_mux.MuxedStream {
  ...
  YamuxStream({
    ...
    _log.fine(
        '$_logPrefix Constructor. Initial local window: $_localReceiveWindow, Initial remote window (our send): $_remoteReceiveWindow, Max frame size: $_maxFrameSize');
  }
  ...
  Future<void> open() async {
    ...
    _openedAt =
        DateTime.now(); // Record when stream opened for duration calculation
    _log.fine(
  ...
  Future<void> write(List<int> data) async {
    ...
    if (_state != YamuxStreamState.open && _state != YamuxStreamState.closing) {
      _log.warning(
          '$_logPrefix YamuxStream.write: Called on non-open/non-closing stream. State: $_state. Requested: $inputDataLength bytes.');
  ...
  Future<void> _writeDirectly(List<int> data) async {
    ...
    try {
      ...
      while (offset < dataToWrite.length &&
          (_state == YamuxStreamState.open ||
    ...
    try {
      ...
      if (_metricsObserver != null) {
        try {
          ...
          _log.warning(
              '$_logPrefix Error notifying metrics observer on reset: $e');
        }
      }
    ...
    try {
      ...
      if (_metricsObserver != null && _openedAt != null) {
        try {
          ...
          _log.warning(
              '$_logPrefix Error notifying metrics observer on close: $e');
        }
      }
  ...
  Future<Uint8List> _performRead(int? maxLength) async {
    ...
    if (_state == YamuxStreamState.closing) {
      ...
      if (_localFinSent) {
        _log.fine(
            '$_logPrefix Both FINs sent and all data consumed. Now safe to cleanup.');
        await _cleanup();
      }
  ...
  Future<void> closeWrite() async {
    ...
    try {
      ...
      _log.fine(
          '$_logPrefix closeWrite() completed. Stream remains readable in state: $_state.');
    } catch (e) {
      _log.severe(
