...
import '../../core/network/conn.dart'
    show Conn, ConnStats, ConnState, Stats; // Added Stats
...
class UDXTransport implements Transport {
  ...
  UDXTransport({
    ...
    _logger.fine(
        '[UDXTransport] Initialized with config: $config. UDX instance created.');
  ...
  Future<TransportConn> dial(MultiAddr addr, {Duration? timeout}) async {
    _logger.fine(
        '[UDXTransport.dial] Attempting to dial $addr with timeout: ${timeout ?? config.dialTimeout}');
  ...
  Future<TransportConn> _performDial(
    ...
    try {
      _logger.fine(
          '[UDXTransport._performDial] Creating RawDatagramSocket for $host');
      ...
      rawSocket = await UDXExceptionHandler.handleUDXOperation(
        () => RawDatagramSocket.bind(
            isIPv6 ? InternetAddress.anyIPv6 : InternetAddress.anyIPv4, 0),
      ...
      _logger.fine(
          '[UDXTransport._performDial] UDPSocket created through multiplexer');

      // Generate random unique stream IDs for this dial attempt
      final random = Random();
      final int localInitialStreamId =
          random.nextInt(0xFFFFFFFF) + 1; // Ensure non-zero
      int remoteInitialStreamId = random.nextInt(0xFFFFFFFF) + 1;
      while (remoteInitialStreamId == localInitialStreamId) {
        // Ensure they are different
        remoteInitialStreamId = random.nextInt(0xFFFFFFFF) + 1;
      }

      _logger.fine(
          '[UDXTransport._performDial] Generated UDX Stream IDs - Local: $localInitialStreamId, Remote: $remoteInitialStreamId');

      // Create initial stream to trigger handshake with timeout and exception handling
      _logger.fine(
          '[UDXTransport._performDial] Creating outgoing UDXStream to $host:$port (localId: $localInitialStreamId, remoteId: $remoteInitialStreamId)');
      ...
      _logger.fine(
          '[UDXTransport._performDial] Outgoing UDXStream created: ${initialStream!.id}');

      // Phase 1.2: UDX Socket Health Monitoring - Handshake timing
      _logger
          .fine('[UDXTransport._performDial] Handshake start for $host:$port');
      ...
      final localProtocol =
          rawSocket.address.type == InternetAddressType.IPv6 ? 'ip6' : 'ip4';
      final localMa = MultiAddr(
          '/$localProtocol/${rawSocket.address.address}/udp/${rawSocket.port}/udx');
      final remoteMa = addr;
      ...
      final sessionConn = UDXSessionConn(
        ...
        onClosed: (conn) {
          ...
          _logger.fine(
              '[UDXTransport._performDial] Dialer connection ${conn.id} closed and removed from active set.');
        },
      );
      _connManager.registerConnection(sessionConn);
      _activeDialerConns.add(sessionConn);
      _logger.fine(
          '[UDXTransport._performDial] UDXSessionConn created, registered, and tracked successfully for $addr. ID: ${sessionConn.id}');
      return sessionConn;
    } catch (error) {
      _logger.warning(
          '[UDXTransport._performDial] Error during dial to $addr: $error. Performing cleanup.');
  ...
  Future<Listener> listen(MultiAddr addr) async {
    ...
    try {
      _logger.fine(
          '[UDXTransport.listen] Creating RawDatagramSocket for $host:$port');
      ...
      final protocol =
          rawSocket.address.type == InternetAddressType.IPv6 ? 'ip6' : 'ip4';
      final boundMa = MultiAddr(
          '/$protocol/${rawSocket.address.address}/udp/${rawSocket.port}/udx');
      ...
      _logger.fine(
          '[UDXTransport.listen] UDXListener created successfully for $boundMa and added to active listeners.');
      return listener;
    } catch (e) {
      _logger.fine(
          '[UDXTransport.listen] Exception during listen on $addr: $e. Cleaning up.');
      multiplexer?.close();
  ...
  Future<void> dispose() async {
    _logger.fine(
        '[UDXTransport.dispose] Disposing UDXTransport. Closing ${_activeListeners.length} active listeners and ${_activeDialerConns.length} active dialer connections.');
    ...
    for (final listener in _activeListeners.toList()) {
      try {
        ...
        _logger.fine(
            '[UDXTransport.dispose] Error closing listener ${listener.addr}: $e');
      }
    }
    _activeListeners.clear();
    _logger.fine(
        '[UDXTransport.dispose] All active listeners closed and cleared.');
    ...
    for (final conn in _activeDialerConns.toList()) {
      try {
        ...
        _logger.fine(
            '[UDXTransport.dispose] Error closing dialer connection ${conn.id}: $e');
      }
    }
    _activeDialerConns.clear();
    _logger.fine(
        '[UDXTransport.dispose] All active dialer connections closed and cleared.');
  }
}
...
class UDXSessionConn implements MuxedConn, TransportConn {
  ...
  final StreamController<MuxedStream> _incomingStreamsController =
      StreamController<MuxedStream>.broadcast();
  ...
  UDXSessionConn({
    ...
    void Function(UDXSessionConn conn)? onClosed,
        ...
        id =
            'udx-${DateTime.now().millisecondsSinceEpoch}-${localMultiaddr.hashCode ^ remoteMultiaddr.hashCode}' {
    // Phase 1.1: UDX Connection Lifecycle Analysis - Enhanced Constructor Logging
    _logger.fine(
        '[UDXSessionConn $id] LIFECYCLE: Constructor start - isDialer=$_isDialer, local=$localMultiaddr, remote=$remoteMultiaddr');
    _logger.fine(
        '[UDXSessionConn $id] LIFECYCLE: Initial stream ID=${initialStream.id}, UDPSocket created');
    _logger.fine(
        '[UDXSessionConn $id] LIFECYCLE: Connection opened at $_openedAt');

    if (_isDialer) {
      _nextOwnStreamId = 3;
      _nextExpectedRemoteStreamId = 4;
      _logger.fine(
          '[UDXSessionConn $id] Dialer stream IDs initialized: nextOwn=$_nextOwnStreamId, nextExpectedRemote=$_nextExpectedRemoteStreamId');
      ...
      _initialStream.on('error').listen((event) {
        ...
        final timeSinceOpen = DateTime.now().difference(_openedAt);
        _logger.severe(
            '[UDXSessionConn $id] INITIAL_STREAM_ERROR_HANDLER: $error, time_since_open: ${timeSinceOpen.inMilliseconds}ms');
        ...
        if (classifiedException is UDXPacketLossException) {
          _logger.warning(
              '[UDXSessionConn $id] Packet permanently lost on initial stream. Connection failed.');
        }
      ...
      _udpSocket.on('close').listen((event) {
        final timeSinceOpen = DateTime.now().difference(_openedAt);
        _logger.warning(
            '[UDXSessionConn $id] UDP_SOCKET_CLOSED: reason=${event.data}, activeStreams=${_activeStreams.length}, time_since_open: ${timeSinceOpen.inMilliseconds}ms');
      });
    } else {
      _nextOwnStreamId = 1;
      _nextExpectedRemoteStreamId = 2;
      _logger.fine(
          '[UDXSessionConn $id] Listener stream IDs initialized: nextOwn=$_nextOwnStreamId, nextExpectedRemote=$_nextExpectedRemoteStreamId');
    }

    _logger
        .fine('[UDXSessionConn $id] Subscribing to initialStream closeEvents.');
    _initialStreamCloseSubscription = _initialStream.closeEvents.listen((_) {
      _logger.fine(
          '[UDXSessionConn $id] Initial stream closed event. Active streams: ${_activeStreams.length}');
      ...
      if (_activeStreams.isEmpty && !_isClosing && !_isClosed) {
        _logger.fine(
            '[UDXSessionConn $id] No other active streams. Closing session.');
        close();
      } else {
        _logger.fine(
            '[UDXSessionConn $id] Other streams still active (${_activeStreams.length}). Keeping session open.');
      }
    }, onError: (err, s) {
      _logger.fine(
    ...
    try {
      _logger.fine(
          '[UDXSessionConn $id] Adapting initial stream ${_initialStream.id} to UDXP2PStreamAdapter.');
      ...
      _logger.fine(
          '[UDXSessionConn $id] Initial stream adapter created, stored, and assigned to initialP2PStream.');
    } catch (e, s) {
      _logger.fine(
          '[UDXSessionConn $id] Error adapting initial stream: $e. Closing session and rethrowing.');
      closeWithError(e, s);
      rethrow;
    }
    if (_isDialer) {
      _logger.fine(
          '[UDXSessionConn $id] Dialer session, subscribing to its own UDPSocket unmatchedUDXPacket events.');
      _socketMessageSubscription = _udpSocket.on('unmatchedUDXPacket').listen(
        ...
        onDone: () {
          _logger.fine(
              '[UDXSessionConn $id] (Dialer) UDPSocket closed. Closing session.');
          close();
        },
      );
    }
  ...
  void handleRemoteOpenedStream(UDXPacket packet, Uint8List rawData,
      InternetAddress remoteAddress, int remotePort) {
    _logger.fine(
        '[UDXSessionConn $id] handleRemoteOpenedStream called by UDXListener. Packet for destId: ${packet.destinationStreamId}, srcId: ${packet.sourceStreamId}');
    if (_isClosed) {
      _logger.fine(
          '[UDXSessionConn $id] Session closed, ignoring handleRemoteOpenedStream.');
      return;
    }
    try {
      ...
      _logger.fine(
          '[UDXSessionConn $id] Creating incoming UDXStream for remote-opened stream. LocalId: $localId, RemoteId: $remoteId');
      ...
      _logger.fine(
          '[UDXSessionConn $id] Incoming UDXStream created: ${newRemoteStream.id}');
      ...
      _logger.fine(
          '[UDXSessionConn $id] Handled socket event for new incoming UDXStream ${newRemoteStream.id}');
      ...
      _logger.fine(
          '[UDXSessionConn $id] Adapted new incoming UDXStream ${newRemoteStream.id} and added to active streams.');
      if (!_incomingStreamsController.isClosed) {
        _incomingStreamsController.add(adapter);
        _logger.fine(
            '[UDXSessionConn $id] Added new stream adapter ${adapter.id()} to incoming streams controller.');
      } else {
        _logger.fine(
            '[UDXSessionConn $id] (Listener-Side): Incoming stream controller closed, closing new remote stream ${newRemoteStream.id}');
        adapter.close();
        _activeStreams.remove(newRemoteStream.id);
      }
    } catch (e, s) {
      _logger.fine(
          '[UDXSessionConn $id] (Listener-Side): Error handling remote opened stream: $e\n$s');
    }
  }

  void _handleDialerUnmatchedPacket(dynamic event) {
    _logger.fine(
        '[UDXSessionConn $id] _handleDialerUnmatchedPacket called. Event: $event');
    if (_isClosed) {
      _logger.fine(
          '[UDXSessionConn $id] Session closed, ignoring _handleDialerUnmatchedPacket.');
      return;
    }
    ...
    _logger.fine(
        '[UDXSessionConn $id] Unmatched packet details: destId=${packet.destinationStreamId}, srcId=${packet.sourceStreamId}, from=${remoteAddress.address}:$remotePort');

    final expectedRemoteHost = _remoteMultiaddr.valueForProtocol(
        remoteAddress.type == InternetAddressType.IPv4 ? 'ip4' : 'ip6');
    final expectedRemotePort =
        int.parse(_remoteMultiaddr.valueForProtocol('udp')!);

    if (remoteAddress.address != expectedRemoteHost ||
        remotePort != expectedRemotePort) {
      _logger.fine(
          '[UDXSessionConn $id] (Dialer): Received unmatched packet from unexpected source ${remoteAddress.address}:$remotePort. Expected $expectedRemoteHost:$expectedRemotePort. Ignoring.');
      return;
    }
    ...
    try {
      ...
      final remoteId = packet.sourceStreamId;
      _logger.fine(
          '[UDXSessionConn $id] Creating incoming UDXStream for remote-opened stream on dialer session. LocalId: $localId, RemoteId: $remoteId');
      ...
      _logger.fine(
          '[UDXSessionConn $id] Incoming UDXStream created on dialer session: ${newRemoteStream.id}');
      ...
      _logger.fine(
          '[UDXSessionConn $id] Handled socket event for new incoming UDXStream ${newRemoteStream.id} on dialer session.');
      ...
      _activeStreams[newRemoteStream.id] = adapter;
      _logger.fine(
          '[UDXSessionConn $id] Adapted new incoming UDXStream ${newRemoteStream.id} on dialer session and added to active streams.');
      if (!_incomingStreamsController.isClosed) {
        _incomingStreamsController.add(adapter);
        _logger.fine(
            '[UDXSessionConn $id] Added new stream adapter ${adapter.id()} to incoming streams controller on dialer session.');
      } else {
        _logger.fine(
            '[UDXSessionConn $id] (Dialer): Incoming stream controller closed, closing new remote stream ${newRemoteStream.id}');
        adapter.close();
        _activeStreams.remove(newRemoteStream.id);
      }
    } catch (e, s) {
      _logger.fine(
          '[UDXSessionConn $id] (Dialer): Error handling unmatched packet for new stream: $e\n$s');
    }
  }
  ...
  Future<MuxedStream> openStream(Context context) async {
    ...
    _nextExpectedRemoteStreamId += 2;
    _logger.fine(
        '[UDXSessionConn $id] Opening new stream. LocalId: $localStreamId, RemoteExpectedId: $remoteStreamId');

    final remoteHost = _remoteMultiaddr.valueForProtocol('ip4') ??
        _remoteMultiaddr.valueForProtocol('ip6');
    final remotePort =
        int.parse(_remoteMultiaddr.valueForProtocol('udp') ?? '0');

    if (remoteHost == null || remotePort == 0) {
      _logger.fine(
          '[UDXSessionConn $id] Remote address not set, cannot open stream.');
      throw StateError('Remote address not properly set for UDXSessionConn');
    }

    _logger.fine(
        '[UDXSessionConn $id] Creating outgoing UDXStream to $remoteHost:$remotePort (localId: $localStreamId, remoteId: $remoteStreamId)');
    ...
    _logger.fine(
        '[UDXSessionConn $id] Outgoing UDXStream created: ${udxStream.id}');
    ...
    _logger.fine(
        '[UDXSessionConn $id] New outgoing stream ${muxedStream.id()} adapted and stored.');
    return muxedStream;
  }
  ...
  Future<MuxedStream> acceptStream() async {
    ...
    if (_isClosed && _incomingStreamsController.isClosed) {
      _logger.fine(
          '[UDXSessionConn $id] Session closed, cannot accept new streams.');
      throw SocketException(
          'UDX session is closed, cannot accept new streams.');
    }
    final stream = await _incomingStreamsController.stream.first;
    _logger.fine(
        '[UDXSessionConn $id] Accepted stream: ${(stream as UDXP2PStreamAdapter).id()}');
    return stream;
  }

  @override
  Future<List<P2PStream>> get streams async =>
      throw UnimplementedError("Use acceptStream() in a loop for MuxedConn.");
  ...
  PeerId get localPeer =>
      _localPeer ?? (throw StateError("Local PeerId not set."));

  @override
  PeerId get remotePeer =>
      _remotePeer ??
      (throw StateError(
          "Remote PeerId not set; established during security handshake."));

  void setRemotePeerDetails(
      PeerId peerId, PublicKey pubKey, String securityProto) {
    _remotePeer = peerId;
  ...
  ConnStats get stat {
    final connStatsInstance = Stats(
      ...
      opened: _openedAt,
      limited:
          false, // Placeholder: UDXSessionConn doesn't have direct access to scope's limited status
      extra: {
        'transport': 'udx',
        'security': _securityProtocol,
        'muxer': '/udx/1.0.0'
  ...
  @override
  ConnScope get scope =>
      throw UnimplementedError("Scope not yet implemented for UDXSessionConn.");

  @override
  Future<P2PStream> newStream(Context context, [int? streamId]) async =>
      throw UnimplementedError(
          "Use openStream(context) for UDXSessionConn, streamId is managed by UDX.");

  @override
  void setReadTimeout(Duration timeout) => throw UnimplementedError(
      "Set timeouts on individual P2PStreams if supported.");

  @override
  void setWriteTimeout(Duration timeout) => throw UnimplementedError(
      "Set timeouts on individual P2PStreams if supported.");

  @override
  Socket get socket => throw UnimplementedError(
      "UDX connections operate over UDPSocket, not a direct dart:io.Socket.");
  ...
  void notifyStreamClosed(int streamId) {
    _logger.fine(
        '[UDXSessionConn $id] Stream $streamId closed. Active streams before removal: ${_activeStreams.length}');
    _activeStreams.remove(streamId);
    _logger.fine(
        '[UDXSessionConn $id] Active streams after removal: ${_activeStreams.length}');

    // If no more active streams and we're not already closing, close the session
    if (_activeStreams.isEmpty && !_isClosing && !_isClosed) {
      _logger.fine(
          '[UDXSessionConn $id] No active streams remaining. Closing session.');
      close();
    }
  }
  ...
  Future<void> close() async {
    ...
    _logger.fine(
        '[UDXSessionConn $id] CLOSE_START: Is already closed: $_isClosed, Is closing: $_isClosing, time_since_open: ${timeSinceOpen.inMilliseconds}ms');
    ...
    _logger.fine(
        '[UDXSessionConn $id] STATE_CHANGE: open -> closing, reason: normal_close');
    _connManager.updateState(this, ConnectionState.closing, error: null);

    // Phase 1.3: Resource cleanup order logging
    _logger.fine(
        '[UDXSessionConn $id] CLEANUP_ORDER: 1. Cancelling socket and initial stream subscriptions');
    ...
    _logger.fine(
        '[UDXSessionConn $id] CLEANUP_ORDER: 2. Closing incoming streams controller');
    if (!_incomingStreamsController.isClosed) {
      await _incomingStreamsController.close();
    }

    _logger.fine(
        '[UDXSessionConn $id] CLEANUP_ORDER: 3. Closing all active streams (${_activeStreams.length} streams)');
    List<Future<void>> closeFutures = [];
    _activeStreams.values.toList().forEach((adapter) {
      _logger
          .fine('[UDXSessionConn $id] Closing stream adapter ${adapter.id()}');
    ...
    try {
      ...
      _logger.fine(
          '[UDXSessionConn $id] CLEANUP_ORDER: All active streams closed in ${streamCloseDuration.inMilliseconds}ms');
      _activeStreams.clear();

      if (_isDialer) {
        _logger.fine(
            '[UDXSessionConn $id] CLEANUP_ORDER: 4. Dialer session, closing its UDPSocket');
        ...
        _logger.fine(
            '[UDXSessionConn $id] CLEANUP_ORDER: Dialer UDPSocket closed in ${socketCloseDuration.inMilliseconds}ms');
      }
    } catch (e) {
      _logger.warning(
          '[UDXSessionConn $id] Error closing UDX resources for session $id: $e');
    }
    ...
    // Phase 1.1: UDX Connection Lifecycle Analysis - State change logging
    _logger.fine(
        '[UDXSessionConn $id] STATE_CHANGE: closing -> closed, reason: cleanup_complete');
    ...
    _logger.fine(
        '[UDXSessionConn $id] CLOSE_COMPLETE: Total close duration: ${totalCloseDuration.inMilliseconds}ms');
  }
  ...
  Future<void> closeWithError(dynamic error, [StackTrace? stackTrace]) async {
    ...
    if (!_closedCompleter.isCompleted) {
      _logger.fine(
          '[UDXSessionConn $id] Completing close completer with error: $error');
  ...
  Future<Uint8List> read([int? length]) async {
    ...
    _logger.fine(
        '[UDXSessionConn $id] read() delegating to initialP2PStream.read(length: $length). Stream isClosed: ${initialP2PStream.isClosed}, Session _isClosed: $_isClosed, _isClosing: $_isClosing');

    // Fail fast if session is closing/closed
    if (_isClosed || _isClosing) {
      _logger.warning(
          '[UDXSessionConn $id] read() called on closing/closed session. Returning empty (EOF).');
      return Uint8List(0);
    }
    ...
    if (result.isEmpty) {
      _logger.warning(
          '[UDXSessionConn $id] read() returned EMPTY DATA (EOF signal). Stream isClosed: ${initialP2PStream.isClosed}');
    }
    return result;
  }
  ...
  Future<void> write(Uint8List data) async {
    ...
    _logger.fine(
        '[UDXSessionConn $id] write() delegating to initialP2PStream.write() data length: ${data.length}. Session _isClosed: $_isClosed, _isClosing: $_isClosing');

    // Fail fast if session is closing/closed to prevent hanging on writes to closed streams
    if (_isClosed || _isClosing) {
      _logger.warning(
          '[UDXSessionConn $id] write() called on closing/closed session. Throwing StateError.');
