...
class SecuredConnection implements TransportConn {
  ...
  int _sendNonce;
  int _recvNonce;
  final _DecryptedDataBuffer _decryptedBuffer =
      _DecryptedDataBuffer(); // Optimized buffer
  ...
  Future<Uint8List> read([int? length]) async {
    ...
    try {
      _log.finer(
          'SecuredConnection.read: Called with length: $length, _decryptedBuffer.length: ${_decryptedBuffer.length}'); // ADDED
      if (length == 0) {
        _log.finer(
            'SecuredConnection.read: Requested length is 0, returning empty Uint8List.'); // ADDED
      ...
      if (length == null) {
        if (!_decryptedBuffer.isEmpty) {
          ...
          _log.finer(
              'SecuredConnection.read (length=null): Returning buffered data of length ${data.length}. Data preview: ${data.take(20).toList()}'); // ADDED
          return data;
        }
        final decryptedMessage = await _readAndDecryptMessage();
        _log.finer(
            'SecuredConnection.read (length=null): Returning directly from _readAndDecryptMessage, length ${decryptedMessage.length}. Data preview: ${decryptedMessage.take(20).toList()}'); // ADDED
      ...
      while (_decryptedBuffer.length < length) {
        ...
        try {
          _log.finer(
              'SecuredConnection.read (length=$length): _decryptedBuffer.length (${_decryptedBuffer.length}) < requested ($length). Calling _readAndDecryptMessage().'); // ADDED
          decryptedChunk = await _readAndDecryptMessage();
        } catch (e) {
          _log.finer(
              'SecuredConnection.read (length=$length): Error in _readAndDecryptMessage: $e. Current _decryptedBuffer.length: ${_decryptedBuffer.length}'); // ADDED
        ...
        if (decryptedChunk.isEmpty) {
          // EOF from _readAndDecryptMessage
          _log.finer(
              'SecuredConnection.read (length=$length): _readAndDecryptMessage returned empty (EOF). Breaking loop.'); // ADDED
          break;
        }
        ...
        _log.finer(
            'SecuredConnection.read (length=$length): Added ${decryptedChunk.length} bytes to _decryptedBuffer. New _decryptedBuffer.length: ${_decryptedBuffer.length}'); // ADDED
      }

      if (_decryptedBuffer.isEmpty) {
        _log.finer(
            'SecuredConnection.read (length=$length): _decryptedBuffer is empty after loop. Returning empty Uint8List.'); // ADDED
        return Uint8List(0);
      }
      ...
      if (_decryptedBuffer.length >= length) {
        ...
        _log.finer(
            'SecuredConnection.read (length=$length): Returning ${dataToReturn.length} bytes. Remaining _decryptedBuffer.length: ${_decryptedBuffer.length}. Data preview: ${dataToReturn.take(20).toList()}'); // ADDED
        return dataToReturn;
      } else {
        // Less data than requested due to EOF
        final dataToReturn = _decryptedBuffer.takeAll();
        _log.finer(
            'SecuredConnection.read (length=$length): Returning ${dataToReturn.length} bytes (less than requested due to EOF). _decryptedBuffer is now empty. Data preview: ${dataToReturn.take(20).toList()}'); // ADDED
        return dataToReturn;
      }
    } finally {
  ...
  Future<Uint8List> _readFullMessage(int expectedLength,
    ...
    while (buffer.length < expectedLength) {
      ...
      if (DateTime.now().difference(startTime) > timeout) {
        _log.warning(
            'SecuredConnection: Timeout reading message after ${readAttempts} attempts. Expected: $expectedLength, Got: ${buffer.length}');
        throw TimeoutException('Timeout reading message', timeout);
      }

      final remaining = expectedLength - buffer.length;
      _log.fine(
          'SecuredConnection: _readFullMessage attempt $readAttempts: need $remaining more bytes (have ${buffer.length}/$expectedLength)');

      final chunk = await _connection.read(remaining);

      if (chunk.isEmpty) {
        _log.fine(
            'SecuredConnection: _readFullMessage got EOF after $readAttempts attempts. Expected: $expectedLength, Got: ${buffer.length}');
        break; // EOF - return what we have
      }

      _log.fine(
          'SecuredConnection: _readFullMessage attempt $readAttempts: received ${chunk.length} bytes. First 4 bytes: ${chunk.take(4).toList()}');
    ...
    _log.finer(
        'SecuredConnection: _readFullMessage completed after $readAttempts attempts. Expected: $expectedLength, Got: ${buffer.length}');

    // Sanity check: if we got more bytes than expected, truncate and warn
    if (buffer.length > expectedLength) {
      _log.warning(
          'SecuredConnection: _readFullMessage got ${buffer.length} bytes but only expected $expectedLength. This should not happen - check UDXP2PStreamAdapter.read()');
  ...
  Future<Uint8List> _readAndDecryptMessage() async {
    ...
    _log.finer(
        'SecuredConnection: Reading combined data of length $dataLength');
    final combinedData = await _readFullMessage(dataLength);
    _log.finer(
        'SecuredConnection: FROM_UNDERLYING_READ (Message Body) - Length: ${combinedData.length}, Bytes: ${hex.encode(combinedData)}');

    if (combinedData.length < dataLength) {
      _log.finer(
          'SecuredConnection: Connection closed while reading message body. Expected: $dataLength, Got: ${combinedData.length}');
      throw StateError(
          'Connection closed while reading message body. Expected: $dataLength, Got: ${combinedData.length}');
    }
    _log.finer(
        'SecuredConnection: Got combined data of length ${combinedData.length}');

    final encrypted = combinedData.sublist(0, dataLength - 16);
    final mac = combinedData.sublist(dataLength - 16);
    _log.finer(
        'SecuredConnection: Split into encrypted(${encrypted.length}) and MAC(${mac.length})');
    _log.finer(
        'SecuredConnection:   Raw Received Ciphertext: ${hex.encode(encrypted)}');
    _log.finer('SecuredConnection:   Raw Received MAC: ${hex.encode(mac)}');
    _log.finer(
        'SecuredConnection: First 4 bytes of encrypted: ${encrypted.take(4).toList()}');
    _log.finer(
        'SecuredConnection: First 4 bytes of MAC: ${mac.take(4).toList()}');
    ...
    try {
      ...
      _log.severe(
          '  - Current _recvNonce counter: $_recvNonce (after increment)');
      rethrow;
    }
  }
  ...
  Future<void> _writeFrame(Uint8List plaintext) async {
    ...
    _log.finer(
        'SecuredConnection: Writing frame: $dataLength bytes (plaintext was ${plaintext.length})');
    ...
    _log.finer(
        'SecuredConnection: TO_UNDERLYING_WRITE - Length: ${combinedData.length}');
  ...
  Socket get socket => throw UnimplementedError(
      'Socket access not supported in SecuredConnection');
  ...
  Future<PublicKey?> get remotePublicKey async =>
      establishedRemotePublicKey ?? await _connection.remotePublicKey;
