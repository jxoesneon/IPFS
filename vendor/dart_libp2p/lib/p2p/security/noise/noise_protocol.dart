...
import 'package:cryptography/cryptography.dart'
    as crypto; // Renamed to avoid conflict
import 'package:meta/meta.dart';

import '../../../core/crypto/keys.dart' as keys;
import '../../../core/crypto/ed25519.dart'
    as ed25519_keys; // Added for Ed25519PublicKey
...
class NoiseSecurity implements SecurityProtocol {
  static const String protocolIdForState =
      '/noise'; // Made public for SecuredConnection
  ...
  Future<Uint8List> _readHandshakeMessage(TransportConn conn) async {
    ...
    if (lengthBytes.length < 2)
      throw NoiseProtocolException(
          'Failed to read handshake message length prefix');
    final length = (lengthBytes[0] << 8) | lengthBytes[1];
    if (length == 0) return Uint8List(0);
    final data = await conn.read(length);
    if (data.length < length) {
      throw NoiseProtocolException(
          'Short read on handshake message: expected $length, got ${data.length}');
    }
    return data;
  }
  ...
  Future<PeerId> _verifyHandshakePayload(
    ...
    if (!payload.hasIdentityKey())
      throw NoiseProtocolException('Remote payload missing identity key');
    if (!payload.hasIdentitySig())
      throw NoiseProtocolException('Remote payload missing signature');
    ...
    if (!sigVerified)
      throw NoiseProtocolException('Failed to verify remote signature');
  ...
  Future<SecuredConnection> secureOutbound(TransportConn connection) async {
    ...
    try {
      ...
      _log.info(
          'secureOutbound: Writing msg3 (s,se + payload): ${msg3.length} bytes');
      await _writeHandshakeMessage(connection, msg3);

      _log.info(
          'secureOutbound: Handshake complete. Remote peer: ${remotePeerId.toBase58()}');

      // Session keys are now derived. Nonces start at 0 (no post-handshake exchange).
      final remoteLibp2pPublicKey = ed25519_keys.Ed25519PublicKey.unmarshal(
          Uint8List.fromList(
              noise_pb.NoiseHandshakePayload.fromBuffer(msg2Payload)
                  .identityKey));
  ...
  Future<SecuredConnection> secureInbound(TransportConn connection) async {
    ...
    try {
      ...
      _log.info(
          'secureInbound: Writing msg2 (e,ee,s,es + payload): ${msg2.length} bytes');
      ...
      final remoteLibp2pPublicKey = ed25519_keys.Ed25519PublicKey.unmarshal(
          Uint8List.fromList(
              noise_pb.NoiseHandshakePayload.fromBuffer(msg3Payload)
                  .identityKey));
