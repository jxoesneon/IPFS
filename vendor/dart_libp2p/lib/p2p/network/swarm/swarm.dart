...
class Swarm implements Network {
  ...
  final Set<String> _relayConnectedPeers = {};
  ...
  Future<P2PStream> newStream(Context context, PeerId peerId) async {
    _logger.warning(
        'Swarm.newStream: Entered for peer ${peerId.toString()}. Context HashCode: ${context.hashCode}');
    // Check if we're closed
    if (_isClosed) {
      _logger.warning(
          'Swarm.newStream: Swarm is closed for peer ${peerId.toString()}. Throwing exception.');
      throw Exception('Swarm is closed');
    }
    _logger.warning(
        'Swarm.newStream: Swarm is open for peer ${peerId.toString()}.');

    // Get or create a connection to the peer
    _logger.warning(
        'Swarm.newStream: Calling dialPeer(context, ${peerId.toString()}).');
    final Conn conn; // Type is Conn, but runtime type should be SwarmConn
    try {
      conn = await dialPeer(context, peerId);
    } catch (e, st) {
      _logger.severe(
          'Swarm.newStream: Error from dialPeer for ${peerId.toString()}: $e\n$st');
      rethrow;
    }
    _logger.warning(
        'Swarm.newStream: Successfully dialed peer ${peerId.toString()}. Conn runtimeType: ${conn.runtimeType}, Conn ID: ${conn.id}, Conn local: ${conn.localPeer}, Conn remote: ${conn.remotePeer}');

    if (conn is! SwarmConn) {
      _logger.severe(
          'Swarm.newStream: conn from dialPeer is NOT SwarmConn. Actual type: ${conn.runtimeType}. Peer: ${peerId.toString()}');
      throw StateError(
    ...
    _logger.warning(
        'Swarm.newStream: About to call (conn as SwarmConn).newStream() for peer ${peerId.toString()} on SwarmConn ${conn.id}.');
    ...
    try {
      ...
      _logger.severe(
          'Swarm.newStream: Error from (conn as SwarmConn).newStream() for peer ${peerId.toString()}: $e\n$st');
      rethrow;
    }

    _logger.warning(
        'Swarm.newStream: Successfully called (conn as SwarmConn).newStream() for peer ${peerId.toString()}. Returned Stream ID: ${stream.id()}, Stream protocol: ${stream.protocol}');
  ...
  Future<void> listen(List<MultiAddr> addrs) async {
    _logger.fine(
        '[Swarm listen] Called with addrs: $addrs for peer ${_localPeer.toString()}'); // Changed from _localPeer.short()
    _logger.fine('Swarm.listen called with addrs: $addrs');
    // Check if we're closed
    if (_isClosed) {
      _logger.fine(
          '[Swarm listen] Swarm is closed. Throwing exception. Peer: ${_localPeer.toString()}');
      throw Exception('Swarm is closed');
    }

    for (final addr in addrs) {
      _logger.fine(
          '[Swarm listen] Processing address: $addr for peer ${_localPeer.toString()}');
      ...
      if (transport == null) {
        _logger.fine(
            '[Swarm listen] No transport found for address: $addr for peer ${_localPeer.toString()}');
      ...
      _logger.fine(
          '[Swarm listen] Attempting transport.listen() for $addr with transport ${transport.runtimeType} for peer ${_localPeer.toString()}');
      _logger.fine(
      ...
      try {
        ...
        _logger.fine(
            '[Swarm listen] Error calling transport.listen() for $addr: $e for peer ${_localPeer.toString()}');
        _logger.severe(
      ...
      _logger.fine(
          '[Swarm listen] transport.listen() successful for $addr. Listener: ${listener.runtimeType}, Listener.addr: $actualListenAddr for peer ${_localPeer.toString()}');
      _logger.fine(
      ...
      _logger.fine(
          '[Swarm listen] Added listener for $actualListenAddr. Current _listeners count: ${_listeners.length}, _listenAddrs: $_listenAddrs for peer ${_localPeer.toString()}');
      _logger.fine(
      ...
      if (!_isUnspecifiedAddress(actualListenAddr)) {
        ...
        _logger.fine(
            '[Swarm listen] Added concrete listen address to peerstore: $actualListenAddr for peer ${_localPeer.toString()}');
      } else {
        _logger.warning(
            '[Swarm listen] Skipping addition of unspecified listen address to peerstore: $actualListenAddr for peer ${_localPeer.toString()}. This should be resolved to concrete addresses by the host.');
      }
    ...
    _logger.fine(
        '[Swarm listen] listen() method finished for peer ${_localPeer.toString()}.');
  }
  ...
  void _handleIncomingConnections(Listener listener) {
    ...
    listener.connectionStream.listen((TransportConn transportConn) async {
      // FIX: For relay connections, extract peer ID and mark as upgrading
      // This prevents race condition where dialPeer() doesn't see the connection during upgrade
      String? remotePeerIdStr;
      Completer<SwarmConn>? upgradingCompleter;

      try {
        // Check if this is a relay connection with peer ID in address
        if (transportConn.remoteMultiaddr.toString().contains('/p2p-circuit')) {
          // Extract destination peer ID from circuit address
          // Format: /ip4/.../p2p/RELAY_ID/p2p-circuit/p2p/DEST_ID
          final addrStr = transportConn.remoteMultiaddr.toString();
          final parts = addrStr.split('/');
          for (int i = parts.length - 1; i >= 0; i--) {
            if (parts[i] == 'p2p' && i + 1 < parts.length) {
              remotePeerIdStr = parts[i + 1];
              break;
            }
          }
          ...
          if (remotePeerIdStr != null) {
            ...
            _logger.info(
                'Swarm: Marked peer $peerIdStr as upgrading (relay connection)');
          }
        }
      } catch (e) {
        _logger
            .warning('Swarm: Error extracting peer ID from relay address: $e');
      }
      ...
      try {
        ...
        if (upgradedConn is UpgradedConnectionImpl &&
            upgradedConn.context != null) {
        ...
        _logger.fine(
            'Storing inbound connection for peer=$actualPeerIdStr, conn_id=${swarmConn.id}');

        // For relayed connections, deduplicate older connections to same peer via same relay
        if (_isRelayedConnection(swarmConn)) {
          final relayId =
              _extractRelayFromCircuitAddr(swarmConn.remoteMultiaddr);
          if (relayId != null) {
            await _connLock.synchronized(() async {
              ...
              final staleRelayConns = existingConns.where((existing) {
                ...
                final existingRelayId =
                    _extractRelayFromCircuitAddr(existing.remoteMultiaddr);
                return existingRelayId == relayId &&
                    existing.id != swarmConn.id;
              }).toList();

              // Close stale connections through the same relay
              for (final stale in staleRelayConns) {
                _logger.info(
                    'Swarm: Closing stale relay connection ${stale.id} in favor of new connection ${swarmConn.id}');
        ...
        await _connLock.synchronized(() {
          if (!_connections.containsKey(actualPeerIdStr)) {
            _connections[actualPeerIdStr] = [];
          }
          _connections[actualPeerIdStr]!.add(swarmConn);
          _logger.fine(
              'Connection stored. Total connections for "$actualPeerIdStr": ${_connections[actualPeerIdStr]!.length}');
          ...
          _connectionHealthStates[actualPeerIdStr] =
          ...
          if (completer != null && !completer.isCompleted) {
            completer.complete(swarmConn);
            _logger.info('Swarm: Completed upgrade for peer $actualPeerIdStr');
          }
        });
    ...
    Future.microtask(() async {
      try {
        while (!conn.isClosed) {
          ...
          final core_mux.MuxedStream acceptedStreamBase =
          ...
          if (conn.streamHandler != null) {
            ...
            _logger.fine(
                'Accepted inbound stream ${acceptedP2PStream.id()} from ${conn.remotePeer} on conn ${conn.id}');
  ...
  Future<Conn> dialPeer(Context context, PeerId peerId) async {
    _logger.fine(
        'Swarm.dialPeer: peer=${peerId.toBase58()}, existing_conns=${_connections.length}');
    ...
    Completer<SwarmConn>? upgradingCompleter;
    await _connLock.synchronized(() {
      upgradingCompleter = _upgradingConnections[peerIDStr];
    });

    if (upgradingCompleter != null) {
      _logger.info(
          'Swarm.dialPeer: Connection to $peerIDStr is being upgraded, waiting...');
      try {
        final swarmConn = await upgradingCompleter!.future.timeout(
          Duration(seconds: 10),
          onTimeout: () {
            _logger.warning(
                'Swarm.dialPeer: Timeout waiting for upgrade to complete for $peerIDStr');
            throw TimeoutException('Upgrade timeout');
          },
        );
        _logger.info(
            'Swarm.dialPeer: Upgrade complete for $peerIDStr, returning connection ${swarmConn.id}');
        return swarmConn;
      } catch (e) {
        _logger.warning(
            'Swarm.dialPeer: Error waiting for upgrade: $e, proceeding with normal dial');
        // Fall through to normal dial if upgrade fails
      }
    }
    ...
    if (existingConns.isNotEmpty) {
      _logger.fine(
          'Swarm.dialPeer: Found ${existingConns.length} existing connection(s) for peer ${peerId.toString()}. Validating health...');
      ...
      if (staleConns.isNotEmpty) {
        _logger.warning(
            'Swarm.dialPeer: Processing ${staleConns.length} stale connection(s) for peer ${peerId.toString()}');
        for (final staleConn in staleConns) {
          // For protected connections, validate by trying to create a test stream
          if (_host?.connManager.isProtected(staleConn.remotePeer, '') ??
              false) {
            _logger.info(
                'Swarm.dialPeer: Protected connection ${staleConn.id} appears stale, validating...');
            try {
              // Try to create a new stream - if connection is alive, this will succeed
              final testStream = await staleConn
                  .newStream(Context())
                  .timeout(const Duration(seconds: 5));
              // Connection is alive - close test stream and keep the connection
              await testStream.reset();
              _logger.info(
                  'Swarm.dialPeer: Protected connection ${staleConn.id} validated, keeping');
              healthyConns.add(staleConn); // Move back to healthy list
              continue; // Skip cleanup for this connection
            } catch (e) {
              _logger.warning(
                  'Swarm.dialPeer: Protected connection ${staleConn.id} failed validation: $e');
              // Continue to cleanup
            }
          }
      ...
      if (healthyConns.isNotEmpty) {
        ...
        _logger.warning(
            'Swarm.dialPeer: Found healthy connection for peer ${peerId.toString()}. Returning newest connection ID: ${healthyConns.last.id}');
        return healthyConns.last;
      } else {
        _logger.warning(
            'Swarm.dialPeer: No healthy connections found for peer ${peerId.toString()}. Will create new connection.');
      }
    }
    _logger.fine(
        'Swarm.dialPeer: No existing connection for peer ${peerId.toString()}. Dialing.');
    ...
    if (_host is BasicHost) {
      ...
      capability = OutboundCapabilityInfo(
          hasIPv4: true, hasIPv6: false, detectedAt: DateTime.now());
    }
          ...
          if (prevProtocol.code == Protocols.p2p.code) {
            try {
              ...
              if (relayPeerId == _localPeer) {
                _logger.fine(
                    'Swarm.dialPeer: Filtering out circuit address that routes through self: $addr');
                return false;
              }
            } catch (e) {
    ...
    _logger.fine(
        'Swarm.dialPeer: After circuit dedup: ${dialableAddrs.length} addresses');
    ...
    // If this peer connected to us via relay, their direct addresses are likely unreachable
    final isRelayPeer = await _connLock.synchronized(() {
      return _relayConnectedPeers.contains(peerIDStr);
    });

    if (isRelayPeer) {
      // Reorder: put circuit/relay addresses first
      final relayAddrs = scoredAddrs
          .where((addr) => addr.toString().contains('/p2p-circuit'))
          .toList();
      final nonRelayAddrs = scoredAddrs
          .where((addr) => !addr.toString().contains('/p2p-circuit'))
          .toList();

      if (relayAddrs.isNotEmpty) {
        scoredAddrs = [...relayAddrs, ...nonRelayAddrs];
        _logger.info(
            'Swarm.dialPeer: Peer $peerIDStr connected via relay, prioritizing ${relayAddrs.length} relay addresses');
      }
    }

    _logger.fine('Dialing $peerId with ${scoredAddrs.length} addresses '
        '(capability: ${capability.capability}, relay-peer: $isRelayPeer)');
    ...
    try {
      ...
      _logger.info(
          'Swarm.dialPeer: Dial latency for $peerId: ${dialLatency.inMilliseconds}ms');
      ...
      if (_isCircuitAddr(addr)) {
        ...
        if (route != null) {
          if (seenRelayRoutes.contains(route)) {
            _logger.fine(
                'Swarm: Skipping duplicate circuit route: $addr (route: $route)');
  ...
  void markConnectionUnhealthy(PeerId peer) {
    ...
    _logger
        .warning('Swarm: Marked connection to ${peer.toBase58()} as unhealthy');
  }

  /// Enhanced connection health check using event-driven state
  bool _isConnectionHealthy(SwarmConn conn) {
    try {
      // Fix 1.3: Grace period - don't aggressively health-check brand new connections
      if (_isRelayedConnection(conn)) {
        final createdAt = _connectionCreatedAt[conn.id];
        if (createdAt != null) {
          final connAge = DateTime.now().difference(createdAt);
          if (connAge < Duration(seconds: 60)) {
            return true; // Trust new connections for first 60 seconds
          }
        }
      }
      ...
      if (_isRelayedConnection(conn)) {
        ...
        if (lastActivity != null) {
          ...
          if (idleTime > Duration(seconds: 60)) {
            _logger.fine(
                'Swarm: Relayed connection ${conn.id} idle for ${idleTime.inSeconds}s');
  ...
  /// Probes a relayed connection to verify liveness
  ///
  /// Fix 2.3: Prefers UDX's native PING frame (1 byte, non-intrusive) over
  ...
  Future<void> _probeRelayedConnection(SwarmConn conn) async {
    ...
    try {
      // Try UDX native PING if available (1 byte, non-intrusive)
      // Check if the underlying connection supports ping (duck typing approach)
      // This works because SwarmConn.conn could be a UDXSessionConn which has ping()
      try {
        // Use dynamic call to ping() to avoid circular dependency
        final dynamic underlying = conn.conn;
        if (underlying.runtimeType.toString().contains('UDXSessionConn')) {
          probeSuccess = await underlying.ping(timeout: Duration(seconds: 5));
          _logger.fine(
              'Swarm: Connection ${conn.id} UDX ping: ${probeSuccess ? "success" : "failed"}');
        }
      } catch (e) {
        // ping() not available or failed, will fall back to test stream
        _logger.fine('Swarm: UDX ping not available: $e');
      }

      // Fallback to test stream for non-UDX connections or if ping not supported
      if (!probeSuccess) {
        final testStream =
            await conn.newStream(Context()).timeout(Duration(seconds: 5));
        await testStream.reset();
        probeSuccess = true;
        _logger
            .fine('Swarm: Connection ${conn.id} test stream probe successful');
      }
      ...
      if (failures >= 3) {
        _logger.warning(
            'Swarm: Connection ${conn.id} probe failed $failures times: $e');
        _connectionHealthStates[peerStr] = ConnectionHealthState.failed;
      } else {
        _logger.fine(
            'Swarm: Connection ${conn.id} probe failed ($failures/3): $e');
  ...
  Future<void> _cleanupStaleConnections() async {
    ...
    if (staleConnections.isNotEmpty) {
      _logger.info(
          'Swarm._cleanupStaleConnections: Processing ${staleConnections.length} stale connections (${healthyConnections}/${totalConnections} healthy)');

      for (final staleConn in staleConnections) {
        // For protected connections, validate by trying to create a test stream
        if (_host?.connManager.isProtected(staleConn.remotePeer, '') ?? false) {
          _logger.info(
              'Swarm._cleanupStaleConnections: Protected connection ${staleConn.id} appears stale, validating...');
          try {
            // Try to create a new stream - if connection is alive, this will succeed
            final testStream = await staleConn
                .newStream(Context())
            ...
            await testStream.reset();
            _logger.info(
                'Swarm._cleanupStaleConnections: Protected connection ${staleConn.id} validated, keeping');
            continue; // Skip cleanup - connection is actually healthy
          } catch (e) {
            _logger.warning(
                'Swarm._cleanupStaleConnections: Protected connection ${staleConn.id} failed validation: $e');
            // Continue to cleanup
          }
        }
