...
class ConnectionHealthMetrics {
  ...
  void recordError(dynamic error) {
    ...
    _logger.warning(
        'Connection error recorded: $error (consecutive: $_consecutiveErrors)');
...
class ConnectionHealthMonitor {
  ...
  void monitorConnection(dynamic connection) {
    ...
    try {
      _logger
          .fine('Setting up transport-agnostic connection health monitoring');
  ...
  void _monitorConnectionClosure(dynamic connection) {
    try {
      ...
      if (connection.onClose != null) {
        _subscriptions.add(connection.onClose.asStream().listen((_) {
  ...
  void _recordStreamError(dynamic error) {
    ...
    if (_streamErrorCount >= 3) {
      _logger.warning(
          'Multiple stream errors detected ($_streamErrorCount), treating as connection degradation');
  ...
  bool _isConnectionLevelError(dynamic error) {
    ...
    if (error.runtimeType.toString().contains('UDXConnectionException'))
      return true;
    if (error.runtimeType.toString().contains('UDXPacketLossException'))
      return true;
  ...
  bool _isStreamLevelError(dynamic error) {
    ...
    if (error.runtimeType.toString().contains('UDXStreamException'))
      return true;
