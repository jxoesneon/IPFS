...
class MessageTooLargeException implements Exception {
  ...
  const MessageTooLargeException(
      [this.message = 'Incoming message was too large']);
...
class IncorrectVersionException implements Exception {
  ...
  const IncorrectVersionException(
      [this.message = 'Client connected with incorrect version']);
...
class MultistreamMuxer implements ProtocolSwitch {
  ...
  @override
  Future<void> addHandlerWithFunc(ProtocolID protocol,
      bool Function(ProtocolID) match, HandlerFunc handler) async {
  ...
  Future<(ProtocolID, HandlerFunc)> negotiate(P2PStream<dynamic> stream) async {
    try {
      ...
      _log.fine(
          "[multistreamMuxer - negotiate] Waiting to read initiator's protocol ID.");
      final initiatorProtoID = await _readNextToken(stream);
      _log.fine(
          "[multistreamMuxer - negotiate] Read initiator's protocol ID: $initiatorProtoID");
      if (initiatorProtoID != protocolID) {
        _log.warning(
            "[multistreamMuxer - negotiate] Initiator sent wrong protocol ID: $initiatorProtoID. Expected: $protocolID");
        await stream.reset();
        throw IncorrectVersionException(
            'Initiator sent wrong protocol ID: $initiatorProtoID');
      }

      // 2. Send our multistream protocol ID back
      _log.fine(
          "[multistreamMuxer - negotiate] Sending our protocol ID: $protocolID");
      ...
      while (true) {
        _log.fine(
            '[multistreamMuxer - negotiate] Waiting to read next protocol offer from initiator.');
        final tok = await _readNextToken(stream);
        _log.fine(
            '[multistreamMuxer - negotiate] Received protocol offer from initiator: "$tok"');

        // Find a handler for this protocol
        _log.fine(
            '[multistreamMuxer - negotiate] Finding handler for token: "$tok"');
        final h = await _findHandler(tok);
        if (h == null) {
          // No handler found, send "na" (not available)
          _log.warning(
              '[multistreamMuxer - negotiate] No handler for "$tok". Sending "na".');
          await _writeDelimited(stream, utf8.encode('na'));
          _log.fine(
              '[multistreamMuxer - negotiate] Sent "na" for "$tok". Continuing loop.');
          continue;
        }

        // Handler found, send the protocol name back
        _log.fine(
            '[multistreamMuxer - negotiate] Handler found for "$tok". Sending acknowledgment: "$tok".');
        await _writeDelimited(stream, utf8.encode(tok));
        _log.fine(
            '[multistreamMuxer - negotiate] Sent acknowledgment for "$tok".');

        // Return the protocol and handler
        _log.fine(
            '[multistreamMuxer - negotiate] Returning protocol "$tok" and its handler.');
        return (tok, h.handle);
      }
    } catch (e) {
      _log.severe(
          '[multistreamMuxer - negotiate for ${protocolID}] Error during negotiation: $e');
      await stream.reset();
      rethrow;
    }
  }
  ...
  Future<void> handle(P2PStream<dynamic> stream) async {
    ...
    if (stream.isClosed) {
      _log.warning(
          '[multistreamMuxer - handle] Stream for protocol $proto was closed during or immediately after negotiation. Aborting handler call.');
      return;
    }

    try {
      _log.fine(
          '[multistreamMuxer - handle] Protocol $proto negotiated. Attempting to set protocol on stream scope and stream itself.');
      await stream.scope().setProtocol(proto);
      await stream.setProtocol(proto);
      _log.fine(
          '[multistreamMuxer - handle] Successfully set protocol $proto on stream scope and stream. Proceeding to call handler.');
    } catch (e, s) {
      _log.severe(
          '[multistreamMuxer - handle] CRITICAL: Error occurred while setting protocol $proto on stream scope/stream: $e\n$s. Resetting stream and not calling handler.');
      await stream.reset();
      rethrow;
    }
  ...
  void _injectLeftover(P2PStream<dynamic> stream) {
    ...
    _log.fine(
        '[multistreamMuxer] Injecting ${leftover.length} leftover bytes back into stream $streamId');
    ...
    if (target is YamuxStream) {
      ...
      _log.warning(
          '[multistreamMuxer] Cannot inject leftover bytes: stream type ${target.runtimeType} does not support pushData');
    }
  }
  ...
  Future<ProtocolID?> selectOneOf(
      P2PStream<dynamic> stream, List<ProtocolID> protocolsToSelect) async {
  ...
  Future<void> _writeDelimited(
      P2PStream<dynamic> stream, List<int> message) async {
  ...
  Future<Uint8List> _performReadDelimited(P2PStream<dynamic> stream) async {
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        retryCount++;
        _log.warning(
            '[multistream] Read timeout (attempt $retryCount/${maxRetries + 1}): ${e.message}');
  ...
  Future<Uint8List> _performSingleReadDelimited(
    ...
    return await _actualReadDelimited(stream).timeout(
      ...
      onTimeout: () => throw TimeoutException(
          'Multistream read operation timed out', readTimeout),
    );
  }
  ...
  Future<Uint8List> _actualReadDelimited(P2PStream<dynamic> stream) async {
    ...
    while (true) {
      ...
      if (!lengthDecoded && accumulated.isNotEmpty) {
        try {
          final (decodedLength, consumed) =
              MultiAddrCodec.decodeVarint(accumulated);
          if (consumed > 0) {
            length = decodedLength;
            lengthDecoded = true;
            varintBytesRead = consumed;
            if (length > 1024) {
              await YamuxExceptionUtils.safeStreamReset(stream,
      ...
      if (lengthDecoded && accumulated.length >= length + varintBytesRead) {
        ...
        if (accumulated.length > totalMessageSize) {
          _leftoverMap[streamId] =
              Uint8List.fromList(accumulated.sublist(totalMessageSize));
        }
  ...
  Future<T> _safeStreamOperation<T>(
    ...
    try {
      ...
      _log.warning(
          '[multistream] Yamux exception during $operationName: ${e.message}');
      ...
      _log.warning(
          '[multistream] StateError during $operationName: ${e.message}');
      ...
      _log.warning(
          '[multistream] Format error during $operationName: ${e.message}');
      await YamuxExceptionUtils.safeStreamReset(stream, context: operationName);
      rethrow;
    } catch (e, stackTrace) {
      _log.severe(
          '[multistream] Unexpected error during $operationName: $e\n$stackTrace');
      await YamuxExceptionUtils.safeStreamReset(stream, context: operationName);
