...
class HttpRequest {
  ...
  Uint8List serialize() {
    ...
    if (body != null && !headers.containsKey('content-length')) {
      buffer.write(
          'content-length: ${body!.length}${HttpProtocolConstants.crlf}');
    }
  ...
  static HttpRequest parse(Uint8List data, PeerId remotePeer) {
    ...
    final headerEndIndex =
        dataString.indexOf(HttpProtocolConstants.headerSeparator);
    ...
    Uint8List? body;
    final bodyStartIndex =
        headerEndIndex + HttpProtocolConstants.headerSeparator.length;
...
class HttpResponse {
  ...
  factory HttpResponse.json(Map<String, dynamic> data,
  ...
  Uint8List serialize() {
    ...
    // Status line
    buffer.write(
        '$version ${status.code} ${status.message}${HttpProtocolConstants.crlf}');
...
class HttpRoute {
  ...
  Map<String, String> extractParams(String path) {
    ...
    for (int i = 0;
        i < paramNames.length && i + 1 < match.groupCount + 1;
        i++) {
...
class HttpProtocolService {
  ...
  void put(String path, HttpRequestHandler handler) =>
  ...
  Future<void> _handleRequest(P2PStream stream, PeerId peerId) async {
    ...
    _logger.info(
        '🎯 [HTTP-SERVER-START] HTTP request handler started for peer ${peerId.toString()}. Stream ID: ${stream.id}');

    try {
      // Phase 1: Stream Setup
      _logger.info(
          '⚙️ [HTTP-SERVER-PHASE-1] Setting up stream service and deadline');
      stream.scope().setService(HttpProtocolConstants.serviceName);
      await stream.setDeadline(
          DateTime.now().add(HttpProtocolConstants.requestTimeout));
      _logger.info(
          '✅ [HTTP-SERVER-PHASE-1] Stream setup completed with timeout: ${HttpProtocolConstants.requestTimeout.inSeconds}s');

      // Phase 2: Read Request Data
      _logger.info(
          '📥 [HTTP-SERVER-PHASE-2] Reading HTTP request data from stream');
      ...
      _logger.info(
          '✅ [HTTP-SERVER-PHASE-2] Request data read in ${readTime.inMilliseconds}ms. Size: ${requestData.length} bytes');

      if (requestData.isEmpty) {
        _logger.warning(
            '⚠️ [HTTP-SERVER-PHASE-2] Received empty request from peer ${peerId.toString()}');
        return;
      }
      ...
      _logger.info(
          '✅ [HTTP-SERVER-PHASE-3] Request parsed in ${parseTime.inMilliseconds}ms: ${request.method.value} ${request.path}');

      // Phase 4: Route Matching
      _logger.info(
          '🔍 [HTTP-SERVER-PHASE-4] Finding route for ${request.method.value} ${request.path}');
      final route = _findRoute(request.method, request.path);
      HttpResponse response;

      if (route != null) {
        _logger.info(
            '✅ [HTTP-SERVER-PHASE-4] Route found for ${request.method.value} ${request.path}');
        ...
        try {
          ...
          if (params.isNotEmpty) {
            _logger.fine(
                '📋 [HTTP-SERVER-PHASE-5] Extracted path parameters: $params');
          }
          ...
          _logger.info(
              '✅ [HTTP-SERVER-PHASE-5] Route handler completed in ${handlerTime.inMilliseconds}ms. Status: ${response.status.code}');
        } catch (e, stackTrace) {
          _logger.severe(
              '❌ [HTTP-SERVER-PHASE-5] Route handler error: $e\n$stackTrace');
          response = HttpResponse.error(
              HttpStatus.internalServerError, 'Internal server error');
        }
      } else {
        _logger.warning(
            '❌ [HTTP-SERVER-PHASE-4] No route found for ${request.method.value} ${request.path}');
      ...
      _logger.info(
          '✅ [HTTP-SERVER-PHASE-6] Response prepared in ${prepTime.inMilliseconds}ms. Size: ${responseData.length} bytes');
      ...
      _logger.info(
          '✅ [HTTP-SERVER-PHASE-7] Response sent in ${sendTime.inMilliseconds}ms');
      _logger.info(
          '🎉 [HTTP-SERVER-COMPLETE] Total server processing time: ${totalTime.inMilliseconds}ms. Status: ${response.status.code} ${response.status.message}');
    } catch (e, stackTrace) {
      final totalTime = DateTime.now().difference(startTime);
      _logger.severe(
          '❌ [HTTP-SERVER-ERROR] Error handling HTTP request from peer ${peerId.toString()} after ${totalTime.inMilliseconds}ms: $e\n$stackTrace');

      try {
        // Send error response if possible
        _logger
            .info('🚨 [HTTP-SERVER-ERROR] Attempting to send error response');
        final errorResponse =
        ...
        _logger.severe(
            '❌ [HTTP-SERVER-ERROR] Failed to send error response: $sendError');
      }
    } finally {
      // Phase 8: Cleanup
      try {
  ...
  Future<Uint8List> _readHttpMessage(P2PStream stream) async {
    final startTime = DateTime.now();
    _logger.info(
        '📖 [HTTP-READ-START] Starting to read HTTP message from stream ${stream.id}');

    final buffer = <int>[];
    final headerSeparatorBytes =
        utf8.encode(HttpProtocolConstants.headerSeparator);
    ...
    while (!headersComplete) {
      readIterations++;
      _logger.fine(
          '📖 [HTTP-READ-HEADERS] Read iteration $readIterations - calling stream.read()');
      ...
      _logger.fine(
          '📖 [HTTP-READ-HEADERS] Read iteration $readIterations completed in ${readTime.inMilliseconds}ms. Chunk size: ${chunk.length} bytes');

      if (chunk.isEmpty) {
        _logger.warning(
            '📖 [HTTP-READ-HEADERS] Received empty chunk on iteration $readIterations, breaking header read loop');
        break;
      }

      buffer.addAll(chunk);
      _logger.fine(
          '📖 [HTTP-READ-HEADERS] Buffer size after iteration $readIterations: ${buffer.length} bytes');
      ...
      if (headerEndIndex == -1) {
        ...
        if (separatorIndex != -1) {
          ...
          _logger.info(
              '📖 [HTTP-READ-HEADERS] Found header separator at index $headerEndIndex after $readIterations iterations');

          // Parse headers to get content length
          final headerSection =
              utf8.decode(bufferBytes.sublist(0, headerEndIndex));
          final lines = headerSection.split(HttpProtocolConstants.crlf);
          ...
          for (final line in lines) {
            if (line.toLowerCase().startsWith('content-length:')) {
              ...
              _logger.info(
                  '📖 [HTTP-READ-HEADERS] Found content-length header: $contentLength bytes');
              break;
            }
          }
          ...
          if (contentLength == 0) {
            _logger.info(
                '📖 [HTTP-READ-HEADERS] No content-length header found or content-length is 0');
          }
        } else {
          _logger.fine(
              '📖 [HTTP-READ-HEADERS] Header separator not found yet, buffer size: ${buffer.length}');
        }
      }

      // Prevent excessive memory usage
      if (buffer.length > HttpProtocolConstants.maxHeaderSize &&
          !headersComplete) {
        _logger.severe(
            '📖 [HTTP-READ-HEADERS] Headers too large: ${buffer.length} bytes > ${HttpProtocolConstants.maxHeaderSize}');
    ...
    _logger.info(
        '📖 [HTTP-READ-HEADERS] Header reading completed in ${headerReadTime.inMilliseconds}ms after $readIterations iterations');

    // Read body if content length is specified
    if (contentLength > 0) {
      _logger.info(
          '📖 [HTTP-READ-BODY] Reading HTTP body ($contentLength bytes)...');
      final bodyStartIndex = headerEndIndex + headerSeparatorBytes.length;
      final currentBodyLength = buffer.length - bodyStartIndex;

      _logger.info(
          '📖 [HTTP-READ-BODY] Body start index: $bodyStartIndex, current body length: $currentBodyLength');

      if (contentLength > HttpProtocolConstants.maxBodySize) {
        _logger.severe(
            '📖 [HTTP-READ-BODY] Body too large: $contentLength bytes > ${HttpProtocolConstants.maxBodySize}');
      ...
      while (buffer.length - bodyStartIndex < contentLength) {
        ...
        _logger.fine(
            '📖 [HTTP-READ-BODY] Body read iteration $bodyReadIterations - need $remainingBytes more bytes');
        ...
        _logger.fine(
            '📖 [HTTP-READ-BODY] Body read iteration $bodyReadIterations completed in ${bodyReadTime.inMilliseconds}ms. Chunk size: ${chunk.length} bytes');

        if (chunk.isEmpty) {
          _logger.warning(
              '📖 [HTTP-READ-BODY] Received empty chunk during body read iteration $bodyReadIterations, breaking body read loop');
          break;
        }
      ...
      _logger.info(
          '📖 [HTTP-READ-BODY] Body reading completed after $bodyReadIterations iterations. Final body length: $finalBodyLength bytes');
    } else {
      _logger.info(
          '📖 [HTTP-READ-BODY] No body to read (content-length: $contentLength)');
    }

    final totalTime = DateTime.now().difference(startTime);
    final result = Uint8List.fromList(buffer);
    _logger.info(
        '📖 [HTTP-READ-COMPLETE] HTTP message reading completed in ${totalTime.inMilliseconds}ms. Total size: ${result.length} bytes');
  ...
  Future<HttpResponse> request(
    ...
    final startTime = DateTime.now();
    _logger.info(
        '🚀 [HTTP-REQUEST-START] Making HTTP request to peer ${peerId.toString()}: ${method.value} $path');

    // Phase 1: Stream Creation
    _logger.info(
        '📡 [HTTP-REQUEST-PHASE-1] Creating new stream to peer ${peerId.toString()}');
    ...
    try {
      ...
      _logger.info(
          '⚙️ [HTTP-REQUEST-PHASE-2] Setting up stream service and deadline');
      stream.scope().setService(HttpProtocolConstants.serviceName);
      ...
      _logger.info(
          '📝 [HTTP-REQUEST-PHASE-3] Creating and serializing HTTP request');
      ...
      final requestCreateTime =
          DateTime.now().difference(requestCreateStartTime);
      _logger.info(
      ...
      _logger.info(
          '📤 [HTTP-REQUEST-PHASE-4] Sending HTTP request data to stream');
      final sendStartTime = DateTime.now();
      ...
      _logger
          .info('📥 [HTTP-REQUEST-PHASE-5] Reading HTTP response from stream');
      final readStartTime = DateTime.now();
      ...
      _logger.info(
          '🎉 [HTTP-REQUEST-COMPLETE] Total request time: ${totalTime.inMilliseconds}ms. Status: ${response.status.code} ${response.status.message}');

      return response;
    } catch (e, stackTrace) {
      final totalTime = DateTime.now().difference(startTime);
      _logger.severe(
          '❌ [HTTP-REQUEST-ERROR] Request failed after ${totalTime.inMilliseconds}ms: $e\n$stackTrace');
      rethrow;
    } finally {
      try {
        ...
        _logger.warning(
            '⚠️ [HTTP-REQUEST-CLEANUP] Error closing request stream: $e');
      }
    }
  }

  /// Convenience methods for making requests
  Future<HttpResponse> getRequest(PeerId peerId, String path,
      {Map<String, String>? headers}) {
    return request(peerId, HttpMethod.get, path, headers: headers);
  }

  Future<HttpResponse> postRequest(PeerId peerId, String path,
  ...
  Future<HttpResponse> postJson(
      PeerId peerId, String path, Map<String, dynamic> data) {
