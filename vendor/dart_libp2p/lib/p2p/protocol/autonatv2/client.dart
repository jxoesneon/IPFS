...
class AutoNATv2ClientImpl implements AutoNATv2Client {
  ...
  Future<Result> getReachability(PeerId peerId, List<Request> requests) async {
    ...
    try {
      await stream
          .scope()
          .reserveMemory(maxMsgSize, ReservationPriority.always);
      // Reserve memory
    } catch (ex) {
      stream.reset();
      throw Exception(
  ...
  Future<Result> _processDialResponse(DialResponse response,
      List<Request> requests, Completer<MultiAddr> completer) async {
    ...
    if (response.status == DialResponse_ResponseStatus.E_DIAL_REFUSED) {
      ...
      return Result(
        ...
        reachability:
            Reachability.private, // Dial refused = not publicly reachable
  ...
  Future<void> _validateAndSendDialData(
      List<Request> requests, Message message, P2PStream stream) async {
  ...
  Result _createResult(
      DialResponse response, List<Request> requests, MultiAddr? dialBackAddr) {
  ...
  Future<void> _handleDialBack(P2PStream stream, PeerId peerId) async {
    ...
    try {
      await stream
          .scope()
          .reserveMemory(dialBackMaxMsgSize, ReservationPriority.always);
      // Reserve memory
    } catch (ex) {
      _log.fine(
