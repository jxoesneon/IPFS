...
const maxOwnIdentifyMsgSize = 4 *
    1024; // smaller than what we accept. This is 4k to be compatible with rust-libp2p
...
class IdentifySnapshot {
  ...
  Future<bool> equals(IdentifySnapshot other) async {
    ...
    final otherRecord =
        other.record is Future ? await other.record : other.record;
    ...
    if (hasRecord) {
      ...
      if (currentRecord is Envelope && otherRecord is Envelope) {
        ...
        recordsAreEqual = (currentRecord ==
            otherRecord); // Fallback for non-Envelope or if one is null
      }
      if (!recordsAreEqual) {
        return false;
      }
...
class IdentifyService implements IDService {
  ...
  IdentifyService(
    ...
    IdentifyOptions? options,
        ...
        disableObservedAddrManager =
            options?.disableObservedAddrManager ?? false {
  ...
  Future<void> start() async {
    ...
    _evtPeerProtocolsUpdated =
        await host.eventBus.emitter(EvtPeerProtocolsUpdated);
    _evtPeerIdentificationCompleted =
        await host.eventBus.emitter(EvtPeerIdentificationCompleted);
    _evtPeerIdentificationFailed =
        await host.eventBus.emitter(EvtPeerIdentificationFailed);
  ...
  Future<void> _startEventLoop() async {
    ...
    _eventBusSubscription!.stream.listen((event) async {
      ...
      _log.finer(
          'IdentifyService._startEventLoop: Received event: ${event.runtimeType}');
      final updated = await _updateSnapshot();
      if (!updated) {
        _log.finer(
            'IdentifyService._startEventLoop: Snapshot not updated, not triggering push.');
        return;
      }
      ...
      _log.fine(
          'IdentifyService._startEventLoop: Snapshot updated, triggering push.');
      // Trigger a push
    ...
    _triggerPushController!.stream.listen((_) {
      ...
      _log.fine(
          'IdentifyService._startEventLoop: Push trigger received, calling _sendPushes.');
      _sendPushes();
    });
  }
  ...
  Future<bool> _updateSnapshot() async {
    _log.fine(
        'IdentifyService._updateSnapshot: Called. Querying host.addrs. Host: ${host.id.toString()}, Host hashCode: ${host.hashCode}');
    final addrs = host.addrs;
    _log.finer(
        'IdentifyService._updateSnapshot: host.addrs returned: $addrs for host ${host.id.toString()}');
    ...
    final trimmedAddrs = _trimHostAddrList(
        addrs, maxOwnIdentifyMsgSize - usedSpace - 256); // 256 bytes of buffer
    ...
    return await _currentSnapshotMutex.synchronized(() async {
      ...
      if (await _currentSnapshot.equals(snapshot)) {
        _log.finer(
            'IdentifyService._updateSnapshot: New snapshot is identical to current one. No update.');
      ...
      _log.fine(
          'IdentifyService._updateSnapshot: Snapshot updated. New seq=${snapshot._seq}, addrs=${snapshot.addrs.length}, protocols=${snapshot.protocols.length}');
      return true;
    });
  }
  ...
  Future<void> _sendPush(Conn conn) async {
    ...
    try {
      ...
      if (connState.toString().contains('closing') ||
          connState.toString().contains('closed')) {
        return;
      }
    } catch (e) {
    ...
    try {
      ...
      _log.warning(
          'IdentifyService._sendPush: Unexpected error sending PUSH to $peerId (isolated): $e');
      // Note: We still don't rethrow - PUSH is non-critical and should never break other operations
    }
  }

  Future<P2PStream?> _newStreamAndNegotiate(
      Conn conn, String protoIDString) async {
    final peerId = conn.remotePeer;
    ...
    try {
      ...
      _log.fine(
          ' [IDENTIFY-STREAM-CREATE] peer=$peerId, stream_id=${stream.id()}, protocol=$protoIDString, duration=${streamCreateDuration.inMilliseconds}ms');
      ...
      if (selectedProtocol == null) {
        _log.warning(
            'Protocol negotiation failed for peer=$peerId, protocol=$protoIDString. Resetting stream.');
        await stream.reset();
        return null;
      }
      ...
      _log.fine(
          ' [IDENTIFY-STREAM-READY] peer=$peerId, stream_id=${stream.id()}, protocol=$selectedProtocol');

      return stream;
    } catch (e, st) {
      final totalDuration = DateTime.now().difference(streamCreateStart);
      _log.severe(
          '[IdentifyService] STREAM_ERROR: peer=$peerId, error=$e, duration=${totalDuration.inMilliseconds}ms, stream_state=${stream?.isClosed ?? 'null'}');

      // Enhanced error detection for the target race condition
      if (e.toString().contains('closed by remote') ||
          e.toString().contains('Stream is closed') ||
          e.toString().contains('Stream closed by remote')) {
        _log.severe(
            '[IdentifyService] PREMATURE_CLOSURE: This is the target error! Conn details: ${conn.id}, ${conn.state}, conn_age=${connAge.inMilliseconds}ms');
        _log.severe(
            '[IdentifyService] PREMATURE_CLOSURE_CONTEXT: stream_id=${stream?.id() ?? 'null'}, protocol=$protoIDString, error_detail=$e');
      }

      // Cleanup the stream if it was opened
      if (stream != null && !stream.isClosed) {
        _log.warning(
            'IdentifyService._newStreamAndNegotiate: Resetting stream due to error for ${conn.remotePeer}.');
        await stream.reset();
      }
      ...
      if (isTimeoutException(e)) {
        _log.severe(
            '[IdentifyService] TIMEOUT_DETECTED: peer=$peerId, duration=${totalDuration.inMilliseconds}ms');
      ...
      if (e.toString().contains('Bad state: Stream is closed')) {
        _log.warning(
            'IdentifyService._newStreamAndNegotiate: Handled race condition for ${conn.remotePeer} for $protoIDString: $e');
        return null;
      }

      // For other errors, wrap in typed exception
      _log.severe(
          'IdentifyService._newStreamAndNegotiate: Error opening or negotiating stream with ${conn.remotePeer} for $protoIDString: $e\n$st');
      throw IdentifyStreamException(
        peerId: peerId,
        message:
            'Error opening or negotiating stream with $peerId for $protoIDString',
        cause: e,
      );
    }
  }
  ...
  Future<void> handleIdentifyRequest(P2PStream stream, PeerId peerId) async {
    _log.fine(
        'IdentifyService.handleIdentifyRequest: SERVER received identify request from ${stream.conn.remotePeer} (reported as $peerId) on stream ${stream.id()}');
  ...
  Future<void> handlePush(P2PStream stream, PeerId peerId) async {
    _log.fine(
        'IdentifyService.handlePush: SERVER received identify PUSH from ${stream.conn.remotePeer} (reported as $peerId) on stream ${stream.id()}');
    _log.finer(
        'IdentifyService.handlePush: Setting deadline for PUSH stream from ${stream.conn.remotePeer}');
    stream.setDeadline(DateTime.now().add(timeout));
  ...
  Future<void> sendIdentifyResp(P2PStream stream, bool isPush) async {
    ...
    // #########################################################################
    _log.fine(
        'IdentifyService.sendIdentifyResp: SERVER ENTRY POINT for peer $peer. IsPush: $isPush. Stream ID: ${stream.id()}');
    // #########################################################################
    _log.fine(
        'IdentifyService.sendIdentifyResp: Preparing to send identify response to $peer. IsPush: $isPush. Stream ID: ${stream.id()}');
    try {
      _log.finer(
          'IdentifyService.sendIdentifyResp: Setting service scope for stream to $peer.');
      await stream.scope().setService(serviceName);
    } catch (e) {
      _log.warning(
          'IdentifyService.sendIdentifyResp: Failed to attach stream to identify service for $peer: $e. Resetting stream.');
      await stream.reset();
      throw Exception(
          'Failed to attach stream to identify service for $peer: $e');
    }

    try {
      _log.finer(
          'IdentifyService.sendIdentifyResp: Acquiring current snapshot for $peer.');
      final snapshot =
          await _currentSnapshotMutex.synchronized(() => _currentSnapshot);
      _log.fine(
          'IdentifyService.sendIdentifyResp: Sending snapshot to $peer: seq=${snapshot._seq}, protocols=${snapshot.protocols.length}, addrs=${snapshot.addrs.length}');

      final mes = await _createBaseIdentifyResponse(stream.conn, snapshot);
      final signedRecordBytes =
          await _getSignedRecord(snapshot); // Await the async call
      if (signedRecordBytes != null) {
        ...
        _log.finer(
            'IdentifyService.sendIdentifyResp: Added signed peer record (${mes.signedPeerRecord.length} bytes) to response for $peer.');
      } else {
        _log.warning(
            "IdentifyService.sendIdentifyResp: No Signed record was found or could be marshalled. This could cause problems.");
        // mes.signedPeerRecord will remain empty by default
      }

      _log.fine(
          'IdentifyService.sendIdentifyResp: SERVER About to write identify message to $peer on stream ${stream.id()} (remote addr: ${stream.conn.remoteMultiaddr})');
      await _writeChunkedIdentifyMsg(stream, mes);
      _log.fine(
          'IdentifyService.sendIdentifyResp: SERVER Identify message written to $peer.');

      if (metricsTracer != null) {
        metricsTracer!
            .identifySent(isPush, mes.protocols.length, mes.listenAddrs.length);
      }

      _log.finer(
          'IdentifyService.sendIdentifyResp: Updating sequence number for connection to $peer.');
      await _connsMutex.synchronized(() async {
        final e = _conns[stream.conn];
        if (e != null) {
          e.sequence = snapshot._seq;
          _log.finer(
              'IdentifyService.sendIdentifyResp: Updated sequence for $peer to ${snapshot._seq}.');
        } else {
          _log.finer(
              'IdentifyService.sendIdentifyResp: Connection to $peer not found in _conns map while trying to update sequence.');
        }
      });

      _log.fine(
          'IdentifyService.sendIdentifyResp: SERVER Signalling end of writes (calling stream.closeWrite()) on stream to $peer.');
      await stream.closeWrite(); // Signal that we are done writing.
      _log.fine(
          'IdentifyService.sendIdentifyResp: SERVER stream.closeWrite() completed for $peer. Stream will NOT be fully closed by sendIdentifyResp anymore.');
      // DO NOT CALL stream.close() here anymore. Let Yamux handle full closure based on FINs from both sides.
      _log.fine(
          'IdentifyService.sendIdentifyResp: Successfully sent identify response and signalled closeWrite to $peer. IsPush: $isPush.');
    } catch (e, st) {
      _log.warning(
  ...
  Future<Identify> _createBaseIdentifyResponse(
      Conn conn, IdentifySnapshot snapshot) async {
    _log.finer(
        'IdentifyService._createBaseIdentifyResponse: Creating base response for peer ${conn.remotePeer}. Snapshot seq: ${snapshot._seq}');
    final mes = Identify();
    ...
    _log.finer(
        'IdentifyService._createBaseIdentifyResponse: ObservedAddr set to ${remoteAddr.toString()} for ${conn.remotePeer}');

    final viaLoopback = localAddr.isLoopback() || remoteAddr.isLoopback();
    _log.finer(
        'IdentifyService._createBaseIdentifyResponse: LocalAddr: $localAddr, RemoteAddr: $remoteAddr, ViaLoopback: $viaLoopback for ${conn.remotePeer}');
    for (final addr in snapshot.addrs) {
      if (!viaLoopback && addr.isLoopback()) {
        _log.finer(
            'IdentifyService._createBaseIdentifyResponse: Skipping loopback listen address $addr for non-loopback connection to ${conn.remotePeer}');
        continue;
      }
      mes.listenAddrs.add(addr.toBytes());
    }
    _log.finer(
        'IdentifyService._createBaseIdentifyResponse: Added ${mes.listenAddrs.length} listen addresses for ${conn.remotePeer}');

    final ownKey = await host.peerStore.keyBook.pubKey(host.id);
    if (ownKey == null) {
      if (await host.peerStore.keyBook.privKey(host.id) != null) {
        _log.fine(
            'IdentifyService._createBaseIdentifyResponse: Did not have own public key in Peerstore for ${host.id}');
      } else {
        _log.fine(
            'IdentifyService._createBaseIdentifyResponse: No public or private key found for self (${host.id}) in Peerstore.');
      }
    } else {
      try {
        ...
        mes.publicKey = kb;
        _log.finer(
            'IdentifyService._createBaseIdentifyResponse: Added public key (${mes.publicKey.length} bytes) for ${host.id}');
      } catch (e) {
        _log.severe(
            'IdentifyService._createBaseIdentifyResponse: Failed to marshal own public key for ${host.id}: $e');
      }
    }
    mes.protocolVersion = protocolVersion;
    mes.agentVersion = userAgent;
    _log.finer(
        'IdentifyService._createBaseIdentifyResponse: ProtocolVersion: $protocolVersion, AgentVersion: $userAgent for ${conn.remotePeer}');
    return mes;
  }

  // Change to async and await the record
  Future<Uint8List?> _getSignedRecord(IdentifySnapshot snapshot) async {
    // Made async, returns Future<Uint8List?>
    _log.finer(
        'IdentifyService._getSignedRecord: Attempting to get signed record. DisableSignedPeerRecord: $disableSignedPeerRecord, Snapshot record type: ${snapshot.record.runtimeType}');
    if (disableSignedPeerRecord) {
      _log.finer(
          'IdentifyService._getSignedRecord: Signed peer record disabled.');
      return null;
    }

    // Await the record if it's a Future
    final actualRecord =
        snapshot.record is Future ? await snapshot.record : snapshot.record;

    if (actualRecord == null) {
      _log.finer(
          'IdentifyService._getSignedRecord: Actual record is null after await.');
      return null;
    }
    ...
    if (actualRecord is! Envelope) {
      _log.warning(
          'IdentifyService._getSignedRecord: Record is not an Envelope. Type: ${actualRecord.runtimeType}');
      return null;
    }
    ...
    try {
      ...
      _log.severe(
          'IdentifyService._getSignedRecord: Failed to marshal signed record: $e\n$st');
      return null;
    }
  }
  ...
  Future<void> _writeChunkedIdentifyMsg(P2PStream stream, Identify mes) async {
    ...
    _log.finer(
        'IdentifyService._writeChunkedIdentifyMsg: Writing to stream ${stream.id()} for peer ${stream.conn.remotePeer}. Total message size: ${msgBytes.length} bytes. SignedPeerRecord present: ${mes.signedPeerRecord.isNotEmpty}');
    if (mes.signedPeerRecord.isEmpty || msgBytes.length <= legacyIDSize) {
      _log.finer(
          'IdentifyService._writeChunkedIdentifyMsg: Sending as single delimited message (size ${msgBytes.length}).');
    ...
    _log.finer(
        'IdentifyService._writeChunkedIdentifyMsg: Message size ${msgBytes.length} > $legacyIDSize and has signed record. Sending in chunks.');
  ...
  Future<void> handleIdentifyResponse(P2PStream stream, bool isPush) async {
    ...
    try {
      ...
      _log.warning(
          ' [HANDLE-IDENTIFY-RESPONSE-PHASE-1-ERROR] ($side) Error attaching stream to identify service for peer=$peer: $e. Resetting stream.');
      await stream.reset();
      throw e;
    }
    ...
    try {
      await stream
          .scope()
          .reserveMemory(signedIDSize, ReservationPriority.always);
      final memoryReserveTime = DateTime.now().difference(handleStart);
    } catch (e) {
      _log.warning(
          ' [HANDLE-IDENTIFY-RESPONSE-PHASE-2-ERROR] ($side) Error reserving memory for identify stream for peer=$peer: $e. Resetting stream.');
      await stream.reset();
      throw e;
    }
    ...
    try {
      ...
      if (metricsTracer != null) {
        metricsTracer!.identifyReceived(
            isPush, mes.protocols.length, mes.listenAddrs.length);
      }
      ...
      await _connsMutex.synchronized(() async {
        ...
        if (e == null) {
          _log.fine(
              'IdentifyService.handleIdentifyResponse ($side): Connection entry for $peer already removed (disconnected). Cannot update push support.');
          return;
        }
        _log.finer(
            'IdentifyService.handleIdentifyResponse ($side): Checking push support for $peer in peerstore.');
        final sup = await host.peerStore.protoBook
            .supportsProtocols(conn.remotePeer, [idPush]);
        if (sup.isNotEmpty) {
          e.pushSupport = IdentifyPushSupport.supported;
          _log.fine(
              'IdentifyService.handleIdentifyResponse ($side): Peer $peer supports push.');
        } else {
          e.pushSupport = IdentifyPushSupport.unsupported;
          _log.fine(
              'IdentifyService.handleIdentifyResponse ($side): Peer $peer does not support push.');
        }
        if (metricsTracer != null) {
      ...
      _log.severe(
          ' [HANDLE-IDENTIFY-RESPONSE-ERROR] ($side) Error reading or processing identify message from peer=$peer, duration=${errorTime.inMilliseconds}ms, error=$e\n$st');
      await stream.reset();
      rethrow;
    } finally {
  ...
  Future<Identify> _readAllIDMessages(P2PStream stream) async {
    ...
    if (msgBytes == null || msgBytes.isEmpty) {
      _log.severe(
          ' [READ-ALL-ID-MESSAGES] No identify message received from peer=$peer, duration=${totalReadTime.inMilliseconds}ms');
      return Identify();
    }

    _log.fine(
        ' [READ-ALL-ID-MESSAGES] Parsed identify from peer=$peer, ${msgBytes.length} bytes, duration=${totalReadTime.inMilliseconds}ms');
  ...
  /// Returns null on EOF.
  Future<Uint8List?> _readDelimitedMsg(
      P2PStream stream, BytesBuilder buffer) async {
    while (true) {
  ...
  void _mergeIdentify(Identify target, Identify source) {
    _log.finer(
        'IdentifyService._mergeIdentify: Merging source into target. Target (before): P:${target.protocols.length},LA:${target.listenAddrs.length},SR:${target.signedPeerRecord.isNotEmpty}. Source: P:${source.protocols.length},LA:${source.listenAddrs.length},SR:${source.signedPeerRecord.isNotEmpty}');
    target.protocols.addAll(source.protocols);
    target.listenAddrs.addAll(source.listenAddrs);
    if (source.hasObservedAddr() && source.observedAddr.isNotEmpty) {
      // Check if not empty
      target.observedAddr = source.observedAddr;
    }
    if (source.hasPublicKey() && source.publicKey.isNotEmpty) {
      // Check if not empty
    ...
    if (source.hasSignedPeerRecord() && source.signedPeerRecord.isNotEmpty) {
      // Check if not empty
      target.signedPeerRecord = source.signedPeerRecord;
    }
    _log.finer(
        'IdentifyService._mergeIdentify: Target (after): P:${target.protocols.length},LA:${target.listenAddrs.length},SR:${target.signedPeerRecord.isNotEmpty}');
  }

  Future<void> _consumeMessage(Identify mes, Conn conn, bool isPush) async {
    final p = conn.remotePeer;
    _log.fine(
        'IdentifyService._consumeMessage: Consuming identify message from $p. IsPush: $isPush.');

    final supported = await host.peerStore.protoBook.getProtocols(p);
    _log.finer(
        'IdentifyService._consumeMessage: Current known protocols for $p: ${supported.length}');
    final mesProtocols =
        mes.protocols.map((s) => s).toList(); // Already List<String>
    _log.finer(
        'IdentifyService._consumeMessage: Received protocols from $p: ${mesProtocols.length}');
    ...
    if (isPush) {
      _log.fine(
          'IdentifyService._consumeMessage: Emitting EvtPeerProtocolsUpdated for $p due to PUSH.');
    ...
    if (mes.observedAddr.isNotEmpty) {
      try {
        ...
        _log.fine(
            'IdentifyService._consumeMessage: Parsed observed address for $p: $obsAddr');
      } catch (e) {
        _log.warning(
            'IdentifyService._consumeMessage: Error parsing received observed addr for $p from conn ${conn.id}: $e');
      }
    } else {
      _log.fine(
          'IdentifyService._consumeMessage: No observed address in message from $p.');
    }

    if (obsAddr != null && !disableObservedAddrManager) {
      _log.fine(
          'IdentifyService._consumeMessage: Recording observed address $obsAddr for $p via conn ${conn.id}.');
    ...
    _log.finer(
        'IdentifyService._consumeMessage: Processing ${laddrs.length} listen addresses from $p.');
    for (final addrBytes in laddrs) {
      try {
        ...
        _log.warning(
            'IdentifyService._consumeMessage: Failed to parse listen multiaddr from $p (conn ${conn.id}, remote ${conn.remoteMultiaddr}): $e. AddrBytes: $addrBytes');
      }
    }
    _log.fine(
        'IdentifyService._consumeMessage: Parsed ${lmaddrs.length} listen addresses for $p.');

    Envelope? signedPeerRecord;
    if (mes.signedPeerRecord.isNotEmpty) {
      try {
        _log.finer(
            'IdentifyService._consumeMessage: Attempting to parse signed peer record for $p (${mes.signedPeerRecord.length} bytes).');
        signedPeerRecord = await signedPeerRecordFromMessage(mes);
        _log.fine(
            'IdentifyService._consumeMessage: Parsed signed peer record for $p: ${signedPeerRecord != null}');
      } catch (e) {
        _log.warning(
            'IdentifyService._consumeMessage: Error getting peer record from Identify message for $p: $e');
      }
    } else {
      _log.finer(
          'IdentifyService._consumeMessage: No signed peer record in message from $p.');
    }

    Duration ttl = RecentlyConnectedAddrTTL;
    final connectedness = host.network.connectedness(p);
    _log.finer(
        'IdentifyService._consumeMessage: Connectedness for $p: $connectedness. Default TTL: $ttl');
    switch (connectedness) {
      case Connectedness.limited:
      case Connectedness.connected:
        ttl = ConnectedAddrTTL;
        _log.finer(
            'IdentifyService._consumeMessage: Peer $p is connected/limited, using TTL: $ttl');
        break;
      default:
        _log.finer(
            'IdentifyService._consumeMessage: Peer $p not connected/limited, using default TTL: $ttl');
        break;
    }

    _log.finer(
        'IdentifyService._consumeMessage: Updating address book for $p. Current TTLs: RecentlyConnectedAddrTTL=$RecentlyConnectedAddrTTL, ConnectedAddrTTL=$ConnectedAddrTTL, TempAddrTTL=$TempAddrTTL.');
    await host.peerStore.addrBook
        .updateAddrs(p, RecentlyConnectedAddrTTL, TempAddrTTL);
    ...
    if (signedPeerRecord != null) {
      try {
        _log.finer(
            'IdentifyService._consumeMessage: Consuming signed peer record for $p.');
        final signedAddrs =
            await _consumeSignedPeerRecord(conn.remotePeer, signedPeerRecord);
        addrsToStore = signedAddrs;
        _log.fine(
            'IdentifyService._consumeMessage: Addresses from signed peer record for $p: ${addrsToStore.length}');
      } catch (e) {
        _log.warning(
            'IdentifyService._consumeMessage: Failed to consume signed peer record for $p: $e. Falling back to listen addresses from message.');
      ...
      addrsToStore = lmaddrs;
      _log.finer(
          'IdentifyService._consumeMessage: Using listen addresses from message for $p as no valid signed record was processed.');
    }

    _log.finer(
        'IdentifyService._consumeMessage: Filtering addresses for $p based on remote connection address ${conn.remoteMultiaddr}. Before filter: ${addrsToStore.length}');
    addrsToStore = _filterAddrs(addrsToStore, conn.remoteMultiaddr);
    _log.finer(
        'IdentifyService._consumeMessage: After filter for $p: ${addrsToStore.length}');

    if (addrsToStore.length > connectedPeerMaxAddrs) {
      _log.fine(
          'IdentifyService._consumeMessage: Too many addresses for $p (${addrsToStore.length} > $connectedPeerMaxAddrs). Trimming.');
      addrsToStore = addrsToStore.sublist(0, connectedPeerMaxAddrs);
    }

    _log.fine(
        'IdentifyService._consumeMessage: Adding ${addrsToStore.length} addresses for $p to peerstore with TTL $ttl.');
    await host.peerStore.addrBook.addAddrs(p, addrsToStore, ttl);
    _log.finer(
        'IdentifyService._consumeMessage: Updating TempAddrTTL to zero for $p.');
    ...
    _log.fine(
        'IdentifyService._consumeMessage: Storing metadata for $p: ProtocolVersion=$pv, AgentVersion=$av');
    host.peerStore.peerMetadata.put(p, 'ProtocolVersion', pv);
    host.peerStore.peerMetadata.put(p, 'AgentVersion', av);

    _log.finer(
        'IdentifyService._consumeMessage: Consuming received public key for $p.');
    await _consumeReceivedPubKey(
        conn, Uint8List.fromList(mes.publicKey)); // Await this
    ...
    if (obsAddr != null) {
      _evtPeerIdentificationCompleted.emit(EvtPeerIdentificationCompleted(
        ...
        conn: conn,
        listenAddrs:
            lmaddrs, // Use the initially parsed listen addrs for the event
        protocols: mesProtocols,
        signedPeerRecord:
            signedPeerRecord, // Use the potentially invalidated one if consumption failed
        agentVersion: av,
        protocolVersion: pv,
    ...
    _log.fine(
        'IdentifyService._consumeMessage: Finished consuming message from $p.');
  }

  Future<Envelope?> signedPeerRecordFromMessage(Identify msg) async {
    _log.finer(
        'IdentifyService.signedPeerRecordFromMessage: Checking for signed peer record.');
    if (msg.signedPeerRecord.isEmpty) {
      _log.finer(
          'IdentifyService.signedPeerRecordFromMessage: No signed peer record in message.');
      return null;
    }
    _log.finer(
        'IdentifyService.signedPeerRecordFromMessage: Attempting to consume envelope from ${msg.signedPeerRecord.length} bytes.');
    try {
      final (envelope, _) = await Envelope.consumeEnvelope(
          Uint8List.fromList(msg.signedPeerRecord), PeerRecordEnvelopeDomain);
      _log.fine(
          'IdentifyService.signedPeerRecordFromMessage: Successfully consumed envelope.');
      return envelope;
    } catch (e) {
      _log.warning(
          'IdentifyService.signedPeerRecordFromMessage: Failed to parse signed peer record: $e');
      return null;
    }
  }

  Future<List<MultiAddr>> _consumeSignedPeerRecord(
    ...
    PeerId id;
    try {
      id = await PeerId.fromPublicKey(
          signedPeerRecord.publicKey); // Added null check operator
      _log.finer(
          'IdentifyService._consumeSignedPeerRecord: Derived PeerId $id from record public key for $p.');
    } catch (e) {
      _log.warning(
          'IdentifyService._consumeSignedPeerRecord: Failed to derive peer ID from record public key for $p: $e');
      throw Exception("failed to derive peer ID: $e");
    }
    if (id != p) {
      _log.warning(
          'IdentifyService._consumeSignedPeerRecord: Signed peer record envelope for unexpected peer ID. Expected $p, got $id.');
      throw Exception(
          "received signed peer record envelope for unexpected peer ID. expected $p, got $id");
    }
    pr.PeerRecord record;
    try {
      record = await signedPeerRecord.record();
      _log.finer(
          'IdentifyService._consumeSignedPeerRecord: Obtained record from envelope for $p.');
    } catch (e) {
      _log.warning(
          'IdentifyService._consumeSignedPeerRecord: Failed to obtain record from envelope for $p: $e');
      throw Exception("failed to obtain record: $e");
    }
    // final peerRecord = record as PeerRecord?; // Already casted
    if (PeerId.fromBytes(Uint8List.fromList(record.peerId)) != p) {
      // Use 'record' directly
      _log.warning(
          'IdentifyService._consumeSignedPeerRecord: Record peer ID mismatch. Expected $p, got ${record.peerId}.');
      throw Exception(
          "received signed peer record for unexpected peer ID. expected $p, got ${record.peerId}");
    }
    _log.fine(
        'IdentifyService._consumeSignedPeerRecord: Successfully consumed signed peer record for $p. Found ${record.addresses.length} addresses.');
    return record.addresses
        .map((el) => MultiAddr.fromBytes(Uint8List.fromList(el.multiaddr)))
        .toList();
  }
  ...
  Future<void> _consumeReceivedPubKey(Conn conn, Uint8List? kb) async {
    ...
    _log.finer(
        'IdentifyService._consumeReceivedPubKey: Consuming public key for remote peer $rp from local peer $lp.');

    if (kb == null || kb.isEmpty) {
      _log.fine(
          'IdentifyService._consumeReceivedPubKey: $lp did not receive public key for remote peer $rp (key bytes null or empty).');
      return;
    }
    _log.finer(
        'IdentifyService._consumeReceivedPubKey: Received ${kb.length} bytes for public key of $rp.');
    ...
    try {
      ...
      _log.finer(
          'IdentifyService._consumeReceivedPubKey: Successfully unmarshalled public key for $rp.');
    } catch (e) {
      _log.warning(
          'IdentifyService._consumeReceivedPubKey: $lp cannot unmarshal key from remote peer $rp: $e');
      return;
    }
    ...
    try {
      ...
      _log.finer(
          'IdentifyService._consumeReceivedPubKey: Derived PeerId $np from received public key for $rp.');
    } catch (e) {
      _log.warning(
          'IdentifyService._consumeReceivedPubKey: $lp cannot get peer.ID from key of remote peer $rp: $e');
      return;
    }
    ...
    if (np != rp) {
      ...
      if (rp.toString().isEmpty && np.toString().isNotEmpty) {
        _log.fine(
            'IdentifyService._consumeReceivedPubKey: Remote peer $rp was initially empty, now identified as $np. Attempting to add key.');
        try {
          host.peerStore.keyBook.addPubKey(rp,
              newKey); // Use original rp if it's a placeholder that needs updating
          _log.fine(
              'IdentifyService._consumeReceivedPubKey: Added key for initially-empty $rp (now $np) to peerstore.');
        } catch (e) {
          _log.warning(
              'IdentifyService._consumeReceivedPubKey: $lp could not add key for initially-empty $rp (now $np) to peerstore: $e');
        }
      } else {
        _log.severe(
            'IdentifyService._consumeReceivedPubKey: $lp received key for remote peer $rp, but derived PeerId $np MISMATCHES!');
      }
      return;
    }

    _log.finer(
        'IdentifyService._consumeReceivedPubKey: Derived PeerId $np matches expected remote peer $rp.');
    final currKey = await host.peerStore.keyBook.pubKey(rp);

    if (currKey == null) {
      _log.fine(
          'IdentifyService._consumeReceivedPubKey: No existing public key for $rp in peerstore. Adding received key.');
      try {
        host.peerStore.keyBook.addPubKey(rp, newKey);
        _log.fine(
            'IdentifyService._consumeReceivedPubKey: Added new public key for $rp to peerstore.');
      } catch (e) {
        _log.warning(
            'IdentifyService._consumeReceivedPubKey: $lp could not add new key for $rp to peerstore: $e');
      }
      return;
    }

    _log.finer(
        'IdentifyService._consumeReceivedPubKey: Existing public key found for $rp. Comparing with received key.');
    if (await currKey.equals(newKey)) {
      _log.fine(
          'IdentifyService._consumeReceivedPubKey: Received public key for $rp matches existing key in peerstore.');
      return;
    }

    _log.warning(
        'IdentifyService._consumeReceivedPubKey: $lp identify got a DIFFERENT key for $rp, but derived PeerID matches. This is unusual.');
    ...
    try {
      ...
      _log.severe(
          'IdentifyService._consumeReceivedPubKey: $lp cannot get peer.ID from LOCAL key of remote peer $rp: $e. This indicates peerstore corruption or a major issue.');
      return;
    }
    if (cp != rp) {
      _log.severe(
          'IdentifyService._consumeReceivedPubKey: $lp local key for remote peer $rp yields different peer.ID $cp. Peerstore might be inconsistent for $rp.');
      return;
    }
    _log.severe(
        'IdentifyService._consumeReceivedPubKey: Both local key and received key for $rp derive to the same PeerID $rp, but the keys themselves are different. This should ideally not happen if keys are canonical.');
  }

  List<MultiAddr> _filterAddrs(List<MultiAddr> addrs, MultiAddr remote) {
    _log.finer(
        'IdentifyService._filterAddrs: Filtering ${addrs.length} addresses based on remote address type: ${remote.toString()} (Loopback: ${remote.isLoopback()}, Private: ${remote.isPrivate()}, Public: ${remote.isPublic()})');
    if (remote.isLoopback()) {
      _log.finer(
          'IdentifyService._filterAddrs: Remote is loopback, returning all addresses.');
      return addrs;
    } else if (remote.isPrivate()) {
      final filtered = addrs.where((a) => !a.isLoopback()).toList();
      _log.finer(
          'IdentifyService._filterAddrs: Remote is private, filtering out loopback. Before: ${addrs.length}, After: ${filtered.length}');
      return filtered;
    } else if (remote.isPublic()) {
      final filtered = addrs.where((a) => a.isPublic()).toList();
      _log.finer(
          'IdentifyService._filterAddrs: Remote is public, filtering for public only. Before: ${addrs.length}, After: ${filtered.length}');
      return filtered;
    } else {
      _log.finer(
          'IdentifyService._filterAddrs: Remote address type unknown or not handled, returning all addresses.');
      return addrs;
    }
  }

  (List<ProtocolID>, List<ProtocolID>) _diff(
      List<ProtocolID> before, List<ProtocolID> after) {
    _log.finer(
        'IdentifyService._diff: Calculating protocol diff. Before: ${before.length}, After: ${after.length}');
    ...
    _log.finer(
        'IdentifyService._diff: Added: ${added.length}, Removed: ${removed.length}');
  ...
  Future<void> identifyConn(Conn conn) async {
    _log.fine(
        'IdentifyService.identifyConn: Called for peer ${conn.remotePeer}. Delegating to identifyWait.');
    await identifyWait(conn);
    _log.fine(
        'IdentifyService.identifyConn: identifyWait completed for peer ${conn.remotePeer}.');
  }
  ...
  void _spawnIdentifyConn(Conn conn, Entry entry) {
    ...
    _identifyConn(conn).then((_) {
      ...
      _log.fine(
          ' [SPAWN-IDENTIFY-SUCCESS] Identify succeeded for peer=$peerId, duration=${duration.inMilliseconds}ms');
      ...
      if (!entry.identifyWaitCompleter!.isCompleted) {
        ...
        _log.warning(
            ' [SPAWN-IDENTIFY-ERROR-ALREADY-COMPLETED] identifyWaitCompleter for peer=$peerId was already completed when error occurred');
      }
    });
  }
  ...
  Future<void> _identifyConn(Conn conn) async {
    ...
    try {
      ...
      final streamNegotiationDuration =
          DateTime.now().difference(identifyStart);
      if (stream == null) {
        _log.severe(
            ' [IDENTIFY-CONN-STREAM-FAILED] peer=$peerId, duration=${streamNegotiationDuration.inMilliseconds}ms, reason=stream_negotiation_failed');
        throw IdentifyStreamException(
          peerId: peerId,
      ...
      if (e.toString().contains('closed by remote') ||
          ...
          e.toString().contains('Stream closed by remote')) {
        _log.severe(
            ' [IDENTIFY-CONN-PREMATURE-CLOSURE] This is the target error! peer=$peerId, conn_age=${connAge.inMilliseconds}ms, stream_id=${stream?.id() ?? 'null'}');
      }
  ...
  Entry _addConnWithLock(Conn conn) {
    // Return type changed to Entry
    _log.finer(
        'IdentifyService._addConnWithLock: Adding connection for ${conn.remotePeer}. Conn ID: ${conn.id}');
    var entry = _conns[conn];
    if (entry == null) {
      if (!_setupCompleted.isCompleted) {
        _log.severe(
            'IdentifyService._addConnWithLock: Identify service not started, cannot add connection for ${conn.remotePeer}.');
        throw Exception('Identify service not started');
      }
      _log.finer(
          'IdentifyService._addConnWithLock: Creating new Entry object for ${conn.remotePeer}.');
      entry = Entry();
      _conns[conn] = entry;
    } else {
      _log.finer(
          'IdentifyService._addConnWithLock: Entry for ${conn.remotePeer} already exists.');
    }
    return entry;
  }
  ...
  List<MultiAddr> ownObservedAddrs() {
    if (disableObservedAddrManager) {
      _log.finer(
          'IdentifyService.ownObservedAddrs: Observed address manager disabled, returning empty list.');
      return [];
    }
    final addrs = _observedAddrMgr!.addrs();
    _log.finer(
        'IdentifyService.ownObservedAddrs: Returning ${addrs.length} observed addresses.');
    return addrs;
  }
  ...
  List<MultiAddr> observedAddrsFor(MultiAddr local) {
    if (disableObservedAddrManager) {
      _log.finer(
          'IdentifyService.observedAddrsFor: Observed address manager disabled, returning empty list for local addr $local.');
      return [];
    }
    final addrs = _observedAddrMgr!.addrsFor(local);
    _log.finer(
        'IdentifyService.observedAddrsFor: Returning ${addrs.length} observed addresses for local addr $local.');
    return addrs;
  }
  ...
  Future<void> close() async {
    ...
    _log.fine(
        'IdentifyService.close: Shutdown signal set, stopping new PUSH operations.');
    ...
    await _pushOperationsMutex.synchronized(() async {
      ...
      _log.fine(
          'IdentifyService.close: Found ${activePushFutures.length} active PUSH operations to wait for.');
    });

    if (activePushFutures.isNotEmpty) {
      _log.fine(
          'IdentifyService.close: Waiting for ${activePushFutures.length} active PUSH operations to complete...');
      try {
        // Wait for all PUSH operations with a reasonable timeout
        await Future.wait(activePushFutures)
            .timeout(const Duration(seconds: 5));
        _log.fine(
            'IdentifyService.close: All active PUSH operations completed.');
      } catch (e) {
        _log.warning(
            'IdentifyService.close: Some PUSH operations did not complete within timeout: $e');
    ...
    if (!disableObservedAddrManager) {
      _log.finer(
          'IdentifyService.close: Closing ObservedAddrManager and NATEmitter.');
...
class Semaphore {
  ...
  Future<void> acquire() async {
    _log.finest(
        'Semaphore.acquire: Attempting to acquire. Current: $_current, Max: $_maxConcurrency, Queue: ${_queue.length}');
    if (_current < _maxConcurrency) {
      _current++;
      _log.finest(
          'Semaphore.acquire: Acquired immediately. Current: $_current');
      return Future.value();
    }
    final completer = Completer<void>();
    _queue.add(completer);
    _log.finest(
        'Semaphore.acquire: Queued. Current: $_current, Queue: ${_queue.length}');
  ...
  void release() {
    _current--;
    _log.finest(
        'Semaphore.release: Released. Current: $_current, Queue: ${_queue.length}');
    if (_queue.isNotEmpty) {
      final completer = _queue.removeAt(0);
      _current++;
      completer.complete();
      _log.finest(
          'Semaphore.release: Dequeued and completed. Current: $_current');
    }
  }
}
...
class _NetNotifiee implements Notifiee {
  ...
  Future<void> connected(Network network, Conn conn,
    ...
    final peerId = conn.remotePeer;
    _log.fine(
        'Identify.Notifiee.connected: Connection established with $peerId. Conn ID: ${conn.id}, Direction: ${conn.stat.stats.direction}');
    await _ids._connsMutex.synchronized(() async {
      _log.finer(
          'Identify.Notifiee.connected: Acquired _connsMutex for $peerId.');
      _ids._addConnWithLock(conn); // Ensure entry exists
      _log.finer(
          'Identify.Notifiee.connected: Released _connsMutex for $peerId.');
    });
    ...
    if (conn.stat.stats.direction == Direction.outbound) {
      ...
      _ids.identifyWait(conn).catchError((e, st) {
        _log.warning(
            'Identify.Notifiee.connected: identifyWait for outbound $peerId failed in background: $e\n$st');
  ...
  Future<void> disconnected(Network network, Conn conn) async {
    final peerId = conn.remotePeer;
    _log.fine(
        'Identify.Notifiee.disconnected: Connection disconnected with $peerId. Conn ID: ${conn.id}');
    await _ids._connsMutex.synchronized(() async {
      _log.finer(
          'Identify.Notifiee.disconnected: Acquired _connsMutex for $peerId.');
      final removedEntry = _ids._conns.remove(conn); // Removed await
      _log.finer(
          'Identify.Notifiee.disconnected: Removed connection entry for $peerId (was present: ${removedEntry != null}). Released _connsMutex.');
    });

    if (!_ids.disableObservedAddrManager && _ids._observedAddrMgr != null) {
      _log.finer(
          'Identify.Notifiee.disconnected: Removing conn ${conn.id} from ObservedAddrManager for $peerId.');
      _ids._observedAddrMgr!.removeConn(conn);
    }

    final connectedness = network.connectedness(peerId);
    _log.finer(
        'Identify.Notifiee.disconnected: Current connectedness for $peerId: $connectedness.');
    switch (connectedness) {
      case Connectedness.connected:
      case Connectedness.limited:
        _log.finer(
            'Identify.Notifiee.disconnected: Peer $peerId still connected/limited, no special addr processing.');
        return;
      default:
        _log.finer(
            'Identify.Notifiee.disconnected: Peer $peerId not connected/limited. Processing addresses for recently disconnected.');
        break;
    }
    ...
    if (isProtected) {
      _log.fine(
          'Identify.Notifiee.disconnected: Peer $peerId is protected, preserving addresses at connectedAddrTTL');
      return;
    }

    final addrs = await _ids.host.peerStore.addrBook.addrs(peerId);
    _log.finer(
        'Identify.Notifiee.disconnected: Found ${addrs.length} addresses for $peerId in peerstore.');
    var n = addrs.length;
    if (n > recentlyConnectedPeerMaxAddrs) {
      _log.finer(
          'Identify.Notifiee.disconnected: Trimming addresses for $peerId from $n to $recentlyConnectedPeerMaxAddrs.');
      ...
      for (var i = 0; i < addrs.length; i++) {
        if (addrs[i].equals(conn.remoteMultiaddr)) {
          ...
          addrs[0] = temp;
          _log.finer(
              'Identify.Notifiee.disconnected: Prioritized disconnected addr ${conn.remoteMultiaddr} for $peerId.');
          break;
        }
      }
    ...
    _log.finer(
        'Identify.Notifiee.disconnected: Updating address TTLs for $peerId.');
    await _ids.host.peerStore.addrBook
        .updateAddrs(peerId, ConnectedAddrTTL, TempAddrTTL);
    await _ids.host.peerStore.addrBook
        .addAddrs(peerId, addrs.sublist(0, n), RecentlyConnectedAddrTTL);
    await _ids.host.peerStore.addrBook
        .updateAddrs(peerId, TempAddrTTL, Duration.zero);
    _log.fine(
        'Identify.Notifiee.disconnected: Address processing complete for $peerId.');
  }
