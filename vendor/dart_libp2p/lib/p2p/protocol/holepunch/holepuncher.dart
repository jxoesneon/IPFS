...
class HolePuncher {
    ...
    for (int i = 1; i <= maxRetries; i++) {
      try {
        ...
        try {
          ...
          _tracer?.holePunchFinished('initiator', i, addrs, obsAddrs,
  ...
  Future<HolePunchResult> _initiateHolePunchImpl(P2PStream str) async {
    ...
    try {
      ...
      if (obsAddrs.isEmpty) {
        _log.warning(
            'No public addresses available for hole punch initiation, but proceeding anyway. Peer: ${str.conn.remotePeer}');
...
abstract class HolePunchTracer {
  ...
  void holePunchFinished(String side, int attempts, List<MultiAddr> addrs,
      List<MultiAddr> obsAddrs, Conn? conn);
