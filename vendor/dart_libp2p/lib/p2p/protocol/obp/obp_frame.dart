...
class OBPFrame {
  ...
  Uint8List encode() {
    if (payload.length > MAX_PAYLOAD_SIZE) {
      throw ArgumentError(
          'Payload too large: ${payload.length} > $MAX_PAYLOAD_SIZE');
    }
  ...
  static OBPFrame decode(Uint8List data) {
    ...
    if (magic != MAGIC) {
      throw FormatException(
          'Invalid magic: 0x${magic.toRadixString(16)} != 0x${MAGIC.toRadixString(16)}');
    }
    offset += 4;
    ...
    if (data.length < HEADER_SIZE + length) {
      throw FormatException(
          'Incomplete frame: ${data.length} < ${HEADER_SIZE + length}');
    }
...
enum OBPFlags {
  ...
  String toString() {
    ...
    return flags.isEmpty
        ? 'OBPFlags(0x${value.toRadixString(16)})'
        : 'OBPFlags(${flags.join('|')})';
  }
}
