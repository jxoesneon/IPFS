...
class OBPProtocolHandler {
  ...
  static Future<bool> performHandshake(
    ...
    try {
      if (isClient) {
        return await _performClientHandshake(stream,
            timeout: timeout, context: context);
      } else {
        return await _performServerHandshake(stream,
            timeout: timeout, context: context);
      }
    } catch (e, stackTrace) {
  ...
  static Future<OBPFrame?> sendRequest(
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        _logger.fine(
            '[$context] Sending request (attempt $attempt/$maxRetries): ${request.type}');
        ...
        // Wait for response
        final response =
            await _readFrame(stream, timeout: timeout, context: context);

        if (response == null) {
          _logger.warning(
              '[$context] No response received for ${request.type} (attempt $attempt)');
        ...
        if (attempt >= maxRetries) {
          _logger.severe(
              '[$context] All request attempts failed for ${request.type}');
          rethrow;
        }
  ...
  static Future<bool> _performClientHandshake(
    ...
    _logger.info(
        '[$context] CLIENT: Starting OBP handshake - sending HandshakeReq');
    ...
    if (handshakeAck == null ||
        handshakeAck.type != OBPMessageType.handshakeAck) {
      _logger.warning(
          '[$context] Invalid handshake response: ${handshakeAck?.type}');
      return false;
    }
    ...
    try {
      final ackData =
          jsonDecode(utf8.decode(handshakeAck.payload)) as Map<String, dynamic>;
      final serverCapabilities =
          List<String>.from(ackData['capabilities'] as List);
      _logger.info(
          '[$context] Handshake successful, server capabilities: $serverCapabilities');
      return true;
    } catch (e) {
      _logger
          .warning('[$context] Failed to parse handshake acknowledgment: $e');
      return false;
    }
  }
  ...
  static Future<bool> _performServerHandshake(
    ...
    if (handshakeReq == null ||
        handshakeReq.type != OBPMessageType.handshakeReq) {
      _logger.warning(
          '[$context] Invalid handshake request: ${handshakeReq?.type}');
      return false;
    }
    ...
    try {
      final reqData =
          jsonDecode(utf8.decode(handshakeReq.payload)) as Map<String, dynamic>;
      final clientCapabilities =
          List<String>.from(reqData['capabilities'] as List);
      _logger.info(
          '[$context] Handshake request received, client capabilities: $clientCapabilities');
    } catch (e) {
  ...
  static Future<OBPFrame?> _readFrame(
    ...
    try {
      ...
      final headerData = await _readExactBytes(stream, OBPFrame.HEADER_SIZE,
          timeout: timeout, context: context);
      if (headerData == null) {
      ...
      final payloadLength =
          headerBuffer.getUint32(8, Endian.big); // Length field at offset 8

      // Read payload if present
      Uint8List? payloadData;
      if (payloadLength > 0) {
        payloadData = await _readExactBytes(stream, payloadLength,
            timeout: timeout, context: context);
        if (payloadData == null) {
      ...
      _logger
          .fine('[$context] Read frame: ${frame.type} (${frame.length} bytes)');
      return frame;
    } catch (e, stackTrace) {
  ...
  static Future<void> _writeFrame(
    ...
    try {
      ...
      _logger
          .info('[$context] WRITE_VERIFY: About to write ${frame.type} frame');
      _logger.info(
          '[$context] WRITE_VERIFY: Frame size: ${frameData.length} bytes');
      _logger.info(
          '[$context] WRITE_VERIFY: Frame header (first 16 bytes): ${frameData.take(16).map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}');
      _logger.info(
          '[$context] WRITE_VERIFY: Stream state before write - isClosed: ${stream.isClosed}');
      ...
      _logger.info(
          '[$context] WRITE_VERIFY: stream.write() completed successfully in ${writeDuration.inMilliseconds}ms');
      _logger.info(
          '[$context] WRITE_VERIFY: Stream state after write - isClosed: ${stream.isClosed}');
      ...
      try {
        ...
        if (streamDynamic.runtimeType.toString().contains('flush')) {
          ...
          _logger.info(
              '[$context] WRITE_VERIFY: Stream does not support flush operation');
        }
      } catch (flushError) {
        _logger.warning(
            '[$context] WRITE_VERIFY: Stream flush failed (non-critical): $flushError');
      }

      _logger.info(
          '[$context] WRITE_VERIFY: Frame write operation completed - ${frame.type} (${frameData.length} bytes) sent successfully');
    } catch (e, stackTrace) {
      _logger.severe(
          '[$context] WRITE_VERIFY: CRITICAL - Frame write failed for ${frame.type}: $e',
          e,
          stackTrace);
      _logger.severe(
          '[$context] WRITE_VERIFY: Stream state on error - isClosed: ${stream.isClosed}');
      rethrow;
    }
  }
  ...
  static Future<Uint8List?> _readExactBytes(
    ...
    _logger.info(
        '[$context] READ_VERIFY: About to read $expectedBytes bytes from stream');
    _logger.info(
        '[$context] READ_VERIFY: Stream state before read - isClosed: ${stream.isClosed}');

    // Get or create buffer for this stream
    final streamBuffer = _streamBuffers.putIfAbsent(streamKey, () => <int>[]);
    _logger.info(
        '[$context] READ_VERIFY: Stream buffer contains ${streamBuffer.length} bytes from previous reads');
    ...
    while (streamBuffer.length < expectedBytes) {
      if (DateTime.now().isAfter(deadline)) {
        _logger.severe(
            '[$context] READ_VERIFY: Read timeout after ${readAttempts} attempts, got ${streamBuffer.length}/$expectedBytes bytes');
      ...
      try {
        readAttempts++;
        _logger.info(
            '[$context] READ_VERIFY: Read attempt $readAttempts - calling stream.read()...');
        ...
        _logger.info(
            '[$context] READ_VERIFY: stream.read() returned ${chunk.length} bytes in ${readDuration.inMilliseconds}ms');

        if (chunk.isNotEmpty) {
          _logger.info(
              '[$context] READ_VERIFY: Received data chunk (first 16 bytes): ${chunk.take(16).map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}');

          // Add all received bytes to the stream buffer
          streamBuffer.addAll(chunk);
          _logger.info(
              '[$context] READ_VERIFY: Stream buffer now contains ${streamBuffer.length} bytes (need $expectedBytes)');
        }
        ...
        if (chunk.isEmpty) {
          ...
          if (streamBuffer.isEmpty) {
            _logger
                .info('[$context] READ_VERIFY: Clean EOF - no data received');
            return null; // Clean EOF
          } else {
            _logger.severe(
                '[$context] READ_VERIFY: Unexpected EOF - got ${streamBuffer.length}/$expectedBytes bytes');
            throw FormatException(
                'Unexpected EOF: got ${streamBuffer.length}/$expectedBytes bytes');
          }
        }
      } catch (e) {
        if (e is TimeoutException) {
          _logger.warning(
              '[$context] READ_VERIFY: Read attempt $readAttempts timed out, continuing...');
          // Continue trying until overall timeout
          continue;
        }
        _logger.severe(
            '[$context] READ_VERIFY: Read attempt $readAttempts failed: $e');
        rethrow;
      }
    }
    ...
    final result =
        Uint8List.fromList(streamBuffer.take(expectedBytes).toList());

    // Remove the extracted bytes from the buffer, keeping any excess for next read
    streamBuffer.removeRange(0, expectedBytes);

    _logger.info(
        '[$context] READ_VERIFY: Successfully extracted $expectedBytes bytes after $readAttempts attempts');
    _logger.info(
        '[$context] READ_VERIFY: ${streamBuffer.length} bytes remain in stream buffer for next read');
