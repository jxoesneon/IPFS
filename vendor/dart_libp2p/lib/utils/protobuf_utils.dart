...
Future<Uint8List> _readNBytesFromP2PStream(
  ...
  while (resultBuilder.length < n) {
    ...
    if (carryOverBuffer.length >= (n - resultBuilder.length)) {
      // If carryOver now has enough
      ...
      // Not enough yet, take all of carryOverBuffer
...
Future<T> readDelimited<T extends GeneratedMessage>(
  ...
  while (true) {
    ...
    if (carryOverBuffer.isNotEmpty) {
      ...
      if (chunk.isEmpty) {
        if (stream.isClosed) {
          throw StateError(
              'P2PStream closed prematurely while reading varint length.');
        } else {
          throw StateError(
              'P2PStream read returned empty chunk while reading varint length.');
        }
      }
      carryOverBuffer.addAll(chunk);
      if (carryOverBuffer.isEmpty) {
        throw StateError(
            'P2PStream logic error: carryOverBuffer empty after adding non-empty chunk.');
      }
      byte = carryOverBuffer.removeAt(0);
    }
    ...
    if ((byte & 0x80) == 0) {
      ...
      try {
        ...
        throw FormatException(
            'Invalid varint decoding: $e. Bytes: ${varintBytesBuilder.toBytes()}');
      }
      break;
    }
    ...
    if (varintByteCount > 10) {
      throw FormatException(
          'Varint prefix is too long (max 10 bytes). Read: ${varintBytesBuilder.toBytes()}');
    }
  }
  ...
  if (messageLength == 0) {
    // Handle zero-length messages (e.g. empty protobuf message)
    return builder(Uint8List(0));
  }

  // 2. Read the message bytes
  final messageData =
      await _readNBytesFromP2PStream(stream, messageLength, carryOverBuffer);
