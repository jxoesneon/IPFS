# Ciel Council of Five Decision: Docker Runtime Base Image Strategy for dart_ipfs v2.2

**Decision ID:** `COUNCIL_DECISION_DOCKER_BASE`  
**Date:** 2026-06-25  
**Convened by:** Ciel Council of Five  
...
- `doc/specs/audits/COUNCIL_AUDIT_OPERATIONS_ECOSYSTEM.md` flags this as a cross-cutting blocker: "Choose glibc base or remove/inline libsodium dependency."
- `doc/specs/audits/COUNCIL_AUDIT_MASTER.md` lists implementation priority: "Docker images (after libsodium base decision)."
...
| **B** | 4/10 | Requires a major dependency audit and refactoring of `pubspec.yaml` and potentially `ipfs_libp2p` usage. The CLI/Docker implementation sequence in `COUNCIL_AUDIT_MASTER.md` places Docker after the libsodium decision, but this option makes the Docker work dependent on a large, uncertain crypto refactor. |
...
Per `COUNCIL_AUDIT_MASTER.md`, Docker images are implemented after the libsodium base decision. With this decision in place, the v2.2 implementation order is:
...
**Final Council Verdict:** Adopt **Option D**. Default runtime base is **`cgr.dev/chainguard/glibc-dynamic`**, with an experimental `static` variant and a `debug` variant. Amend `DOCKER_SPEC.md` accordingly.
