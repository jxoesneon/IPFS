# Council of Five Decision: Reconcile the Two IPLDCodec Interfaces in dart_ipfs

**Status:** Council Verdict  
**Date:** 2026-06-25  
**Scope:** dart_ipfs v2.0 core data layer (IPLD codecs)  
**Related specifications:** `doc/specs/features/DAG_JSON_SPEC.md`, `doc/specs/audits/COUNCIL_AUDIT_CORE_DATA_LAYER.md`, `doc/specs/PROTOCOL_COMPLIANCE_SPEC.md`
...
- **Spec direction:** `doc/specs/features/DAG_JSON_SPEC.md` mandates removing the duplicate `dag_json_codec.dart` implementation and its `IPLDCodec` interface (sections 4.4, 5, 7.4), but it does not resolve how the two interface shapes should be reconciled. The audit in `doc/specs/audits/COUNCIL_AUDIT_CORE_DATA_LAYER.md` (lines 55-58, 229-230) explicitly flags that consolidation requires merging or deprecating one interface, not merely deleting one file.

The question before the Council is therefore twofold:
...
**The Council selects Option C: a new unified `IPLDCodec` interface that combines the best of both existing contracts.**
...
- `doc/specs/audits/COUNCIL_AUDIT_CORE_DATA_LAYER.md` (lines 55-58, 229-230) — audit finding on the two interfaces.
