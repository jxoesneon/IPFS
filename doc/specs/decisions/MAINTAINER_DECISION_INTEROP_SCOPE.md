# Maintainer Decision — Interoperability Test Suite Scope for dart_ipfs v2.2

**Decision ID:** `MAINTAINER_DECISION_INTEROP_SCOPE`  
**Date:** 2026-06-25  
...
| IPNS resolution | `allowed-to-skip` or `allowed-to-fail` | IPNS resolution hardcodes `QmResolvedCid` (`ipns_handler.dart:191`) and `_publishToDHT` is stubbed (`ipns_handler.dart:194-197`). The spec also requires missing `PeerId` base36 primitives (`MAINTAINER_AUDIT_NETWORKING_P2P_1.md`, section 5.4). |
