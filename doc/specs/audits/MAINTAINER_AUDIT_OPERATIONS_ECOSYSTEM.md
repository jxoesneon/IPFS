# Project Review — Operations & Ecosystem Specs
...
### Summary of Weaknesses
- The spec makes DHT provide/find and IPNS resolution P0 release-blocking. In the current codebase, DHT and IPNS are still stabilizing (per prior maintainers notes and the `lib/src/protocols/dht/` and `lib/src/protocols/ipns/` paths). Gating the release on these tests creates a high risk of v2.2.0 slipping if DHT/IPNS specs are not fully implemented first.
...
- Workspace tooling is left as "evaluate Melos or use `pubspec_overrides.yaml`." This is not a decision; implementation will stall on tooling choice without maintainers guidance.
...
### Specific Recommendations
1. Make the decision on workspace tooling before implementation begins: recommend Melos for a monorepo with cross-package testing and version alignment, or native Dart workspaces (`pubspec_overrides.yaml`) if the team wants to avoid an extra dependency. Record the maintainer decision in the spec.
...
The maintainers should confirm the v2.0/v2.1 implementation status before committing to the v2.2 release date.
