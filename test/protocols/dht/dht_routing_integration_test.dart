...
void main() {
  group('DHT Routing Integration - Distance-based Peer Selection', () {
    ...
    test('selects closest peers by XOR distance', () {
      ...
      // Calculate distances
      final distances = peers.map((p) => metric.calculateDistanceToKey(p, targetKey)).toList();

      // Sort peers by distance
      final sortedPeers = List<PeerId>.from(peers);
      sortedPeers.sort((a, b) => 
        metric.calculateDistanceToKey(a, targetKey).compareTo(
          metric.calculateDistanceToKey(b, targetKey)));
      ...
      expect(sortedPeers[4].value, equals([255, 255, 255, 255])); // Distance = large
    });
    ...
    test('handles K closest peers selection', () {
      ...
      final peers = List.generate(10, (i) => 
        PeerId(value: Uint8List.fromList([i, 0, 0, 0])));
      ...
      for (int i = 0; i < closestPeers.length - 1; i++) {
        ...
        final distNext = metric.calculateDistanceToKey(closestPeers[i + 1], targetKey);
    ...
    test('handles edge case of identical peer IDs', () {
      ...
      expect(metric.calculateDistanceToKey(sortedPeers[0], targetKey), equals(0));
      expect(metric.calculateDistanceToKey(sortedPeers[1], targetKey), equals(0));
    });
  });
}
