...
void main() {
  ...
  group('BitswapHandler Coverage Tests', () {
    ...
    test('getStatus returns correct stats', () async {
        ...
        expect(status.keys, containsAll(['active_sessions', 'wanted_blocks', 'peers', 'blocks_received', 'blocks_sent']));
    });
    ...
    test('_handleBlocks rejects invalid blocks (SEC-002)', () async {
       ...
       final fakeCid = CID.decode('QmYwAPJzv5CZsnA6ULBXebJWvruP6P3wXhHjS2Mtc38E2z'); // Valid CID structure
    ...
    test('_handleWantlist processes items in priority order', () async {
       ...
       // Both found
       when(mockBlockStore.getBlock(cid1)).thenAnswer((_) async => 
          GetBlockResponse()..found = true..block = (block_pb.BlockProto()..data = d1..cid = CID.decode(cid1).toProto())
       );
       when(mockBlockStore.getBlock(cid2)).thenAnswer((_) async => 
          GetBlockResponse()..found = true..block = (block_pb.BlockProto()..data = d2..cid = CID.decode(cid2).toProto())
       );
