  ...
  group('PubSubClient Coverage', () {
    ...
    test('deduplicates messages', () async {
      ...
      final packet2 = NetworkPacket(
        srcPeerId: senderStr,
        datagram: Uint8List.fromList(utf8.encode(jsonEncode(msgMap2))),
      );
    ...
    test('Handles IWANT and replies with cached message', () async {
      ...
      packetHandler(
        NetworkPacket(
          srcPeerId: msgSender,
          datagram: Uint8List.fromList(utf8.encode(jsonEncode(pubMsg))),
        ),
      );
    ...
    test('Handles GRAFT and PRUNE', () async {
      ...
      packetHandler(
        NetworkPacket(
          srcPeerId: peer,
          datagram: Uint8List.fromList(utf8.encode(jsonEncode(graftMsg))),
        ),
      );
      ...
      packetHandler(
        NetworkPacket(
          srcPeerId: peer,
          datagram: Uint8List.fromList(utf8.encode(jsonEncode(pruneMsg))),
        ),
      );
    });
    ...
    test('Handles malformed JSON gracefully', () async {
      await client.start();
      packetHandler(
        NetworkPacket(srcPeerId: 'p1', datagram: Uint8List.fromList([1, 2, 3])),
      ); // Invalid UTF8/JSON
    ...
    test('Handles IWANT send error', () async {
      ...
      packetHandler(
        NetworkPacket(
          srcPeerId: sender1,
          datagram: Uint8List.fromList(utf8.encode(jsonEncode(validPubMsg))),
        ),
      );
