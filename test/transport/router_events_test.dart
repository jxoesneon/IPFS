...
void main() {
  ...
  group('NetworkPacket', () {
    test('stores srcPeerId and datagram', () {
      final packet = NetworkPacket(srcPeerId: 'QmPeer123', datagram: Uint8List.fromList([1, 2, 3]));
  ...
  group('ConnectionEvent', () {
    test('connected event has correct type', () {
      final event = ConnectionEvent(type: ConnectionEventType.connected, peerId: 'QmPeer');
      expect(event.type, equals(ConnectionEventType.connected));
    });

    test('disconnected event has correct type', () {
      final event = ConnectionEvent(type: ConnectionEventType.disconnected, peerId: 'QmPeer');
  ...
  group('MessageEvent', () {
    test('stores peerId and message', () {
      final event = MessageEvent(peerId: 'QmSender', message: Uint8List.fromList([1, 2, 3]));
  ...
  group('DHTEvent', () {
    test('valueFound event with data', () {
      final event = DHTEvent(type: DHTEventType.valueFound, data: {'key': 'value'});
  ...
  group('ErrorEvent', () {
    test('connectionError type', () {
      final event = ErrorEvent(type: ErrorEventType.connectionError, message: 'Connection failed');
    ...
    test('all error types exist', () {
      ...
      expect(ErrorEventType.values, contains(ErrorEventType.disconnectionError));
  ...
  group('StreamEvent', () {
    test('opened event', () {
      final event = StreamEvent(type: StreamEventType.opened, streamId: 'stream-123');
    ...
    test('closed event', () {
      final event = StreamEvent(type: StreamEventType.closed, streamId: 'stream-789');
