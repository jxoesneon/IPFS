  ...
  group('EnhancedCBORHandler', () {
    ...
    group('Encoding', () {
      test('encodes and decodes primitives', () async {
        final scenarios = [
          ...
          (IPLDNode()..kind = Kind.BOOL..boolValue = true, Kind.BOOL),
          (IPLDNode()..kind = Kind.INTEGER..intValue = Int64(42), Kind.INTEGER),
          (IPLDNode()..kind = Kind.FLOAT..floatValue = 3.14, Kind.FLOAT),
          (IPLDNode()..kind = Kind.STRING..stringValue = 's', Kind.STRING),
        ];
      ...
      test('encodes and decodes BYTES (Tag 45)', () async {
        ...
        final node = IPLDNode()..kind = Kind.BYTES..bytesValue = data;
      ...
      test('encodes and decodes CID LINK (Tag 6)', () async {
       ...
       test('encodes and decodes CID V1 LINK (Tag 6)', () async {
        ...
        final multihash = Uint8List.fromList([0x12, 0x20] + List.filled(32, 0)); // Valid dummy multihash (sha2-256)
      ...
      test('encodes and decodes LIST and MAP', () async {
         ...
         final list = IPLDList();
         list.values.add(IPLDNode()..kind=Kind.INTEGER..intValue=Int64(1));
         
         final map = IPLDMap();
         map.entries.add(MapEntry()..key='key'..value=(IPLDNode()..kind=Kind.STRING..stringValue='val'));
         
         final nodeMap = IPLDNode()..kind=Kind.MAP..mapValue=map;
         final nodeList = IPLDNode()..kind=Kind.LIST..listValue=list;
    ...
    group('MerkleDAG Conversion', () {
       test('convertFromMerkleDAGNode correctly shapes IPLDMap', () {
          ...
          final link = Link(name: 'link1', cid: CID.decode('QmYwAPJzv5CZsnA6ULBXebJWvruP6P3wXhHjS2Mtc38E2z'), size: 100);
       ...
       test('convertToMerkleLink converts IPLDNode map back to Link', () {
           ...
           final node = IPLDNode()
             ..kind = Kind.MAP
             ..mapValue = (IPLDMap()
               ..entries.add(MapEntry()..key='Name'..value=(IPLDNode()..kind=Kind.STRING..stringValue='linkName'))
               ..entries.add(MapEntry()..key='Tsize'..value=(IPLDNode()..kind=Kind.INTEGER..intValue=Int64(50)))
               ..entries.add(MapEntry()..key='Hash'..value=(IPLDNode()..kind=Kind.LINK..linkValue=(IPLDLink()..version=0..codec='dag-pb'..multihash=cid.multihash.toBytes())))
             );
       ...
       test('convertToMerkleLink handles hash as bytes (legacy)', () {
           ...
           final node = IPLDNode()
             ..kind = Kind.MAP
             ..mapValue = (IPLDMap()
               ..entries.add(MapEntry()..key='Hash'..value=(IPLDNode()..kind=Kind.BYTES..bytesValue=cid.multihash.toBytes()))
             );
       ...
       test('convertToMerkleLink throws if not MAP', () {
          expect(() => EnhancedCBORHandler.convertToMerkleLink(IPLDNode()..kind=Kind.STRING), throwsA(isA<IPLDEncodingError>()));
       });
    });

    group('Error Handling', () {
       test('convertCborValueToIPLDNode throws on unsupported type', () {
          expect(() => EnhancedCBORHandler.convertCborValueToIPLDNode(CborUndefined()), throwsA(isA<IPLDDecodingError>()));
       });
    });
