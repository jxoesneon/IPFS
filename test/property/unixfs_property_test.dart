...
void main() {
  ...
  group('UnixFS property-based tests', () {
    ...
    test('directories: links are name-sorted, Tsize is cumulative, and every '
        'entry resolves back to its child', () async {
      for (var i = 0; i < 25; i++) {
        ...
        for (var j = 0; j < entryCount; j++) {
          ...
          final leaf = UnixFSBuilder().build(Stream.value(data.toList())).last;
          final leafBlock = await leaf;
          await store.putBlock(leafBlock);
          final name = 'file_${j}_${rng.nextInt(1 << 20)}.bin';
          entries.add(
            UnixFSDirectoryEntry(name: name, cid: leafBlock.cid, tsize: 0),
          );
        ...
        for (final link in dir.pbNode.links) {
          expect(link.size.toInt(), equals(expectedChildTsize[link.name]));
          final resolved = await resolver.resolve(dir.cid, link.name);
          expect(resolved.toBytes(), equals(Uint8List.fromList(link.hash)));
        }
